...
class MY_Model extends CI_Model
{
    ...
    // CUSTOM Added by SATS
	protected $use_column_as_key = NULL;
	...
	public function __construct()
	{
		...
		*/
        ...
        // Logging
        $this->after_create[] = 'log';
        $this->after_update[] = 'log';
	}

	/**
	 * CUSTOM - must set log_threshold to debug in config.php
	 * Return the id and log query
	 * @param $id
	 * @param $data
	 * @return mixed
	 */
    ...
    {
	    if($this->debug_log) {
		    log_message('debug', $this->table . ' Query' . PHP_EOL . json_encode($data, JSON_PRETTY_PRINT));
	    }
    ...
    public function _prep_after_read($data, $multi = true)
    {
        ...
        if ($this->use_column_as_key !== null && isset($data[0][$this->use_column_as_key])) {
            // CUSTOM Added by SATS
            $tmp = [];
            foreach ($data as $row) {
                $tmp[$row[$this->use_column_as_key]] = $row;
            }
            $data = $tmp;
            $this->use_column_as_key = null;
    ...
    /**
     * CUSTOM Added by SATS
     * Set a table column to be used as the array key*
     * @param string $column Must be a table column, typically the primary key by
     * @return $this
     */
    public function use_column_as_key($column = '')
    {
        $this->return_as = 'array';
        $this->use_column_as_key = $this->primary_key;

        if(!empty($column)){
            $this->use_column_as_key = $column;
        }
