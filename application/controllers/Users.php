...
class Users extends MY_Controller {
	...
	/** @var Staff_accounts_model $staff_accounts_model */
        ...
        if ($this->form_validation->run() == true) {
            ...
            if( $user_class == 6 ){ // if technician, create accomodation
                ...
                $this->db->insert("accomodation", [
                    ...
                    "street_number" => $accommodationData["street_number"],
                    "street_name" => $accommodationData["street_name"],
                    "suburb" => $accommodationData["suburb"],
                    "state" => $accommodationData["state"],                    
                    "phone" => $phone,
                    "email" => $email,
                    "rate" => "",
                    "comment" => "STAFF",
    ...
    public function view($id = null, $tab = "personal") {
	    ...
	    $user = $this->staff_accounts_model->with_staff_class()->as_array()->get($id);

	    if(!$user){
		    return show_404();
	    }

	    $user['ClassName'] = $user->staff_class->ClassName;
	    ...
	    if (!is_null($user)) {
		    switch ($tab) {
                ...
                case "staff_permissions":
                    ...
                    // don't show 'Prohibited from web CRM login' permission for non-tech
                    if( $user['staff_class']->ClassID != 6 ){ // tech
                        $perm_list_sql_filter = "AND `id` != 5"; 
                    }
                    
                    // permission list
                    $data['perm_list_sql'] = $this->db->query("
                    SELECT *
                    FROM `permission_list`
                    WHERE `active` = 1
                    {$perm_list_sql_filter}
                    ");

                    // staff permission saved
                    $staff_perm_sql = $this->db->query("
                    SELECT `has_permission_on`
                    FROM `staff_permissions`
                    WHERE `staff_id` = {$id}
                    ");

                    $staff_perm_arr = [];
                    foreach( $staff_perm_sql->result() as $staff_perm_row ){
                        $staff_perm_arr[] = $staff_perm_row->has_permission_on;
                    }
                    $data['staff_perm_arr'] = $staff_perm_arr;
                ...
                */
			    ...
			    default:
				    ...
				    $callCentreData = $this->_getOtherCallCentre($user);
				    $data = array_merge($data, $devicesData, $permissionsData, $licencingData, $personalData, $callCentreData);
				    break;
		    }
        ...
        $staff_perm_sql = $this->db->query("
        SELECT COUNT(`id`) AS staff_perm_count
        FROM `staff_permissions`
        WHERE `staff_id` = {$id}
        AND `has_permission_on` = 5
        ");
        $data['prohib_using_web_crm'] = ( $staff_perm_sql->row()->staff_perm_count > 0 )?true:false;
        */                       
                ...
                $uploadFile = function($fileField, $uploadPath) use ($id) {
                    ...
                    if (isset($_FILES[$fileField]) && !empty($_FILES[$fileField]['name'])) {
                        ...
                        $uploadParams = array(
                            ...
                            'max_size' => '5000', // Set the max size to 5MB
    ...
    public function update_location_access() {
        ...
        if ($this->input->method() == 'post') {
            ...
            // update SAS access marker
            $update_data = array(
                'sas_access' => $postData["sas_access"]
            );
            
            $this->db->where('StaffID', $id);
            $this->db->update('staff_accounts', $update_data);
    ...
    public function update_password() {
        ...
        if ($this->input->method() == "post") {
            ...
            $userData = $postData['user'];
            $hashedNewPassword = password_hash($userData['NewPassword'], PASSWORD_DEFAULT);

            $this->db->trans_start();
            $this->db->set('password_new', $hashedNewPassword);
            $this->db->where('StaffID', $StaffID);
            $affectedRows = $this->db->update('staff_accounts');
    ...
    public function index()
    {
        ...
        $data['title'] = $this->config->item('company_name_short') . " Users";
        ...
        if( $export == 1 && $tab == 1){
            ...
            // check permission
            $permission = 10; // Can edit permissions
            $staff_perm_sql = $this->db->query("
            SELECT COUNT(`id`) AS count
            FROM `staff_permissions`
            WHERE `staff_id` = {$this->session->staff_id}
            AND `has_permission_on` = {$permission}
            ");
            $data['can_edit_permissions'] = ( $staff_perm_sql->row()->count > 0 )?true:false;
    ...
    public function change_address() {
        ...
        // FORM SUBMIT
        if($this->input->post('btn_change_address')){
           ...
           $tmp_move_date = $this->input->post('move_date');
           $move_date = $this->system_model->formatDate($tmp_move_date);

           $address = "{$street_number} {$street_name} {$suburb} {$state} {$postcode}";
        ...
        $email_data['street_number'] = $street_number;
        $email_data['street_name'] = $street_name;
        $email_data['suburb'] = $suburb;
        $email_data['state'] = $state;
        $email_data['postcode'] = $postcode;
        $email_data['move_date'] = $move_date;

        $to = make_email('hr');
        $cc = make_email('operations');
        $subject = "Change of Address for {$email_data['employee_name']}";

        $this->email->to($to);
        $this->email->cc($cc);
        $this->email->subject($subject);
        $e_body = $this->load->view('emails/change_address_email', $email_data, TRUE);
        $this->email->message($e_body);
        $this->email->send();

        $success_message = "Change of address has been succesfully sent";

        $this->session->set_flashdata(array('success_msg'=>$success_message,'status'=>'success'));
        redirect(base_url('/users/change_address/'), 'refresh'); 

        }

        $data['title'] = "Change Of Address Form";
        ...
        //load views
        $this->load->view('templates/inner_header', $data);
        $this->load->view('reports/change_address', $data);
        $this->load->view('templates/inner_footer', $data);
    }
    ...
    public function update_leave_details(){
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($send_email) {
                ...
                $pdf_data = $this->getLeavePdf('S', $leave_id);
                $pdf_filename = 'leave_request' . date('dmYHis') . '.pdf';
                $today = date('Y-m-d');

                $this->db->select('hr_app_by, backup_leave');
                $this->db->from('leave');
                ...
                //Employee name
                $employee_name_params = array('sel_query' => 'sa.FirstName, sa.LastName, sa.Email','staff_id'=>$employee);
                $employee_name_row = $this->gherxlib->getStaffInfo($employee_name_params)->row_array();
                ...
                //HR manager name
                $hr_params = array('sel_query' => 'sa.FirstName, sa.LastName, sa.Email','staff_id'=>$data['hr_app_by']);
                $hr_row = $this->gherxlib->getStaffInfo($hr_params)->row_array();
                ...
                $to = $employee_name_row['Email'];
                $subject = "Approved: {$tol_str} from {$lday_of_work} to {$fday_back}";

                $this->email->to($to);
                $this->email->subject($subject);
                // Add HR
                $this->email->cc($hr_row['Email']);
                // Add LM
                $this->email->cc($line_manager_row['Email']);
    ...
    public function ajax_prohibit_web_crm_access(){

        $no_web_crm_access = $this->input->get_post('no_web_crm_access');
        $staff_id = $this->input->get_post('staff_id');
        $staff_permission = 5; // Prohibited from web CRM login

        if( $staff_id > 0 ){

            // clear db first, this also help fix duplicates
            $this->db->query("
            DELETE
            FROM `staff_permissions`
            WHERE `staff_id` = {$staff_id}
            AND `has_permission_on` = {$staff_permission}
            ");

            if( $no_web_crm_access == 1 ){ // ticked

                // insert
                $data = array(
                    'staff_id' => $staff_id,
                    'has_permission_on' => $staff_permission
                );            
    ...
    private function _getOtherCallCentre($user)
    {
        if(empty($user)){
            log_message('error', '_getOtherCallCentre: Empty user');
            return false;
        }
        
        $callCentre = $this->db->select('FirstName as callCentreFirstName, LastName as callCentreLastName')
            ->from('staff_accounts')
            ->where('active', 1)
            ->where('StaffID', $user['other_call_centre'])
            ->get()->row_array();

        return compact("callCentre");
    }
    ...
    public function assign_permissions() {
        ...
        $data['title'] = $title;
        $uri = '/users/assign_permissions';
        $data['uri'] = $uri;

        // permission list
        $data['perm_list_sql_res'] = $this->db->query("
        SELECT *
        FROM `permission_list`
        WHERE `active` = 1
        ")->result();
        ...
        // get staff
        $data['staff_account_sql'] = $this->db->query("
        SELECT 
            sa.`StaffID`,
            sa.`FirstName`,
            sa.`LastName`
    ...
    public function save_multiple_user_permissions(){

        $staff_arr = $this->input->get_post('staff_arr');
        $permission_arr = $this->input->get_post('permission_arr');
    ...
    public function remove_multiple_user_permissions(){
