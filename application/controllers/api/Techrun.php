...
class TechRun extends MY_ApiController {
    ...
    public function add_kms() {
        ...
        $kms_data = $this->kms_model->getKmsByVehicleId($vehicleId, $upload)->result_array();
        ...
        foreach($kms_data as $key){
            ...
            $updated_date = explode(" ",$key['kms_updated']);
            $kms_id = $key['kms_id'];

            if($key['kms'] == $kms){

                if($date == $updated_date[0]){
                    ...
                    $this->api->setStatusCode(201);
                    $this->api->setSuccess(true);
                    $this->api->setMessage('Kms needs to update.');
    ...
    public function get_kms(){
        ...
        $tmpkmsdata = explode(" ",$kms_data[0]['kms_updated']);
        $last_kms = $tmpkmsdata[0];
        $today = date("Y-m-d");
    ...
    public function can_take_break() {
        ...
        if (time() >= $lunchBreakTime) {
            ...
            if($techBreaks == 0) {
                ...
                $techBreaks = $this->db->query("
                    SELECT `tech_break_id`, `tech_break_start`
                    FROM `tech_breaks`
                    WHERE `tech_break_taken` = 1
                    AND `tech_id` = {$techId}
                    ORDER BY `tech_break_id` DESC
                    LIMIT 1
                ")->result_array();

                if(!empty($techBreaks)) {
                    $this->api->putData('tech_breaks', $techBreaks[0]);
                    $this->api->setSuccess(true);
                } else {
                    $this->api->setSuccess(false);
                }
