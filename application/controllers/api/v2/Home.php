...
class Home extends MY_ApiController {
    ...
    public function alarm_stocks() {
        ...
        $techStock_result= $this->db->select("tech_stock_id")
        ->from("tech_stock")
        ...
        ->get();
        if($techStock_result) {
            $techStockId = $techStock_result->row()->tech_stock_id;

            $stocks_result = $this->db->select("s.stocks_id, s.code, s.item, s.display_name, s.carton, tsi.quantity")
            ->from("stocks AS s")
            ...
            ->get();
            if($stocks_result) {
                $stocks = $stocks_result->result_array();
            }
        }
