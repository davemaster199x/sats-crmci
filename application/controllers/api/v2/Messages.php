...
class Messages extends MY_ApiController {
    ...
    public function mark_all_as_read(){
        ...
        $results = $this->messages_model->mark_as_read_all($staffId);
    ...
    /*
    *   API to get last unread message for alert
    *
    */
    public function get_last_unread_message()
    {
        $data = $this->messages_model->get_last_unread_message($this->api->getJWTItem('staff_id'));

        if (empty($data)) {
            $this->api->setSuccess(false);
            $this->api->putData('message', []);
        } else {
            $this->api->setSuccess(true);
            $this->api->putData('message', $data[0]);
        }
    }
}
