...
class Tech_docs extends MY_ApiController
{
    ...
    public function section_by_header_name()
    {
        ...
        if ($headerName) {
            ...
            if( $headerName == "Multi-Lingual"){ // DOOR KNOCK LETTERS
                $headerName = "DOOR KNOCK LETTERS";
            }
            ...
            $techDocHeader = $this->resources_model->get_tech_doc_header([
                ...
                ",
                'country_id' => $this->config->item('country'),
                'custom_where' => "tdh.`name` LIKE '%".$headerName."%'",
                //'header_id' => $headerId,
            ])->row_array();

            if (!is_null($techDocHeader)) {
                $headerId = $techDocHeader["tech_doc_header_id"];
                if ($headerId == 9) {
    ...
    */
    ...
    function add_expense()
    {
        ...
        if ($success) {
            ...
            $result = $this->db->set($insertData)->insert("expenses");
            ...
            if ($expenseId) {
                ...
                $exp_summary_data = array(
                    "date"          => date('Y-m-d'),
                    "employee"      => $staffId,
                    "who"           => $staffId,
                    "total_amount"  => $postData["amount"] ?? 0,
                    "line_manager"  => $postData["line_manager"] ?? 0,
                    "country_id"    => $countryId ?? 0,
                );

                $exp_summary_id = $this->expensesummary_model->add_expense_summary($exp_summary_data);

                if ($exp_summary_id) {
                    $this->expensesummary_model->update_expense($exp_summary_id, $expenseId);
                }
            }
        ...
        if (!empty($expenseIds)) {
            ...
            if ($expenseSummaryId !== 0) {
                ...
                try {
                    ...
                    $to = array(make_email('accounts'), $lm_email);
    ...
    */
    ...
    public function get_leave_type()
    {
        ...
        $result = $this->system_model->get_leave_types();

        $this->api->setSuccess(true);
        $this->api->putData("leave_type", $result->result_array());
    }
    ...
    function save_leave_request()
    {
        ...
        if (!empty($postData["staff_id"])) {
            $employee = $postData["staff_id"];
            unset($postData["staff_id"]);
        }

        if (
            !empty($postData["lday_of_work"]) &&
            !empty($postData["fday_back"])
        ) {
            $date_diff = date_diff(date_create($postData["lday_of_work"]), date_create($postData["fday_back"]));
            $postData["num_of_days"] = $date_diff->format("%a") + 1;
        }
        ...
        $type_of_leave = $postData["type_of_leave"];
        $backup_leave = $postData["backup_leave"];
        ...
        if ($postData["full_day"] == "1") {
            $postData["start_time"] = "00:00:00";
            $postData["end_time"] = "00:00:00";
        } elseif (
            !empty($postData["start_time"]) &&
            !empty($postData["end_time"])
        ) {
            $start_time_obj = new DateTime($postData["start_time"]);
            $end_time_obj = new DateTime($postData["end_time"]);
            $interval = $start_time_obj->diff($end_time_obj);
            $postData["total_time"] = $interval->format('%H');
        }
        ...
        if ($this->form_validation->run() == true) {
            ...
            $email_data['type_of_leave'] = $this->users_model->getLeaveType($type_of_leave);
    ...
    private function getLeavePdf($output, $leave_id)
    {
        ...
        $pdf->Cell($cell_width, $cell_height, $this->users_model->getLeaveType($leave['type_of_leave']), $cell_border, $col2_cell_new_line, $col2_cell_align);
