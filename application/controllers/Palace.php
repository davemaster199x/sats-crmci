...
class Palace extends MY_Controller {
            ...
            if( $duplicate_query->num_rows()>0 ){ // existing property found
                ...
                if( $disable_add != 1 ){
                    ...
                    */
                    ...
                    $this->db->insert('property', $prop_data);

                    if ($this->db->affected_rows() > 0) {
                        //Poperty was successfully inserted to DB

                        // Property insert last id
                        $prop_insert_id = $this->db->insert_id();

                        // Now insert api relevant datas to api_property_data table----------------------------
                        ...
                        $this->properties_model->add_data_property($separated_data);

                        // Get Palace Tenants and insert to CRM tenants-------------------------------------------
                        $palace_params = array(
                            'agency_id' => $agency_id,
                        ...
                        if(!empty($tenant_arr)){
                            // Tenants array is not empty > proceed to insert api tenants into crm tenants
                        ...
                        log_message('debug', "Palace.bulk_connect_add_property Insert error: No property data inserted");
                        return false;
                    }
                    ...
                    //if( $add_property && !empty($prop_insert_id) ){
                        ...
                        // insert property log > disabled and moved above
                        // $params = array(
                        //     'title' => 2, //New Property Added
                        //     'details' => 'Added from Palace Bulk Match',
                        //     'display_in_vpd' => 1,
