...
class Home extends MY_Controller {
    ...
    public function __construct(){
        parent::__construct();
        // If there is no user logged in yet, redirect
    ...
    public function assign_user_class_block(){
        ...
        $permission_needed = 7; // Can update user class block on homepage settings
        if( $this->system_model->can_edit_account($permission_needed) == true ){
    ...
    public function ajax_get_goal_count_data(){
        ...
        $data['result'] = $this->db->query("SELECT `id`, `name`,`total_goal`,`staff_id` FROM `main_page_total` WHERE `name` NOT IN($exclude)");
        $data['active_user'] = $this->db->query("SELECT `StaffID`,`FirstName`,`LastName` FROM `staff_accounts` WHERE active = 1 ORDER BY FirstName");
    ...
    /* disabled
    public function update_main_page_total(){
    ...
    public function ajax_save_goal_count_data_v2(){

        $data['status'] = false;

        $goal_arr = $this->input->post('goal_arr');

        $cnt = 0;
        foreach( $goal_arr as $goal_row ){

            $goal_row_dec = json_decode($goal_row);

            $goal_id = $goal_row_dec->goal_id;
            $total_goal = $goal_row_dec->total_goal;
            $staff = $goal_row_dec->staff;

            $main_page_total_row = $this->db->select('*')->from('main_page_total')->where('id', $goal_id)->get()->row_array();

            if( $goal_id!="" && is_numeric($goal_id) ){

                if( $main_page_total_row['total_goal']!=$total_goal || $main_page_total_row['staff_id']!=$staff ){ //update field with changes only
