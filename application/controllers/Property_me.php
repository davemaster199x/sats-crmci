...
class Property_Me extends MY_Controller {
            ...
            <tbody>
                ...
                ?>
                    <tr>	
                        ...
                        <td class="crmAdd" data-crm_prop_link="/properties/details/?id=<?=$row->property_id;?>"><?=$prop_address?></td>
...
//            INNER JOIN `agency` AS `a` ON
//                  (
//                      CASE WHEN pnv.`property_source`= 1
//                              THEN p.`agency_id` = a.`agency_id`
//                           WHEN ( pnv.`property_source`= 2 OR pnv.`property_source`= 3 OR pnv.`property_source`= 7)
//                              THEN pnv.`agency_id` = a.`agency_id`
//                      END
//                    )
...
//        );
        ...
        $custom_where = "( p.`is_nlm` = 0 OR p.`is_nlm` IS NULL ) AND (  apd.`api_prop_id` = '' OR apd.`api_prop_id` IS NULL ) AND p.`is_sales` = 0 {$property_id_not_in}";
        ...
        if($export==1){
            ...
            foreach($sql->result() as $row) {
                ...
                if(  $row->property_source == 1 ){ // crm
                    ...
                    $p_address = "
                    <a href='/properties/details/?id={$row->property_id}' target='_blank'>
                        {$crm_full_address}
                    </a>";
    ...
    public function send_all_certificates_and_invoices_via_vjd() {
        ...
        $url = $this->config->item("crmci_link") . '/jobs/details/' . $job_id;
    ...
    public function add_billing_on_upload_invoice($agencyId, $pmePropId, $jobId, $countryId, $job_details, $property_details, $alarm_details, $num_alarms) {
        ...
        if (!is_null($suppId->pme_supplier_id) && floatval($getTotalAmount) > 0) {
            ...
            $log_details = "<a href='".$protocol."://{$baseUrl}/pdf/combined/{$encrypt}'>Combined Invoice/Cert</a>, #{$invoice_number} uploaded to PMe Agency as a bill of {$getTotalAmount}";
            $log_params = array(
    ...
    */ 
    ...
    public function updated_tenants() {
        ...
        // main query body
        $main_query = "
        FROM `api_last_tenant_update` AS altu
        INNER JOIN `api_property_data` AS apd_pme ON altu.`api_property_data_id` = apd_pme.`id`
        LEFT JOIN `property` AS p ON ( 
            apd_pme.`crm_prop_id` = p.`property_id` AND 
            apd_pme.`api` = 1 AND 
            apd_pme.`active` = 1 AND 
            (
