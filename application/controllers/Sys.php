    ...
    public function search_results() {
        ...
        if ($search_type == 1) { //  Job ID
            ...
            $params = array(
                'sel_query' => $sel_query,
                'job_id' => $job_id,
                //'is_nlm' => 2, // 2 means not include NLM filter in a query /** CRM-1396 : is_nlm was removed was removed since we need to be able to search inactive properties **/
                ...
                //'p_deleted' => 0, /** CRM-1396 : p_deleted was removed since we need to be able to search inactive properties **/
                'display_query' => 0
            );
            ...
            $params = array(
                ...
                'is_nlm' => 2, // 2 means not include NLM  filterin a query
                'country_id' => $country_id,
                'custom_joins' => [
                    "join_table" => "jobs j",
            ...
            $params = array(
                'sel_query' => $sel_query,
                'custom_where' => $custom_where,
                'is_nlm' => 2, // 2 means not include NLM filter in a query
                'sort_list' => array(
                    array(
                        'order_by' => 'p.`address_2`',
                        'sort' => 'ASC',
            ...
            //exit();
        } else if ($search_type == 8) { // Tenant Email

            $tenant_email = str_replace(' ', '', trim($search_val));
            ...
            $sel_query = "
                    ...
                    p.`is_sales`
				";
            $custom_where = "pt.`tenant_email` LIKE '%{$tenant_email}%'";
            $params = array(
                'sel_query' => $sel_query,
                'custom_where' => $custom_where,
                'display_query' => 0
