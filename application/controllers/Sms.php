<?php
	...
	/** @Property Sms_api_sent_model $sms_api_sent_model */
	/** @Property Sms_api_replies_model $sms_api_replies_model */
...
class Sms extends MY_Controller {
    ...
    public function templates() {
        ...
        if( $this->config->item('country') == 1 ){ // AU
            ...
            */
            ...
            $allowed_staff = array(2070, 2025, 2287, 2175, 2226);
            ...
            */
            ...
            $allowed_staff = array(2070, 2025, 2231, 2193, 2214, 2124);
        ...
        if( $job_id > 0 ){
            ...
            if ($template == 1) {
                ...
                if ($job_row->agency_id == 1200) { // private agency
                    $template_body = "Please call ".$this->config->item('company_name_short')." {tenant_number} to make an appointment to service your smoke alarms";
                } else if ( $job_row->job_type == "240v Rebook" || $job_row->is_eo == 1 ) {  // jobs with job type 240 rebook
                    $template_body = "{serv_name} need to be replaced on behalf of your landlord and {agency_name}. Please call ".$this->config->item('company_name_short')." {tenant_number}";
                    ...
                    $template_body = $this->config->item('company_name_short')." need to test {serv_name} on behalf of your landlord. Please call {tenant_number}";
                }
            }
    ...
    public function parse_tags() {
        ...
        if( $job_id > 0 ){
            ...
            /**
             * Add catch here when tenant_id is/not empty
             */
            $tenant_name = "";

            if(!empty($tenant_id)){
               /**
                * Tenant_id is valid and not empty
                * Put and run tenant query here
                */
                ...
                if(!empty($tenants_row)){
                    $tenant_name = $tenants_row->tenant_firstname;
                }
            }
	...
	/**
     * SMS must have message_ids for this page to display anything.
     * Ensure your 2 endpoints are configured in yabbr correctly.
	 * @return void
	 */
	public function view_incoming_sms() {
        ...
        $page_url = '/sms/view_incoming_sms';
	...
	/**
     * After a message has been sent, yabbr sends to this endpoint so that we can store the status of the sent message.
     * This setting is found via api keys > edit key
	 * @return void
	 */
	...
	/**
	 * This is where yabbr will forward any SMS on to us with a JSON request
	 * This setting is found via api keys > edit key
	 * @return void
	 */
    public function yabbr_catch_reply(){
        ...
        $json_dec = json_decode($json_data);
        $event_obj = $json_dec->messages[0];
        $sms_ref_id = $this->input->get_post('sms_ref_id');
    ...
    public function workorders_and_unlinked_sms(){
        
        $data['title'] = "Workorders and Unlinked SMS";
        $per_page = $this->config->item('pagi_per_page');
        $offset = ( null != $this->input->get_post('offset') ) ? $this->input->get_post('offset') : '0';
        ...
        if( $sms_ref_id > 0 ){
            ...
            if( $sms_ref_id == 1 ){ // Sales Upgrade Requests
                $mpt_name = 'sales-sms'; 	
            }else if( $sms_ref_id == 2 ){ // SATS Tenant Line
        ...
        $pagi_link_params = '/sms/workorders_and_unlinked_sms/?'.http_build_query($pagi_links_params_arr);
	...
	/**
	 * Callback for wholesale sms outgoing sms to return to us delivery status
	 * https://support.wholesalesms.com.au/hc/en-us/articles/4412067980431-Messaging-API
	 * @return false|void
	 * @throws \Pusher\PusherException
	 */
    public function wholesalesms_delivery()
    {
        $this->load->model('sms_api_sent_model');
        ...
        $this->sms_api_sent_model->update($data, 'message_id');

    }

	/**
     * Callback for wholesale sms replies to our outgoing sms
     * https://support.wholesalesms.com.au/hc/en-us/articles/4412067980431-Messaging-API
	 * @return false|void
	 * @throws \Pusher\PusherException
	 */
	public function wholesalesms_reply()
	{
//
//        if(empty($this->input->get_post('message_id'))){
//	        $this->output->set_status_header(400);
//        }

		$this->load->model('sms_api_replies_model');

        // Init some constants
		$ev = "ev01";
        $notf_type_ids = [
            'SMS notification' => 2,
            'SMS No Show notification' => 3,
        ];

        // INSERT REPLY INTO SMS REPLIES TABLE

        // catch the GET data returned by SMS API
		$message_id = $this->input->get_post('message_id');
		$mobile = $this->input->get_post('mobile');
		$datetime_entry = $this->input->get_post('datetime_entry');
		$response = $this->input->get_post('response');
		$longcode = $this->input->get_post('longcode');
		...
		$this->sms_api_replies_model->insert($data);

        // NEXT FIND WHO SENT THE ORIGINAL AND NOTIFY THEM OF THE REPLY
        $query = $this->db->query("
                SELECT 
                    original.`sent_by`, 
                    a.`country_id`, 
                    original.`sms_type` 
                FROM `sms_api_sent` AS original 
                LEFT JOIN `jobs` AS j ON original.`job_id` = j.`id`
                LEFT JOIN `property` AS p ON j.`property_id` = p.`property_id`
                LEFT JOIN `agency` AS a ON p.`agency_id` = a.`agency_id`
                WHERE original.`message_id` = ?", $message_id);

		$original_sms = $query->row();

        if(empty($original_sms)){
	        log_message('error', 'wholesalesms_reply - No original SMS found: ' . json_encode($data, JSON_PRETTY_PRINT));
        }
        ...
        $notification_data = [
	        'notf_type'            => $notf_type_ids['SMS notification'],
	        'notification_message' => 'New <a href="/sms/view_incoming_sms" target="_blank">SMS</a> from ' . $mob_num,
	        'notify_to'            => $original_sms->sent_by,
		...
		$pusher_data['notif_type'] = $notf_type_ids['SMS notification'];
		$ch = "ch" . $original_sms->sent_by;
		$out = $pusher->trigger($ch, $ev, $pusher_data);

        // If No Show
        if ($original_sms->sms_type == 4) {
            // customer service people
	        $staff_ids = [2147, 2124];

            foreach ($staff_ids as $staff_id) {
                $notification_data = [
	                'notf_type'            => $notf_type_ids['SMS No Show notification'],
	                'notification_message' => 'New <a href="sms/view_incoming_sms">SMS</a> from ' . $mob_num,
	                'notify_to'            => $staff_id,
	                'country_id'           => config_item('country'),
                ];
	            $this->db->insert('notifications', $notification_data);
                ...
                // pusher notification
                $ch = "ch" . $staff_id;
	            $data['notif_type'] = $notf_type_ids['SMS No Show notification'];
