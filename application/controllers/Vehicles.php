...
class Vehicles extends MY_Controller {
    ...
    public function ajax_update_vehicles() {
        ...
        if ($addRes || $updateRes) {
            ...
            // USER LOG - ADD LOG FOR EACH AFFECTED DRIVER
            if($og_staff_id != $staff_id){
                // init array
                $saveData = [
                    'date' => date("Y-m-d H:i:s"),
                    'details' => "",
                ...
                // If it changed, and there was a driver, log the removal
                if(!empty($og_staff_id)){
                    $saveData['details'] = "Driver removed from Vehicle ID: " . $vehicles_id;
                    $saveData['staff_id'] = $og_staff_id;
                    $this->db->insert("user_log", $saveData);
                }

                // If it changed, add there is now a driver, log the added
                if(!empty($staff_id)){
                    // no original driver - 1 log
                    $saveData['details'] = "Driver added to Vehicle ID: " . $vehicles_id;
                    $saveData['staff_id'] = $staff_id;
                    $this->db->insert("user_log", $saveData);
                }
            }
