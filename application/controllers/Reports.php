...
class Reports extends MY_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('smoke_alarms_company_model', 'sac');

        $this->country_id = $this->config->item('country');
    }
    ...
    public function report_admin() {
        ...
        if ($staff_id === 0) {
            $staff_details['FirstName'] = $this->config->item('company_name_short')." System";
    ...
    public function sales_report() {
        ...
        # Get Staff details for display if needed
        if ($data['staff_id'] === 0) {
            $dat['staff_details']['FirstName'] = $this->config->item('company_name_short')." System";
        ...
        */
        ...
        $params_get_sales_report = array(
            ...
            'custom_where' => 'sr.contact_type > 0'
        );
        $data['get_sales_report_sql'] = $this->reports_model->get_sales_report($params_get_sales_report)->result_array();*/

        $data['get_sales_report_sql'] = $this->db->query("
        SELECT `main_log_type_id`, `contact_type`
        FROM `main_log_type` 
        WHERE `active` = 1
        ")->result_array();
        //echo $this->db->last_query();
        ...
        $sales_class = 5;        
        $sales_staff_custom_where = "sr.staff_id > 0  AND sa.`Deleted` = 0 AND sa.`active` = 1";
        if( $data['date_filter_str'] != '' ){
            $sales_staff_custom_where .= " AND {$data['date_filter_str']}";
        }
        $params_get_sales_report_staff = array(
            ...
            'custom_where' => $sales_staff_custom_where
        );
    ...
    public function add_opportunity() {
        ...
        if ($this->form_validation->run() != FALSE) {
            ...
            if ($insert_agency_log == 1) {
                ...
                if(!empty($properties)){
                    $log_details .= " <strong>Properties:</strong> {$properties}.";
                }

                if(!empty($status)){
                    $snaphot_row = $this->db->select('name')->from('sales_snapshot_status')->where('sales_snapshot_status_id', $status)->get()->row_array();
                    $log_details .= " <strong>Status:</strong> {$snaphot_row['name']}.";
                }

                if(!empty($details)){
                    $log_details .= " <strong>Details:</strong> {$details}.";
                }
    ...
    public function ajax_delete_snapshot_sales_rep() {
        ...
        if ($ss_sr_id && $ss_sr_id != "" && is_numeric($ss_sr_id)) {
            ...
            $json_data['status'] = false;
            $json_data['msg'] = "Server Error: Please Contact ".$this->config->item('company_name_short')." Admin";
        }
    ...
    public function new_properties_report() {
        ...
        if ($this->input->get_post('btnGetStats') || $this->input->get_post('get_sats') == 1) {
            ...
            if($this->input->get_post('export')==1){
                ...
                $head_text_last_part = array("Total New","Deleted","Net","By ".$this->config->item('company_name_short'),"By Agency");
    ...
    public function discarded_alarms() {
        ...
        $sel_query = "
            ...
            sa.FirstName,
            sa.LastName,
            `reg`.`region_name`, `reg`.`region_state`
		";
        $main_params = array(
            'sel_query' => $sel_query,
            'filterDate' => array(
        ...
        if ($this->input->get_post('export') == 1) {
            ...
            //header
            $header = array("Job Type", "Job ID", "Job Date", "Technicians", "Make", "Model", "Type", "Power", "Expiry", "Reason", "RFC", "Region", "State");
            fputcsv($file, $header);
            ...
            foreach ($lists->result_array() as $exportrow) {
                ...
                $exportdata['region_name'] = $exportrow['region_name'];
            ...
            // echo "<pre/>";
            // var_dump($data['query_string']); exit;
...
//		echo json_encode( $json_data );
    ...
    public function add_purchase_order() {
        ...
        $data['tech_list'] = $this->gherxlib->getStaffInfo($tech_params);
    ...
    public function ajax_daily_figures_fetch_data() {
        ...
        $subs_revenue = $this->figure_model->jGetSales($params);

        // Non-Subs Revenue
        $non_subs_revenue = ($sales-$subs_revenue);
        ...
        $arr = array(
            ...
            "non_subs_revenue" => $non_subs_revenue,
            "techs" => $techs,
            "jobs" => $jobs,
    ...
    public function ajax_update_daily_figures_per_date() {
        ...
        $subs_revenue = $this->input->get_post('subs_revenue');
    ...
    public function ajax_daily_figures_statistics() {
        ...
        $data['subs_revenue'] = $dfpd['subs_revenue'];
        $data['non_subs_revenue'] = ($dfpd['sales']-$dfpd['subs_revenue']);
...
//$vip = array(11); // ness
        ...
        if ($help_topic != '') {
            $custom_where_arr[] = "ct.`sub_category` = {$help_topic}";
        }

        if ($category != '') {
            $custom_where_arr[] = "ct.`category` = {$category}";
        }
        ...
        ctht.`sub_category_name` AS ctht_help_topic,
        ...
        //get main ticket category list
        $data['ticket_category_q'] = $this->db->query("
        SELECT *
        FROM `crm_task_category` 
        WHERE `active` = 1
        ORDER BY `category_name` ASC
        ");
        //get main ticket category list end
...
//$vip = array(11); // ness
    ...
    public function ticket_response(){
        ...
        if( $crm_task_id > 0 ){
            ...
            LEFT JOIN `crm_task_sub_category` AS ctht ON ct.`sub_category` = ctht.`id`
            LEFT JOIN `crm_task_category` AS ctc ON ct.`category` = ctc.`id`
            ...
            $new_help_topic = $crm_task_help_topic_row->sub_category_name;        
            if( $crm_task_row->sub_category != $help_topic ){
      
                $crm_task_log_text = "Sub category updated from <b>{$crm_task_row->ctht_help_topic}</b> to <b>{$new_help_topic}</b>";
                $this->insert_crm_task_logs($crm_task_id,$crm_task_log_text);
            ...
            //category
            $new_category = null;
            $crm_category_sql = $this->db->query("
            SELECT `category_name`
            FROM `crm_task_category` 
            WHERE `active` = 1
            AND `id` = {$category}        
            ");
            $crm_category_row = $crm_category_sql->row();
            $new_category = $crm_category_row->category_name;
            
            if( $crm_task_row->category != $category ){
      
                $crm_task_log_text = "Category updated from <b>{$crm_task_row->category_name}</b> to <b>{$new_category}</b>";
                $this->insert_crm_task_logs($crm_task_id,$crm_task_log_text);
                ...
                <body>
                    ...
                    <table>
                        ...
                        <tr>
                            ...
                            <td style='text-align: left; padding: 5px;'> {$new_category}</td>
                        </tr>
                        <tr>
                            <th style='text-align: right; padding: 5px;'>Sub Category:</th>
                            <td style='text-align: left; padding: 5px;'> {$new_help_topic}</td>
                        </tr>
                        <tr>
    ...
    public function update_crm_task_action_form_submit() {
        ...
        $response = $this->input->get_post('response');
        $status = $this->input->get_post('status');
        $category = $this->input->get_post('ticket_category');
        ...
        ");
        $crm_task_help_topic_row = $crm_task_help_topic_sql->row();
        $help_topic_txt = $crm_task_help_topic_row->sub_category_name;

        //get category name
        $category_sql = $this->db->query("
        SELECT `category_name`
        FROM `crm_task_category` 
        WHERE `id` = {$category}
        ");
        $crm_task_category_row = $category_sql->row();
        $category_txt = $crm_task_category_row->category_name;
                ...
                <html>
                    ...
                    <body>
                        ...
                        <table>
                            <tr>
                                <th style='text-align: right; padding: 5px;'>Category:</th>
                                <td style='text-align: left; padding: 5px;'> {$category_txt}</td>
                            </tr>
                            <tr>
                                <th style='text-align: right; padding: 5px;'>Sub Category:</th>
    ...
    public function update_exp_summary_action_ajax()
    {
        ...
        if ($action === "update_exp_sum_line_manager") {
            ...
            $params = [
                ...
                'email_approval_action'  => 'users'
            ];

            /**
             * This function will run the expense send emails for users
             */
            $this->run_expense_summary_emails($params);

            // This function will update the expense status
...
//        echo "</pre>";
    ...
    function view_add_expense() {
        ...
        $data['last_query'] = $this->db->last_query();
        $staff_accounts = $this->expensesummary_model->getStaffAccountsByCountryId($this->country_id)->result_array();
        $loggedin_staff_id = $this->session->staff_id;
        $jparams = array(
            'sort_list' => array([
                    'order_by' => '`account_name`',
    ...
    public function add_expense_action_form_submit()
    {
        ...
        if (!is_dir($config['upload_path'])) {
            // Create the directory with read/write permissions for owner
            mkdir($config['upload_path'], 0700, true);

            // Make sure the directory is writable
            chmod($config['upload_path'], 0700);
        }
    ...
    public function add_expense_summary_action_form_submit()
    {
        ...
        try {
            ...
            /**
             * Send Expense Emails for Line Manager
             * @function run_expense_summary_emails
             */
            ...
            $params = array(
                ...
                'email_approval_action' => "line_manager",
                'sort_list' => array(
                    array(
                        'order_by' => $order_by,
                        'sort' => $sort
            ...
            /**
             * Run expense summary emails function
             */
            $this->run_expense_summary_emails($params);
    ...
    public function gained_lost_serverside() {
        ...
        if($properties > 0) {
            foreach ($properties as $property) {
                $link = '/properties/details/?id=' . $property->property_id ;
    ...
    public function mark_as_not_payable() {
        ...
        if ($export == 1) { //EXPORT         
            ...
            foreach ( $main_sql->result() as $row ){ 
                ...
                AND pel.`event_details` = 'Smoke Alarms Changed from ' . config_item('company_name_short') . ' to DIY'                      
                AND DATE(pel.`log_date`) BETWEEN '2022-03-01' AND '2022-03-31' 	
                ";
    ...
    */
    ...
    public function api_unlinked_properties(){
        ...
        */
        ...
        $sel_query = "`p`.`property_id`, `p`.`address_1` AS `p_address_1`, `p`.`address_2` AS `p_address_2`, `p`.`address_3` AS `p_address_3`, `p`.`state` AS `p_state`, `p`.`postcode` AS `p_postcode`, `p`.`holiday_rental`, `a`.`agency_id`, `a`.`agency_name`, apt.agency_api_token_id, apd.api_prop_id, apd.api, aapi.api_name, apt.api_id, pccta.`comment` AS pccta_reason";
        $params = array(
            'sel_query' => $sel_query,
            'custom_where' => $custom_where,
            'limit' => $per_page,
    ...
    public function compliance_report_qld_old() {
        ...
        if ($export == 1) { //EXPORT         
            ...
            // var_dump($data['lists']->result_array());exit;
            //count compliance 
            $compliant_properties_count = 0;
            $non_compliant_properties_count = 0;
            foreach ($data['lists']->result_array() as $row) {
                if($row['jdate'] <= $row['due_date']){
                    $compliant_properties_count++;
                }else{
                    $non_compliant_properties_count++;
                }
            }
            $data['compliant_total'] = $compliant_properties_count;
            $data['non_compliant_total'] = $non_compliant_properties_count;
    ...
    public function compliance_report_qld()
    {
        ...
        $params = [
            'startDate' => $this->input->get_post('start_date') ? $this->system_model->formatDate($this->input->get_post('start_date')) : null,
            'endDate'   => $this->input->get_post('end_date') ? $this->system_model->formatDate($this->input->get_post('end_date')) : null,
            'agency'    => $this->input->get_post('agency_filter'),
        ];
        
        $data['distinct_agency'] = $this->agency_model->distinct_agency_filter();
        $data['data'] = $this->reports_model->getComplianceReportData($params);
        ...
        $percentage = ($compliant_properties_count / count($data['data'])) * 100;
        ...
        //load views
        $this->load->view('templates/inner_header', $data);
        $this->load->view('v2/reports/compliance_report_qld', $data);
        $this->load->view('templates/inner_footer', $data);
    }
    ...
    public function properties_from_source_of_client() 
    {


        $data['title'] = "Properties From Source of Client";
        $uri = '/reports/properties_from_source_of_client';
        $data['uri'] = $uri;
        ...
        $sac_str_inner = "";
        if ($source_of_client > 0) {
            $sac_str_inner = "AND pfoc.company_id = {$source_of_client}";
        }
        ...
        $sel_query = "
        SELECT    
            pfoc.`added_date` AS pfoc_added_date,
        ...
        LEFT JOIN `smoke_alarms_company` as sac ON sac.`sac_id` = pfoc.company_id
        WHERE pfoc.`active` = 1
        AND afoc.`active` = 1
        AND ps.`service` = 1
        {$sac_str_inner}
        ...
        if( $export == 1 ) { // export         
            ...
            $header = array('Property address','Agency','Property status','Service Price','Date of creation','Date of deactivation (If Applicable)','From List');
            ...
            foreach ( $main_list->result() as $row ){ 
                ...
                $from_list_query = $this->db->query("
                SELECT *
                    FROM `logs`
                WHERE `details` = 'This property was added from a property list'
                AND `property_id` = {$row->property_id}                
                ");
                $from_list = $from_list_query->num_rows();
                if ($from_list > 0) {
                    $csv_row[] = 'Yes';
                } else {
                    $csv_row[] = 'No';
            ...
            $sac_property_list = $data['lists'];
            $sac_property_arr = array();
            
            foreach($sac_property_list->result() as $sac_row) {
                
                $price_var_params = array(
                    'service_type' => $sac_row->service_type,
                    'property_id' => $sac_row->property_id
                );
                
                $price_var_arr = $this->system_model->get_property_price_variation($price_var_params);
                $service_price = $price_var_arr['price_breakdown_text'];

                $service_price_num = substr($service_price, 1);
                
                $sac_row->service_price = $service_price;
                $sac_row->service_price_inc_gst = number_format($service_price_num,2);
                $sac_row->service_price_ex_gst = number_format($service_price_num/1.1,2);

                $sac_property_arr[] = $sac_row;
            }
            $data['sac_property_arr'] = $sac_property_arr;

            // total rows            
            $total_rows_sql = $this->db->query("
            SELECT COUNT(ps_inner_query.`property_id`) as pcount
            {$main_query}
            {$agency_filter_sql_str}
            {$ps_filter_str}
            GROUP BY ps_inner_query.`property_id`
            ORDER BY ps_inner_query.`property_id` DESC
            ");
            $total_rows = count($total_rows_sql->result());
            ...
            $data['agency_filter_sql'] = $this->db->query("
                ...
                ORDER BY a.`agency_name` ASC           
            ");
            $data['sac_list'] = $this->sac->get_smoke_alarms_company_list();
            ...
            $pagi_links_params_arr = array(            
                ...
                'source_of_client' => $source_of_client
            );
            $pagi_link_params = "{$uri}/?".http_build_query($pagi_links_params_arr);
            $data['pagi_links_params_arr'] = $pagi_links_params_arr;
    ...
    public function agencies_from_source_of_client() 
    {


        $data['title'] = "Agencies From Source of Client";
        $uri = '/reports/agencies_from_source_of_client';
        $data['uri'] = $uri;
        ...
        $afoc_filter = null;
        if ($source_of_client > 0) {
            $afoc_filter = " AND afoc.company_id = {$source_of_client} ";
        }
        ...
        LEFT JOIN `alarm_job_type` AS ajt ON ps.`alarm_job_type_id` = ajt.`id`
        INNER JOIN `property` AS p ON ps.`property_id` = p.`property_id`  
        LEFT JOIN `properties_from_other_company` AS pfoc ON ( p.`property_id` = pfoc.`property_id` AND pfoc.`active` = 1 )  
        LEFT JOIN `smoke_alarms_company` as sac ON pfoc.company_id = sac.`sac_id`     
        LEFT JOIN `agency` AS a ON p.`agency_id` = a.`agency_id`
        INNER JOIN `agencies_from_other_company` AS afoc ON ( a.`agency_id` = afoc.`agency_id` AND afoc.`active` = 1 )        
        WHERE ps.`service` = 1
        AND afoc.`afoc_id` IS NOT NULL
        AND p.`deleted` = 0
        AND ( p.`is_nlm` = 0 OR p.`is_nlm` IS NULL )
        ";
        ...
        if( $export == 1 ) { // export 
            ...
            // file name
            $date_export = date('YmdHis');
            $filename = "agencies_from_source_of_client{$date_export}.csv";
            ...
            $header = array('Property address','Service Type','Service  Price (INC GST)','Service Price (EX GST)','Agency','Source','Property status');
            ...
            foreach ( $main_list->result() as $row ){ 
                ...
                $csv_row[] = $row->service_type_name;
                 ...
                 // get property price from variation
                 $price_var_params = array(
                    'service_type' => $row->service_type,
                    'property_id' => $row->property_id
                );
                ...
                $csv_row[] = number_format($price_var_arr['dynamic_price_total'],2);
                $csv_row[] = number_format($price_var_arr['dynamic_price_total']/1.1,2);
                
                $csv_row[] = $row->agency_name; 
                $csv_row[] = $row->company_name;
                $csv_row[] = ( $row->ps_service == 1 )?'Active':'Not Active';
            ...
            foreach($sac_property_list->result() as $sac_row) {
                ...
                $sac_row->service_price_inc_gst = number_format($price_var_arr['dynamic_price_total'],2);
                $sac_row->service_price_ex_gst = number_format($price_var_arr['dynamic_price_total']/1.1,2);
            ...
            // total rows            
            $total_rows_sql = $this->db->query("
            SELECT COUNT(ps.`property_id`) as pcount
            {$main_query}
            {$agency_filter_sql_str}
            {$afoc_filter}
            ");
            $total_rows = $total_rows_sql->row()->pcount;
            ...
            // Source of client filter
            $data['sac_list'] = $this->db->query("
            SELECT DISTINCT(sac.`sac_id`), sac.company_name
            {$main_query} 
            {$agency_filter_sql_str}
            AND sac.`sac_id` > 0
            ORDER BY sac.company_name ASC              
            ");
    ...
    public function tracking_tenant_lease_periods() {


        $title = "Tracking Tenant Lease Periods";
        $data['title'] = $title;
        $uri = '/reports/tracking_tenant_lease_periods';
        $data['uri'] = $uri;

        $api_id = 1; // PMe
        
        $agency_filter = $this->input->get_post('agency_filter');
        $date_search = $this->input->get_post('date_search');   
        $month_filter = $this->input->get_post('month_filter');        
        $year_filter = $this->input->get_post('year_filter');
        ...
        $date_sql_str = null;
        if( $date_search > 0 && $month_filter > 0 && $year_filter > 0 ){

            $month = str_pad($month_filter, 2, '0', STR_PAD_LEFT); // add zero to single digit

            switch( $date_search ){

                case 1:
                    $date_field_filter = 'atd.`start`';
                break;
    
                case 2:
                    $date_field_filter = 'atd.`end`';
                break;
    
                case 3:
                    $date_field_filter = 'atd.`agreement_start`';
                break;
    
                case 4:
                    $date_field_filter = 'atd.`agreement_end`';
                break;            
    
            }   
            
            // get last day of month
            $last_day_of_month = date('t',strtotime("{$year_filter}-{$month}-01"));

            // construct date filter
            $date_sql_str = "AND {$date_field_filter} BETWEEN '{$year_filter}-{$month}-01' AND '{$year_filter}-{$month}-{$last_day_of_month}'";
        ...
        SELECT    
            ...
            DATE_FORMAT(atd.`start`,'%d/%m/%Y') AS atd_start_dmy,
            DATE_FORMAT(atd.`end`,'%d/%m/%Y') AS atd_end_dmy,
            DATE_FORMAT(atd.`agreement_start`,'%d/%m/%Y') AS atd_agree_start_dmy,
            DATE_FORMAT(atd.`agreement_end`,'%d/%m/%Y') AS atd_agree_end_dmy,
        ...
        INNER JOIN `api_tenancy_data` AS atd ON apd_pme.`id` = atd.`api_property_data_id`
        LEFT JOIN `agency` AS a ON p.`agency_id` = a.`agency_id`
        WHERE  p.`deleted` = 0
        AND ( p.`is_nlm` = 0 OR p.`is_nlm` IS NULL )
        AND a.`status` = 'active'
        ...
        if( $export == 1 ) { // export         
            ...
            $header = array('Property address','Agency','Tenancy Start','Tenancy End','Agreement Start','Agreement End');
            ...
            foreach ( $main_list->result() as $row ){ 
                ...
                $csv_row[] = "{$row->p_address_1} {$row->p_address_2}, {$row->p_address_3}"; // property address
                $csv_row[] = $row->agency_name; // agency                
                $csv_row[] = $row->atd_start_dmy; // Tenancy Start
                $csv_row[] = $row->atd_end_dmy; // Tenancy End
                $csv_row[] = $row->atd_agree_start_dmy; // Agreement Start
                $csv_row[] = $row->atd_agree_end_dmy; // Agreement End
    ...
    public function property_missed_renewal() {


        $title = "Property Missed Renewal";
        $data['title'] = $title;
        $uri = '/reports/property_missed_renewal';
        $data['uri'] = $uri;
        
        $agency_filter = $this->input->get_post('agency_filter');
        $month_filter = ( $this->input->get_post('month_filter') != '' )?$this->input->get_post('month_filter'):date('m');    
        $data['month_filter'] = $month_filter;    
        $year_filter = ( $this->input->get_post('year_filter') != '' )?$this->input->get_post('year_filter'):date('Y');  
        $data['year_filter'] = $year_filter;     
        $export = $this->input->get_post('export');
        $renewals_to_show = $this->input->get_post('renewals_to_show');
        $show_fixed_prop = $this->input->get_post('show_fixed_prop');
        $custom_interval = $this->input->get_post('custom_interval');
        $state_filter = $this->input->get_post('state_filter');
        ...
        // dates
        $base_date = date('Y-m-d',strtotime("{$year_filter}-{$month_filter}-01"));        

        // cron run timestamp
        $cron_renew_ts = date('Y-m-d',strtotime("{$base_date} -1 month"));
        $cron_renew_ts_plus_2_months = date('Y-m-d',strtotime("{$cron_renew_ts} +2 months"));

        // header filters                  
        // "renewal to show" filter
        $renewals_to_show_sq_str = null;
        if( $renewals_to_show == 1  ){
            $renewals_to_show_sq_str = " AND DATE(j.`created`) > '{$cron_renew_ts}' ";
        } 

        // hide "jobs created" to fix missed jobs by renewal cron
        // this filter gets applied by default since, show_fixed_prop = 0(toggled OFF) is default
        $show_fixed_prop_sql_str = null;
        if( $show_fixed_prop == 0  ){

            $show_fixed_prop_sql_str = " 
            AND p.`property_id` NOT IN(

                SELECT p_inner2.`property_id`
                FROM `jobs` AS j_inner2 
                LEFT JOIN `property` AS p_inner2 ON j_inner2.`property_id` = p_inner2.`property_id`
                LEFT JOIN `agency` AS a_inner2 ON p_inner2.`agency_id` = a_inner2.`agency_id`
                WHERE j_inner2.`job_type` = 'Yearly Maintenance'                
                AND j_inner2.`del_job` = 0
                AND p_inner2.`deleted` = 0
                AND ( p_inner2.`is_nlm` = 0 OR p_inner2.`is_nlm` IS NULL )
                AND a_inner2.`status` = 'active'  
                AND a_inner2.`deleted` = 0   
                AND 
                (
                    (
                        j_inner2.`status` NOT IN('Completed','Cancelled')
                    ) OR 
                    (
                        j_inner2.`status` = 'Completed' AND  
                        j_inner2.`date` BETWEEN '{$cron_renew_ts}' AND '{$cron_renew_ts_plus_2_months}'
                    )
        ...
        // state filter
        $state_filter_sql_str = null;
        if( $state_filter != ''  ){
            $state_filter_sql_str = " AND p.`state` = '{$state_filter}' ";
        } 

        // custom interval
        $custom_interval_sql_str = null;
        if( $custom_interval == 1  ){ // custom interval

            $custom_interval_sql_str = " 
            AND aop.`id` IS NOT NULL 
            AND aop.`renewal_interval` != '' 
            ";
            
            if( $agency_filter > 0  ){                

                // get agency preference
                $agency_interval_sql = $this->db->query("
                SELECT `renewal_interval`
                FROM `agency_other_pref`
                WHERE `agency_id` = {$agency_filter}
                ");
                $agency_interval_row = $agency_interval_sql->row();

                // get YM completed last year (based on custom interval)
                $last_ym_completed_start = date('Y-m-d',strtotime("{$base_date} -{$agency_interval_row->renewal_interval} months"));
                $last_ym_completed_end = date('Y-m-t',strtotime("{$base_date} -{$agency_interval_row->renewal_interval} months"));

                $custom_interval_sql_str .= " AND j.`date` BETWEEN '{$last_ym_completed_start}' AND '{$last_ym_completed_end}' ";

            } 

        }else{ // default - 12 months

            // get YM completed last year (default -12 months)
            $last_ym_completed_start = date('Y-m-d',strtotime("{$base_date} -12 months"));
            $last_ym_completed_end = date('Y-m-t',strtotime("{$base_date} -12 months"));
        ...
        AND a.`country_id` = {$country_id}        
        {$custom_interval_sql_str}
        {$renewals_to_show_sq_str}        
        AND p.`property_id` NOT IN(

            SELECT p_inner.`property_id`
            FROM `logs` AS l_inner
            LEFT JOIN `jobs` AS j_inner ON l_inner.`job_id` = j_inner.`id`
            LEFT JOIN `property` AS p_inner ON j_inner.`property_id` = p_inner.`property_id`
            WHERE l_inner.`title` = 51
            AND l_inner.`details` = 'Service Due Job Created'
            AND l_inner.`display_in_vjd` = 1
            AND l_inner.`auto_process` = 1
            AND DATE(l_inner.`created_date`) = '{$cron_renew_ts}'
    ...
    public function pmr_dynamic_query(){
        ...
        if( $custom_interval == 1  ){ // custom interval
            ...
            $custom_interval_sql_str = " AND aop.`renewal_interval` != '' ";
        ...
        // agency filter
        $agency_filter_sql = $this->db->query("
        SELECT DISTINCT(a.`agency_id`), a.`agency_name`
        {$main_query} 
        ORDER BY a.`agency_name` ASC           
        ");
    ...
    public function invoiced_but_not_yet_on_myob() {


        $title = "Invoiced but not yet on MYOB";
        $data['title'] = $title;
        $uri = '/reports/invoiced_but_not_yet_on_myob';
        $data['uri'] = $uri;
        
        $agency_filter = $this->input->get_post('agency_filter');
        $from_date = ( $this->input->get_post('from_date') != "" )?$this->system_model->formatDate($this->input->get_post('from_date')):null;
        $to_date = ( $this->input->get_post('to_date') != "" )?$this->system_model->formatDate($this->input->get_post('to_date')):null;
        $zero_jobs_included = $this->input->get_post('zero_jobs_included');
        ...
        // $0 priced jobs included
        $zero_jobs_included_sql_str = null;
        if( $zero_jobs_included == 0  ){
            $zero_jobs_included_sql_str = " AND DATE(j.`job_price`) > 0 ";
        }
        
        // date filter
        $date_filter_sql_str = null;
        if( $this->input->get_post('from_date') !='' && $this->input->get_post('to_date') != ''  ){
            $date_filter_sql_str = " AND j.`date` BETWEEN '{$from_date}' AND '{$to_date}' ";
        }
        ...
        LEFT JOIN `api_job_data` AS ajd ON ( j.`id` = ajd.`crm_job_id` AND ajd.`api` = {$console_api} )
        ...
        AND a.`country_id` = {$country_id}
        AND (
            j.`client_emailed` IS NULL OR
            j.`client_emailed` = 0
        ) AND  
        (
            ( ajd.`api_inv_uploaded` = 0 OR ajd.`api_inv_uploaded` IS NULL ) AND
            ( ajd.`api_cert_uploaded` = 0 OR ajd.`api_cert_uploaded` IS NULL )
        ) AND
        (
            ( j.`is_pme_invoice_upload` = 0 OR j.`is_pme_invoice_upload` IS NULL ) AND 
            ( j.`is_pme_bill_create` = 0 OR j.`is_pme_bill_create` IS NULL )
        ) AND
        (
            ( j.`is_palace_invoice_upload` = 0 OR j.`is_palace_invoice_upload` IS NULL ) AND 
            ( j.`is_palace_bill_create` = 0 OR j.`is_palace_bill_create` IS NULL )
        )
        {$zero_jobs_included_sql_str}
        {$date_filter_sql_str}
        ";        
        ...
        if( $export == 1 ) { // export         
            ...
            */
            ...
            // pagination
            $pagi_links_params_arr = array(            
                'zero_jobs_included' => $zero_jobs_included,
                'from_date' => $from_date,
                'to_date' => $to_date,
                'order_by' => $order_by,
                'sort' => $sort
    ...
    public function proccess_renewal(){
        ...
        if( count($job_id_arr) > 0 ){
            ...
            foreach( $job_sql->result() as $job_row ){
                ...
                if( $job_row->jid > 0 ){
                    ...
                    $this->cron_model->process_renewal($params_obj);

                }                
                
            }

            redirect("/reports/property_missed_renewal/?renew_success=1");
    ...
    public function jobs_not_exported_to_myob()
    {

        $title = "Jobs not exported to MYOB";
        $data['title'] = $title;
        $uri = '/reports/jobs_not_exported_to_myob';
        $data['uri'] = $uri;
        ...
        $main_query = "
            ...
            AND j.id NOT IN (
                ...
                LEFT JOIN `logs` as ll_inner ON ll_inner.job_id = j2.`id` 
                WHERE ll_inner.title = 27 
                AND 
                    (
                        ll_inner.details = 'Job status updated from <b>Merged Certificates</b> to <b>Completed</b>' 
                        OR ll_inner.details = 'Job status updated from <strong>Merged Certificates</strong> to <strong>Completed</strong>'
                    )
            )
        ";
        
        if (!empty($month_filter)) {
            $main_query .= "AND (MONTH(j.date) = {$month_filter} AND YEAR(j.date) = {$year_filter})";
        } else {
            $main_query .= "AND (MONTH(j.date) = MONTH(now()) AND YEAR(j.date) = {$year_filter})";
        }
    ...
    /**
     * Send Expense Summary Emails for LineManager and User
     * @param array $params
     * @return void
     */
    public function run_expense_summary_emails(array $params = [])
    {
        try {
            $output = 'S';
            $pdf_filename = 'expense_summary_' . date('dmYHis') . '.pdf';
            $sql = $this->expensesummary_model->getButtonExpenseSummary($params)->row();

            $employee_id = $sql->employee;
            $line_manager_id = $sql->line_manager;
            $total_amount = $sql->total_amount;

            // Get Country Data
            $country_data = $this->gherxlib->getCountryViaCountryId($this->country_id);

            // employees
            $employee_params = ['staff_id' => $employee_id];
            $employees = $this->gherxlib->getStaffInfo($employee_params)->row();
            $employees_fullname = "{$employees->FirstName} {$employees->LastName}";
            $employees_email = $sql->Email;

            // line_manager
            $line_manager_params = ['staff_id' => $line_manager_id];
            $line_manager_row = $this->gherxlib->getStaffInfo($line_manager_params)->row();
            $line_manager_name = "{$line_manager_row->FirstName} {$line_manager_row->LastName}";
            $line_manager_email = $line_manager_row->Email;

            /**
             * @var $dynamic_email - This variable will handle if we are sending emails to line_manager or users
             * @var $approval_text - Approval text
             */
            if (isset($params['email_approval_action']) && $params['email_approval_action'] === "line_manager") {
                $dynamic_email = $line_manager_email;
                $approval_text = "Approval Required!";
            } else {
                $dynamic_email = array($this->config->item('sats_accounts_email'), $employees_email);
                $approval_text = "Approved!";
            }

            $date_format = date('d/m/Y');
            $amount_format = number_format($total_amount, 2);
            $email_content = "<p>
                            <table style='border:none; margin: 0;'>
                                    <tr><td>Date: </td><td>{$date_format}</td></tr>
                                    <tr><td>Staff: </td><td>{$employees_fullname}</td></tr>
                                    <tr><td>Amount: </td><td>{$amount_format}</td></tr>
                                    <tr><td>Line Manager: </td><td>{$line_manager_name} <strong style='color:red;'>{$approval_text}</strong></td></tr>
                            </table>
                    </p>
                    <p>Please find attached Expense Claim Form</p>
            ";

            $subject = "Expense Summary for {$employees_fullname}";
            $to = $dynamic_email;
            $pdf_data = $this->expensesummary_model->get_expense_summary_pdf($params, $output);
            $email_data = [
                'content' => $email_content,
                'email_signature' => $country_data->email_signature,
                'trading_name' => $country_data->trading_name,
            ];
            ...
            if (!$this->email->send()) {
                log_message('error', 'Statement successfully saved but necessary emails were not broadcasted: ' . $this->email->print_debugger());
            }
        } catch (Exception $e) {
