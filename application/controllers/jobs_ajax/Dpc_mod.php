...
class Dpc_mod extends CI_Controller {
    ...
    public function ajax_dha_precomp_email_agency_accounts($ajob_id = "", $apdf_type = "", $aqt_type = "") {
		...
		if(empty($job_id)){
			log_message('error', 'ajax_dha_precomp_email_agency_accounts: Empty job id');
			return false;
		}
		...
		if($job_id!=''){
			...
			if(empty($email_job_details)){
				log_message('error', 'ajax_dha_precomp_email_agency_accounts: Empty $email_job_details');
				return false;
			}

			if(empty($jemail)){
				log_message('error', 'ajax_dha_precomp_email_agency_accounts: Empty $jemail');
				return false;
			}
