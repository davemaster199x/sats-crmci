...
class Daily extends MY_Controller {
    ...
    /**
     * This variable handle the BREADCRUMBS
     * @var $breadcrumbs
     */
    private $breadcrumbs = [];

    /**
     * This variable should be used to handle $data from query results
     * @var $results
     */
    private $results = [];

    /**
     * This variables handle the Cache
     * @var $cacheDynamicKey
     * @var $cacheTTL
     */
    private $cacheDynamicKey = "";
    private $cacheTTL = 600;

    /**
     * This variable handle the filters
     * @var $state_filter
     * @var $job_type_filter
     */
    private $state_filter;
    private $job_type_filter;

    /**
     * This variable handle the country_id from configuration
     * @var $country_id
     */
    private $country_id;
    ...
    public function view_last_contact() 
    {
        ...
        if($export==1){
            ...
            $sa_tech = $this->staff_accounts_model->get_staff_accounts_with_country_access([
                        ...
                        'custom_where' => "ca.`country_id` ={$this->country_id}
                                AND sa.`Deleted` = 0
                                AND sa.`ClassID` = 6
                                AND sa.`active` = 1",
                        "sort_list" => array(
    ...
    public function multiple_jobs(){
        ...
        $this->state_filter = $this->input->get_post('state_filter');
        $agency_filter = $this->input->get_post('agency_filter');
        // $this->job_type_filter = $this->input->get_post('job_type_filter');
        ...
        // upfront bill
        $custom_where = "a.`allow_upfront_billing` = {$upfront_bill} AND NOT ( j.`job_type` = 'Yearly Maintenance' AND j.`status` = 'Pending' )";

        // state filter        
        if( $this->state_filter != '' ){
            $custom_where .= " AND p.`state` = '{$this->state_filter}' ";
        }     
        
        if( $agency_filter != '' ){
            $custom_where .= " AND a.`agency_id` = '{$agency_filter}' ";
        } 

        // if( $this->job_type_filter != '' ){
        //     $custom_where .= " AND j.`job_type` = '{$this->job_type_filter}' ";
        // } 
        ...
        /***  get dynamic agency_name lists ***/
        $data['agency_lists'] = $this->daily_model->get_dynamic_agency_name_on_multiple_jobs()->result();
        ...
        //if($upfront_bill == 0 && empty($this->state_filter)){
            // update page total
            $page_tot_params = array(
                'page' => $page_url,
                'total' => $total_rows
    ...
    public function unserviced(){
        ...
        if( $this->input->get_post('export') ==1 ){ //EXPORT
            ...
            $data['lists'] = $this->daily_model->get_unserviced_by_markers($offset,$per_page,$agency_filter,$this->job_type_filter,$is_show);
            $data['sql_query'] = $this->db->last_query(); //Show query on About
            ...
            $query = $this->daily_model->get_unserviced_by_markers('','','','',$is_show);
            $total_rows = $query->num_rows();
    ...
    public function unserviced_is_acknowledge() {
        $success = false;
        $property_id = $this->input->post('property_id');
        $acknowledge = $this->input->post('acknowledge');
        $hidden_from_pages = $this->input->post('hidden_from_pages');
        $staff_id = $this->session->staff_id;
        ...
        try {
            $params = array(
                ...
                'hidden_from_pages'   => $hidden_from_pages,
                'staff_id'            => $staff_id
            );

            $result = $this->daily_model->hide_properties($params);
    ...
    public function incorrectly_upgraded_properties(){
        ...
        if($this->config->item('country')==2){ //not applicable in NZ
            show_404();
        }
    ...
    public function overdue_nsw_jobs() {
        ...
        */
        ...
        $params = array(
            ...
            'custom_where' => $custom_where,
            'custom_where_arr' => array(
                $custom_where_overdue_tot, $custom_where_overdue_nsw_jobs_filter_time, $custom_where_ym_completed_last_year
            ),
            'p_deleted' => 0,
            'a_status' => 'active',
            'state_filter' => 'NSW',
    ...
    /**
     * Overdue QLD Jobs Page
     * @return void
     */
    public function overdue_qld_jobs()
    {
        /**
         * START -- Breadcrumbs
         * */
        $title = "Overdue QLD Jobs";
        $uri = '/daily/overdue_qld_jobs';
         ...
         */
        $this->state_filter = 'QLD';

        /**
         *  GET Job Type Data
         */
        $job_types_qry = $this->daily_model->getJobType();
        $data['job_types_qry'] = $job_types_qry;
        ...
        $data['service_types_qry'] = $this->system_model->getServiceTypes($params)->result();

        /**
         * Return all columns from the Query
         */
        $sel_query = "
            p.`property_id`, 
            p.`address_1` AS p_address1,
            ...
            j.`id` AS jid,
            j.`status` AS jstatus,
            DATE(j.`created`) AS jcreated,
            DATEDIFF(Date(j.`due_date`), CURDATE()) AS deadline,
            
            `psr`.`subregion_name` AS `postcode_sub_region`
        ";

        /**
         * SQL Where Clause
         * dynamic:: due_date should be IS NOT NULL AND Less than or equal to +5 days
         */
        $next_5_days = date('Y-m-d',strtotime("+5 days"));
        $custom_where = "
            j.`del_job` = 0 AND 
            j.`status` = 'To Be Booked' AND
            j.`job_type` IN ('Lease Renewal', 'Change of Tenancy') AND
            (  j.`due_date` IS NOT NULL AND j.`due_date` <= '{$next_5_days}' ) AND 
            a.`deleted` = 0 AND
            a.`franchise_groups_id` != 14
        ";
        ...
        $params = array(
            ...
            'custom_joins' => [
                'join_table' => 'property_services` AS ps',
                'join_on' => ' (p.property_id = ps.property_id AND ps.alarm_job_type_id = j.service AND ps.service = 1 AND ps.alarm_job_type_id != 6 )', // must be serviced to SATS and excluding CW
                'join_type' => 'inner'
            ],
            'join_table' => ['jobs', 'postcode', 'sub_regions', 'regions'],
            'custom_where' => $custom_where,
            'sort_list' => [$sort_list]
        );
        ...
        /**
         * Cache for Overdue QLD Jobs
         */
        $this->cacheDynamicKey = "cacheOverdueQLDJobs";
        $this->results = $this->getCacheDatas($lists, $this->cacheDynamicKey);

        /**
         * @variable $end_date_color - if end date is greater than today_date then we show this color(#000000), by default we show this color(#ff0000)
         * @variable $end_date date format (d/m/Y)
         * @variable $allow_en (Allow Entry Notice) - This function getAllowEnValues() will return string YES/NO/NR/ONLY
         * @variable $bg_red (Background Red)
         * @variable $is_eo (Electrician Only)
         * @variable $is_eo_txt (Electrician Only) - display Yes or empty string
         */
        $end_date_color = "#ff0000";
        $is_eo_bg_color = "transparent";
        $bg_red = "";
        $is_eo_txt = "";
        $address = "";
        foreach ($lists as $row)
        {
            ...
            /**
             * @variables $keys - if property.no_en IS EQUAL to 1 means (No Entry Notice) OR property.no_keys IS EQUAL to 1 means (No Keys)
             * @variables $address - Property Address
             */
            $keys = (int)$row['no_en'] === 1 || (int)$row['no_keys'] === 1 ? "" : "YES";
            $end_date = $this->system_model->isDateNotEmpty($row['due_date']) ? $this->system_model->formatDate($row['due_date'], 'd/m/Y') : NULL;
            $today_date = date('d/m/Y');
            $allow_en = $this->customlib->getAllowEnValues($row['allow_en']);
            $is_eo = (int)$row['is_eo'];

            if ($end_date > $today_date) {
                $end_date_color = "#000000";
            }

            if ($is_eo === 1) {
                $is_eo_bg_color = "#e77e7e73";
                $is_eo_txt = "YES";
            }
            ...
            if ($allow_en === "YES" || $keys === "YES" || $is_eo === 1) {
                $bg_red = "#ff0000";
            }

            $address = "{$row['p_address1']} {$row['p_address2']}, {$row['p_address3']} {$row['p_state']} {$row['p_postcode']}";

            $this->results['job_id'][] = $row['jid'];
            $this->results['property_id'][] = $row['property_id'];
            $this->results['end_date'][] = $end_date;
            $this->results['allow_en'][] = $allow_en;
            $this->results['address'][]  = $address;
            $this->results['service_type'][] = $this->system_model->display_job_icons($job_icons_params);
            $this->results['state'][] = $row['p_state'];
            $this->results['regions'][] = $row['postcode_sub_region'];
            $this->results['agency_name'][] = $this->gherxlib->crmLink('vad',$row['agency_id'],$row['agency_name']);
            $this->results['preferred_time'][] = $row['preferred_time'];
            $this->results['job_type'][] = $row['j_type'];
            $this->results['keys'][] = $keys;
            $this->results['is_eo'][] = $is_eo_txt;
            $this->results['is_eo_bg_color'][] = $is_eo_bg_color;
            $this->results['end_date_color'][] = $end_date_color;
            $this->results['bg_red'][] = $bg_red;

            $this->results[] = $row;
        }

        $data['lists'] = $this->results;
        $result_counts = count($this->results);
        ...
        $techs = $this->system_model->getTech($params)->result_array();
        ...
        foreach ($techs as $tech) {
            $tech_data['fullname'][] = $this->system_model->formatStaffName($tech['FirstName'],$tech['LastName']).( ( $tech['is_electrician'] == 1 )?' [E]':null );
            $tech_data[] = $tech;
        }
        $data['tech'] = $tech_data;

        /**
         * Update home page box total
         */
        $page_tot_params = [
            'name' => 'overdue-qld-jobs',
            'total' => $result_counts
        ];
        $this->system_model->update_main_page_total($page_tot_params);
    ...
    public function active_properties_without_jobs()
    {
        ...
        $query = "
            ...
            AND a_main.`country_id` = {$this->country_id}
        ...
        if ($export == 1) {
            ...
            $result = $this->daily_model->get_active_properties_without_jobs($this->country_id, $agency_filter, $offset, $per_page);
            ...
            if (!empty($agency_filter)) {
                ...
                $data['property_sql'] = $this->daily_model->get_active_properties_without_jobs($this->country_id, $agency_filter, $offset, $per_page);
    
                // total row
                $property_sql = $this->daily_model->get_total_rows_property_count($this->country_id, $agency_filter);
     ...
     */
    private function getCacheDatas(array $datas, string $key): array
    {
        ...
        if (empty($cached_data)){
            ...
            $this->cache->save($key, $cached_data, $this->cacheTTL);
        }
