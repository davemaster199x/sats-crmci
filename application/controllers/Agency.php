...
class Agency extends MY_Controller {
    ...
    public function add_agency(){
        ...
        //Get all agency alarms
        $data['alarm_sql'] = $this->agency_model->get_alarms();
    ...
    public function add_new_agency(){
        ...
        if ($this->form_validation->run() == true) {
			...
			redirect(base_url('/agency/view_agency_details/'.$agency_last_id),'refresh');
        }else{
    ...
    public function agency_send_mail($params){
        ...
        // To email
        $email_to_arr = []; // clear
        $email_to_arr[] = make_email('accounts');
        $email_to_arr[] = make_email('workorders');
        
        $email_subject = "New Agency Added";
        if (!empty($email_data['agency_id'])) {
            $email_subject = ucfirst($email_data['old_status']) . " Agency Activated";
        }

        //email
        $this->email->to($email_to_arr);
        $this->email->cc([
            make_email('sales'), make_email('info')
        ]);
//        $this->email->bcc(make_email('bcc'));
  ...
  /**
   * This is the version 2 for active agency list
   * @return void
   */
  public function view_agencies_v2()
    {
        $title = "View Agencies";
        $uri = '/agency/view_agencies_v2';
        ...
        $data = [
            'bc_items' => $this->breadcrumbs,
            'states_label' => $this->gherxlib->getDynamicState($this->country_id),
            'regions_label' => $this->gherxlib->getDynamicRegion($this->country_id),
            'agency_number_abbr' => ($this->country_id === 1) ? 'ABN' : 'NZBN'
        ];

        //load views
        $this->load->view('templates/inner_header', $data);
        $this->load->view('agency/view_agencies_v2', $data);
        $this->load->view('templates/inner_footer', $data);
    }
    ...
    public function view_agencies()
    {
        ...
        $source_of_client_filter = $this->input->get_post('source_of_client_filter');
        ...
        $custom_where_ar = [];
        if( $source_of_client_filter > 0 ){
            $custom_where_ar[] = "sac.`sac_id` = {$source_of_client_filter}";
        }
        ...
        $params = array(
            ...
            'custom_joins_arr' => array(
                ...
                array(
                    'join_table' => 'agencies_from_other_company AS afoc',
                    'join_on' => 'a.agency_id = afoc.agency_id AND afoc.`active` = 1',
                    'join_type' => 'left'
                ),
                array(
                    'join_table' => 'smoke_alarms_company AS sac',
                    'join_on' => 'afoc.`company_id` = sac.`sac_id`',
                    'join_type' => 'left'
                )
            ...
            'custom_where_arr' => $custom_where_ar,
            'sort_list' => array(
                array(
                    'order_by' => 'a.`agency_name`',
                    'sort' => 'ASC'
        ...
        if ($this->input->get_post('export') && $this->input->get_post('export') == 1) { //EXPORT
            ...
            if(!empty($agencyIds)){
                ...
                ##get last contact
                $last_contact = $this->db->select('l.agency_id, MAX(l.created_date) as last_contact')
                ->from('logs AS l')
                ->join('staff_accounts AS sa', 'l.created_by_staff = sa.StaffID', 'left')
                ->where_in('l.agency_id', $agencyIds)
                ->where('l.display_in_vad', 1)
                ->where('sa.display_on_wsr', 1) // filtered from user details "other" section, requested by ben
                ->group_by('l.agency_id')
                ->get()->result_array();
            ...
            // get DISTINCT smoke alarms company 
            $custom_where_ar2[] = "sac.`sac_id` > 0";
            $params_total = array(
                'sel_query' => "DISTINCT(sac.`sac_id`), sac.`company_name`",
                'join_table' => array('postcode_regions', 'salesrep', 'country', 'agency_priority', 'agency_priority_marker_definition'),
                'custom_joins_arr' => array(
                ...
                'postcodes' => $postcodes,
                'high_touch_filter' => $agency_ht_filter,
                'custom_where_arr' => $custom_where_ar2,
                'sort_list' => array(
                    array(
                        'order_by' => 'sac.`company_name`',
                        'sort' => 'ASC',
                    )
                ),
                'display_query' => 0
            );
            $data['source_of_client_filter'] = $this->agency_model->get_agency($params_total);
             
            //base url params
            $pagi_links_params_arr = array(
                'state_filter' => $state_filter,
                'agency_ht_filter' => $agency_ht_filter,
                'sales_rep_filter' => $sales_rep_filter,
                'sub_region_ms' => $sub_region_ms,
                'search_filter' => $search_filter,
                'source_of_client_filter' => $source_of_client_filter
            );
            ...
            $pagi_link_params = "{$uri}/?".http_build_query($pagi_links_params_arr);  
             ...
             // explort link
            $data['export_link'] = "{$uri}/?export=1&export_type=a&".http_build_query($pagi_links_params_arr);  
    ...
    public function get_agency_normal_export($params){
        ...
        // $lists_export_query = $this->agency_model->get_agency($params);
        $lists_export_query = $this->agency_model->get_agency($params)->result_array();
    ...
    public function view_agency_details($agency_id=null, $tab=1)
    {
        ...
        ##pass data
        $data['agency_id'] = (int) $agency_id; #pass to view
        ...
        //get franchise group
        $data['fg_sql'] = $this->db->select('*')->from('franchise_groups')->where('country_id',$this->config->item('country'))->order_by('name')->get();
        $disable_user = $this->db->query("
            SELECT ClassID FROM `staff_accounts` WHERE StaffID = {$this->session->staff_id}
        ")->result();
        $data['fg_users_disable'] = $disable_user[0]->ClassID;
        ...
        $data['agency_api_documents_data'] = $this->agency_model->get_agency_api_documents($agency_id)->row();
        ...
        if($tab==3){ ##load only for Portal Users tab > get/load portal users datas/items
            ...
            $sel_query="
                ...
                au_2fa.`type` AS au_2fa_type,
                au_2fa.`send_to` AS au_2fa_send_to,
            ...
            */
            ...
            //get agency Paid alarms preference
            $data['paid_alarms_pref'] = $this->agency_model->get_agency_preference_label(22);
            $data['paid_alarms_pref_selected'] = $this->agency_model->get_agency_preference_selected(22,$agency_id);
            //get agency Paid alarms preference end

            //get agency Photos on Compliance Cert preference
            $data['photos_on_complicance_pref'] = $this->agency_model->get_agency_preference_label(23);
            $data['photos_on_complicance_pref_selected'] = $this->agency_model->get_agency_preference_selected(23,$agency_id);
            //get agency Photos on Compliance Cert preference end

            //get agency "No Annual Visits" preference
            $data['no_annual_visits'] = $this->agency_model->get_agency_preference_label(24);
            $data['no_annual_visits_selected'] = $this->agency_model->get_agency_preference_selected(24,$agency_id);
            //get agency "No Annual Visits" preference end

            // Agency Renewals in Portal
            $data['renewals_agency_portal_pref'] = $this->agency_model->get_agency_preference_label(25);
            $data['renewals_agency_portal_pref_selected'] = $this->agency_model->get_agency_preference_selected(25,$agency_id);

            // Send renewals reminders via Email
            $data['renewals_reminder_via_email_pref'] = $this->agency_model->get_agency_preference_label(26);
            $data['renewals_reminder_via_email_pref_selected'] = $this->agency_model->get_agency_preference_selected(26,$agency_id);

            $data['agency_api_documents_data'] = $this->agency_model->get_agency_api_documents($agency_id)->row();

            $api_params = array(
                'sel_query' => "agen_api_int.`api_integration_id`, agen_api_int.`connected_service`, agen_api_int.`active`, agen_api_int.`date_activated`,agen_api.`api_name`, agen_api.`agency_api_id`",
                'agency_id' => $agency_id,
                'sort_list' => array(
                    array(
             ...
             * Please use this as reference for agency_api*
             */
            ...
            if(!empty($connected_serviceByIds)){
                ...
                foreach( $connected_serviceByIds as $conn_ser_api_id ){

                    if( $conn_ser_api_id == 5 ){ // console webhooks - API keys
                        ...
                        foreach( $console_sql->result() as $console_row ){
                            $connected_serviceById[5]['agency_api_token_id'] = $console_row->id; // Console API ID
                            $connected_serviceById[5]['agency_api_id'] = 5; // console API
                        }

                    }else{ // default API, using agency tokens
            ...
            // Check if API is connected or not
            $api_header = '';
            foreach($api_q as $api_row){
                //for api console
                if ($api_row['agency_api_id'] === 5 && !empty($api_row['agency_api_id']) && !empty($api_row['agency_api_token_id'])) {
                    ...
                    $data["agency_console_api_connected"] = ($cak_sql->row()->cak_count > 0) ? 1 : 0;
                    $api_header .= $api_row['api_name'] . ' | ';
                } else {
                    // for other api's
                    if (!empty($api_row['agency_api_token_id']) && (int)$api_row['active'] == 1) {
                        $api_header .= $api_row['api_name'] . ' | ';
                    }
                }

                $data['upload_pref_api_id'] = $api_row['agency_api_id'];

            }

            // Dynamic API header
            $data['api_header'] = empty($api_header) ? '(No Active API)' : rtrim($api_header, " | ");

            //If logged user is global
            $staff_sql_str = $this->db->query("
                SELECT COUNT(`StaffID`) AS sa_count
                FROM `staff_accounts`
                WHERE `StaffID` = {$this->session->staff_id}
                AND `ClassID` = 2
            ");
            $data['logged_user_is_global'] = $staff_sql_str->row()->sa_count > 0;
            ...
            // pagination end
             ...
             $active_agency_contact_type_params = array(
                'sel_query'=> "main_log_type_id, contact_type",
                'sort_list' => array(
                    array(
             ...
             $log_limit = $this->config->item('pagi_per_page');
             $log_offset = $this->input->get_post('offset');
             $sel_query = "l.log_id,l.created_date,l.title,l.details,ltit.title_name,aua.fname,aua.lname,aua.photo,sa.StaffID,sa.FirstName,sa.LastName,l.created_by_staff,l.log_type";
            ...
            $is_nlm_filter = ($prop_status == 0) ? 'is_nlm = 0' : (($prop_status == 1) ? 'is_nlm = 1' : '');
            ...
            if($prop_type == 1 || !$prop_type){ // SATS service
                ...
                $params_prop_sats = array(
                    'sel_query' => 'p.property_id',
                    'agency_filter' => $agency_id,
                    'is_nlm' => 0, // 0 means there's no is_nlm filter
                ...
                $params_prop = array( 
                    ...
                    'pm_id' => $pm_id,
                    'is_nlm' => 0, // 0 means there's no is_nlm filter
                    'custom_where_arr' => array($not_in_where,$is_nlm_filter),
                    'group_by' => 'p.property_id',
                    'sort_list' => array(
                        array(
                            'order_by' => 'p.address_2',
                ...
                $inactive_count = 'is_nlm = 1'; // get all the inactive property
                $params_prop_inactive = array(
                    ...
                    'is_nlm' => 0, // 0 means there's no is_nlm filter
                    'pm_id' => $pm_id,
                    'custom_where_arr' => array($not_in_where,$inactive_count),
                    'group_by' => 'p.property_id',
                    'sort_list' => array(
                        array(
                            'order_by' => 'p.address_2',
                ...
                $params_prop = array(
                    ...
                    'pm_id' => $pm_id,
                    'join_table' => array('property_services','jobs'),
                    'custom_where_arr' => array($prop_custom_where,$is_nlm_filter),
                    'group_by' => 'p.property_id',
                    'sort_list' => array(
                        array(
                            'order_by' => 'p.address_2',
            ...
            if($export==1){ // EXPORT CSV
                ...
                $header = array("Address", "Suburb", "1st Tenant Name", "1st Tenant Ph", "1st Tenant Mobile", "1st Tenant Email", "2nd Tenant Name", "2nd Tenant Ph", "2nd Tenant Mobile", "2nd Tenant Email","3rd Tenant Name","3rd Tenant Ph","3rd Tenant Mobile","3rd Tenant Email","4th Tenant Name","4th Tenant Ph","4th Tenant Mobile","4th Tenant Email","Last Attended","Property Manager","Land Lord","Key Number","Status","NLM Date");
                 ...
                 } else if($agency_id == 1598 && $this->config->item('country')==1){
                ...
                foreach($export_property as $row){
                    ...
                    if( $agencies['franchise_groups_id'] == 39 || ($agency_id == 1598 && $this->config->item('country')==1)){
                ...
                if( $prop_type==1 ){ // Count active/not nlm only
                    ...
                    $params_prop_count = array(
                        ...
                        'agency_filter' => $agency_id,
                        'search' => $prop_search,
                        'is_nlm' => 0, // not include is_nlm filter
                        'pm_id' => $pm_id,
                        'custom_where_arr' => array($not_in_where,$is_nlm_filter),
                        'group_by' => 'p.property_id',
                        'display_query' => 0
                    ...
                    $params_prop_count = array(
                        ...
                        'pm_id' => $pm_id,
                        'join_table' => array('property_services','jobs'),
                        'custom_where_arr' => array($not_in_where,$prop_custom_where,$is_nlm_filter),
                        'group_by' => 'p.property_id',
                        'display_query' => 0
                    );
                }
                ...
                foreach( $connected_serviceByIds as $conn_ser_api_id ){
                    ...
                    if( $conn_ser_api_id == 5 ){ // console webhooks - API keys
                        ...
                        foreach( $console_sql->result() as $console_row ){
                            $connected_serviceById[5]['agency_api_token_id'] = $console_row->id; // Console API ID
                            $connected_serviceById[5]['agency_api_id'] = 5; // console API
                            $connected_serviceById[5]['token_connection_date'] = $console_row->created; // token connection date
                        }
                        ...
                        $api_custom_where = "agen_api_tok.api_id IN({$connected_serviceByIds_imp})";
                        $get_agency_api_tokens_params = array(
                            'sel_query' => 'agen_api_tok.agency_api_token_id, agen_api_tok.api_id,connection_date',
                            'active' => 1,
                            'agency_id' => $agency_id,
                            'custom_where' => $api_custom_where
                        ...
                        foreach($agency_api_tokens as $agency_api_token){
                            $connected_serviceById[$agency_api_token['api_id']]['agency_api_token_id'] = $agency_api_token['agency_api_token_id'];
                            $connected_serviceById[$agency_api_token['api_id']]['agency_api_id'] = $agency_api_token['api_id']; // no use yet
                            $connected_serviceById[$agency_api_token['api_id']]['token_connection_date'] = $agency_api_token['connection_date']; // token connection date 
                        } 
            ...
            $data['pt_has_preference'] = ( $pt_agen_pref_sql->num_rows() > 0 )?true:false;
            $data['pt_agen_pref_row'] = $pt_agen_pref_sql->row();
        ...
        $data['connectedAPI'] = reset($api_q);

        $user_class = $this->staff_accounts_model->fields('ClassID')->as_array()->get($this->session->staff_id);
        $data['user_class'] = $user_class['ClassID'];
    ...
    /**
     * Update Agency for emails, agency status, agency comments, agency details, agency emails, account emails
     * @TODO - Remove unused variables (We should remove unused variables to avoid confusion but we need to ask other devs to verify)
     * @return void
     */
    public function update_agency()
    {
        ...
        $curr_agency_det_params = array('sel_query'=>'a.status', 'country_id'=> $this->config->item('country'), 'agency_id'=>$agency_id );
        ...
        if($agency_id!="" && is_numeric($agency_id)){ //Validate
            ...
            if($tab==1){ ## >>>>>>>>>>>>>>>>>>>>>>>>>>>>
                ...
                $allow_upfront_billing = $this->input->post('allow_upfront_billing');
                $invoice_pm_only = ( is_numeric($this->input->post('invoice_pm_only')) )?$this->input->post('invoice_pm_only'):null;
                ...
                $add_inv_to_agen = ( is_numeric($this->input->post('add_inv_to_agen')) )?$this->input->post('add_inv_to_agen'):null;
                ...
                $is_invoice = $this->input->post('send_invoices');
                $is_certificate = $this->input->post('send_certificates');
                ...
                $preferences_value = [];
                $current_value = null;
                foreach($agency_pref_arr as $agency_preferences_id => $agency_preferences_val ) {
                    $prev_value_selected = $this->agency_model->get_agency_preference_selected($agency_preferences_id, $agency_id);

                    // We assign null || -1 || 0 || 1 to $current_value variable
                    // If $prev_value_selected return null or -1 then it should match to current values
                    if ($prev_value_selected == null && $agency_preferences_val == 1) {
                        $current_value = null;
                    } else if($prev_value_selected == null && $agency_preferences_val == "-1") {
                        $current_value = "-1";
                    } else {
                        $current_value = $agency_preferences_val;
                    }

                    $preferences_value[$agency_preferences_id] = [
                        'prev' => $prev_value_selected,
                        'current' => $current_value,
                    ];
                }

                foreach($preferences_value as $agency_pref_id => $agency_pref_val)
                {
                    //insert/update if the prev is not equal to current data
                    if ($agency_pref_val['prev']->sel_pref_val != $agency_pref_val['current']) {
                        $proceed_insert = true;
                        $log_label = $this->agency_model->get_agency_preference_label($agency_pref_id);
                        $prev_value = $agency_pref_val['prev']->sel_pref_val;

                        if( $agency_pref_id == 24 && $agency_pref_val['current'] == 1 ){
                        ...
                        if ($agency_id > 0 && $agency_pref_id > 0) {
                            ...
                            $from = $this->get_value_api_preferences($prev_value);
                            $to = $this->get_value_api_preferences($agency_pref_val['current']);

                            if ($agency_pref_sel_sql->row()->aps_count > 0) { // exist, update

                                $update_data = array(
                                    'sel_pref_val' => $agency_pref_val['current']
                                );
                                $this->db->where('agency_id', $agency_id);
                                ...
                                $log_details = "<strong>Preferences $log_label->pref_text</strong> Updated from {$from} to {$to}";
                                $log_params = array(
                                    'title' => 46, // agency
                                    'details' => $log_details,
                                    'display_in_vad' => 1,
                ...
                /**
                 * We are saving the invoices/certificate preferences
                 * **/
                try {
                    ...
                    if (!empty($agency_id)) {
                        ...
                        if ($count > 0) {
                            ...
                            if ($this->input->post('send_invoices') != $this->input->post('og_send_invoices') || $this->input->post('send_certificates') != $this->input->post('og_send_certificates')) {
                                //insert
                                $data = array(
                                    'agency_id' => $agency_id,
                                    'is_invoice' => $is_invoice,
    ...
    private function _insertAgencyLogs($post, $tab){
        ...
        if($tab==1){ ##Agency details tab
                ...
                if( $post['og_active_prop_with_sats']!=$post['active_prop_with_sats'] ){
                    $edited_field_arr[] = array(
                        'field' => "Active Properties with ".$this->config->item('company_name_short'),
                ...
                if( $post['joined_sats']!=$post['og_joined_sats'] ){
                    $edited_field_arr[] = array(
                        'field' => "Joined ".$this->config->item('company_name_short'),
            ...
            if( $post['allow_en']!=$post['og_allow_en'] ){
                ...
                if ($post['allow_en'] == 2) {
                    $this->agency_send_mail_allow_en($post['agency_id']);
                }
            }
            ...
            if( $post['send_invoices']!=$post['og_send_invoices'] ){
                $og_send_invoices = $this->_checkbox_yes_no($post['og_send_invoices']);
                $send_invoices = $this->_checkbox_yes_no($post['send_invoices']);
                $edited_field_arr[] = array(
                    'field' => "Send Invoices via API not email",
                    'msg' => "Updated from {$og_send_invoices} to {$send_invoices}"
                );
            }

            if( $post['send_certificates']!=$post['og_send_certificates'] ){
                $og_send_certificates = $this->_checkbox_yes_no($post['og_send_certificates']);
                $send_certificates = $this->_checkbox_yes_no($post['send_certificates']);
                $edited_field_arr[] = array(
                    'field' => "Send Certificates via API not email",
                    'msg' => "Updated from {$og_send_certificates} to {$send_certificates}"
                );
            }
    ...
    public function agency_send_mail_allow_en($agency_id){
        ...
        $email_subject = $agency_name." Allow Entry Notice Only";
        ...
        Hi Team,
        <br><br>
        Please be advised, has updated their booking preference to Entry Notice ONLY.
        <br><br>
        Please update any schedules applicable
        ";
        $this->email->message($body);
        $this->email->send();
    ...
    public function ajax_unlink_connected_api_prop(){
        ...
        if($agency_id!="" && $agency_id >0){
            ...
            LEFT JOIN `property` AS p ON cp.`crm_prop_id` = p.`property_id`
            LEFT JOIN `agency` AS a ON p.`agency_id` = a.`agency_id`
            WHERE a.`agency_id` = {$agency_id}
            ");
    ...
    /**
     * AJAX Request
     * unlink and remove or delete integrated api
     */
    public function ajax_unlink_disable_connected_api(){
        ...
        $remove_token = $this->input->post('remove_token');
        $del_api_integ = $this->input->post('del_api_integ');
                ...
                switch($api_id){
                    ...
                    case 3: // PropertyTree
                        ...
                        if( $agency_id > 0 ){
                            ...
                            $this->db->where('agency_id', $agency_id);
                            $this->db->delete('propertytree_agency_preference');
                    ...
                    case 6: // OURTRADIE
                        ...
                        $data = array('ourtradie_supplier_id'=>NULL);
                        $this->db->where('agency_id', $agency_id);
                        $this->db->update('agency',$data);
    ...
    {
        $jdata['status'] = false;
        $agency_id = (int)$this->input->post('agency_id');
        $api_billable = (int)$this->input->post('api_billable');
        $og_api_billable = (int)$this->input->post('og_api_billable');
                ...
                if($type=="btn_update_agency_name"){ ##UDPATE AGENCY NAME
                    ...
                    if($status!=""){
                            ...
                            if ($status == 'active' && ( $og_status=="target" || $og_status=="deactivated" ) ) {
                                ...
                                $agency_params = array('sel_query'=>'a.*', 'country_id'=> $this->config->item('country'), 'agency_id'=>$agency_id );
                    ...
                    $yes_including_prop = $this->input->get_post('yes_including_prop');
                    ...
                    if( $afoc_sql->num_rows() > 0 ){ // if exist, update
                        ...
                        if( $from_other_company > 0 ){ // if company is selected
                            ...
                            //insert log
                            $log_details = "Agency detail <b>Source of Client</b> has been updated from <b>{$afoc_row->company_name}</b> to <b>{$company_row->company_name}</b>";
                            $log_params = array(
                    ...
                    if( $yes_including_prop == 1 ){
                        ...
                        foreach( $prop_sql->result() as $prop_row ){
                            
                            // deactivate current active
                            $update_data = array(
                                'active' => 0
                            );        
                            $this->db->where('property_id', $prop_row->property_id);
                            $this->db->where('active', 1);
                            $this->db->update('properties_from_other_company', $update_data);

                            // insert new
                            $insert_data = array(
                                'property_id' => $prop_row->property_id,
                                'company_id' => $from_other_company,
                                'added_date' => $today
                            );        
                            $this->db->insert('properties_from_other_company', $insert_data);
    ...
    public function agency_health_check($agency_id=null) {
        ...
        $data['about_query']['total_property'] = $this->db->last_query();
        ...
        $data['about_query']['joined_sats'] = $this->db->last_query();
        ...
        $data['api'] = $query;
        $data['about_query']['api'] = $this->db->last_query();
        ...
        $data['about_query']['new_properties'] = $this->db->last_query();
        ...
        $data['about_query']['deactivate_properties'] = $this->db->last_query();
        ...
        $data['about_query']['jobs_send_letters'] = $this->db->last_query();
        ...
        $data['about_query']['jobs_booked'] = $this->db->last_query();
        ...
        $data['about_query']['jobs_on_hold'] = $this->db->last_query();
        ...
        $data['about_query']['jobs_escalate'] = $this->db->last_query();
        ...
        $data['about_query']['jobs_allocate'] = $this->db->last_query();
        ...
        $data['about_query']['jobs_to_be_booked'] = $this->db->last_query();
        ...
        $data['about_query']['jobs_merged'] = $this->db->last_query();
        ...
        $data['about_query']['jobs_completed'] = $this->db->last_query();
        ...
        $data['about_query']['jobs_missed'] = $this->db->last_query();
        ...
        $data['list'] = $sms_sql;
        $data['about_query']['sms_list'] = $this->db->last_query();
        ...
        $data['about_query']['jobs_failed_qld'] = $this->db->last_query();
        ...
        $data['about_query']['jobs_failed_nsw'] = $this->db->last_query();
    ...
    public function add_source_of_client(){

        $company_name = $this->input->get_post('soc_company_name');

        $insert_data = array(
                'company_name' => $company_name
        );        
        $this->db->insert('smoke_alarms_company', $insert_data);

    }


    public function update_source_of_client(){

        $sac_id = $this->input->get_post('sac_id');
        $soc_company_name = $this->input->get_post('soc_company_name');

        $update_data = array(
            'company_name' => $soc_company_name
        );        
        $this->db->where('sac_id', $sac_id);
        $this->db->update('smoke_alarms_company', $update_data);

    }

    public function delete_source_of_client(){
    ...
    public function update_property_and_job_service_type_process(){
        ...
        $update_last_ym_comp_job_serv_chk = $this->input->get_post('update_last_ym_comp_job_serv_chk'); 
        $update_last_ym_comp_job_price_chk = $this->input->get_post('update_last_ym_comp_job_price_chk'); 
        $update_any_non_comp_job_serv_chk = $this->input->get_post('update_any_non_comp_job_serv_chk'); 
        $update_any_non_comp_job_price_chk = $this->input->get_post('update_any_non_comp_job_price_chk');   
        
        $staff_id = $this->session->staff_id;

        $jobs_with_merge_precomp_arr = [];

        // get service type name for logs
        // FROM service
        ...
        $price_var_params = array(
            'service_type' => $from_service_type,
            'agency_id' => $agency_id
        );
        $price_var_arr = $this->system_model->get_agency_price_variation($price_var_params);	
        $from_service_type_price = $price_var_arr['dynamic_price_total'];			
        ...
        $price_var_arr = $this->system_model->get_agency_price_variation($price_var_params);	
        $to_service_type_price = $price_var_arr['dynamic_price_total'];		
        
        // log details
        $log_details = 'Service was updated from <b>'.$from_service_type_name.' $'.number_format($from_service_type_price,2).'</b> to <b>'.$to_service_type_name.' $'.number_format($to_service_type_price,2).'</b>';

        // update job service or price
        if( count($prop_id_arr) > 0 ){            
            ...
            AND ps.`service` = 1
            AND a.`agency_id` = {$agency_id}  
            AND p.`property_id` IN({$prop_id_imp})    
            ");

            foreach( $prop_sql->result() as $prop_row ){
                                                                      
                if( $update_last_ym_comp_job_serv_chk == 1 ){ // Last YM Completed 

                    // get latest YM completed job
                    ...
                    WHERE j.`property_id` = {$prop_row->property_id}
                    AND j.`service` = {$from_service_type}
                    AND j.`job_type` = 'Yearly Maintenance'
                    AND j.`status` = 'Completed'  
                    AND j.`del_job` = 0               
                    ...
                    if( $job_sql->num_rows() > 0 ){
                        ...
                        if( $job_row->j_id > 0 ){
                            ...
                            $this->agency_model->vad_change_service_process($params_obj);
                ...
                if( $update_any_non_comp_job_serv_chk == 1 || $update_any_non_comp_job_price_chk == 1 ){ // Non-Completed 
                ...
                if( $update_any_non_comp_job_serv_chk == 1 || $update_any_non_comp_job_price_chk == 1 ){ 
                    ...
                    // get merge and precomp jobs
                    $job_sql2 = $this->db->query("
                    SELECT 
                        DISTINCT(p.`property_id`), 
                    ...
                    WHERE j.`property_id` = {$prop_row->property_id}
                    AND j.`service` = {$from_service_type}
                    AND j.`status` IN('Merged Certificates','Pre Completion')          	
                    ");
                    
                    foreach( $job_sql2->result() as $job_row ){

                        $prop_address = "{$job_row->p_address_1} {$job_row->p_address_2}, {$job_row->p_address_3} {$job_row->p_state} {$job_row->p_postcode}";

                        $jobs_with_merge_precomp_arr[] = array(
                ...
                SET pv.`active` = 0
                WHERE pv.`property_id` = {$prop_row->property_id}
                AND apv.`reason` != 3
                ");
            ...
            foreach( $ps_sql->result() as $ps_row ){
                ...
                // check property if the "service TO" type already exist
                $ps_inner_sql = $this->db->query("
                SELECT COUNT(`property_services_id`) AS ps_count
                FROM `property_services`
                WHERE `alarm_job_type_id` = {$to_service_type}
                AND `property_id` = {$ps_row->property_id}
                AND `service` = 1
                ");
                
                if( $ps_inner_sql->row()->ps_count == 0 ){ // no serviced to SATS found
                    
                    // clear "service TO" non serviced to SATS
                    $this->db->query("
                    DELETE 
                    FROM `property_services`
                    WHERE `alarm_job_type_id` = {$to_service_type}
                    ...
                    LEFT JOIN `property` AS p ON ps.`property_id` = p.`property_id`
                    LEFT JOIN `agency` AS a ON p.`agency_id` = a.`agency_id`
                    SET ps.`alarm_job_type_id` = {$to_service_type}
                    WHERE ps.`property_services_id` = {$ps_row->property_services_id} 
                    ";
            ...
            echo json_encode($jobs_with_merge_precomp_arr);            

        }		

    }

    public function get_value_api_preferences($value): string
    {
        $from = '';
        switch ($value) {
            case 1:
