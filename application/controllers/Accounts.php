...
//        $this->load->view('templates/inner_footer', $data);
    ...
    public function pdfexport_statement() {            
        ...
        $agency_id = HashEncryption::decodeString(rawurldecode($this->input->get_post('a')));
        $from = HashEncryption::decodeString(rawurldecode($this->input->get_post('f')));
        $to = HashEncryption::decodeString(rawurldecode($this->input->get_post('t')));;
        $phrase = HashEncryption::decodeString(rawurldecode($this->input->get_post('p')));
        ...
        foreach ($agency_id_arr as $json) {
            ...
            */
            ...
            if ($this->email->send()) {
                ...
                //insert logs
                $log_details = "Statement as of " . date('d/m/Y') . " <a href='{$this->config->item("crmci_link")}/accounts/pdfexport_statement/?a=".rawurlencode(HashEncryption::encodeString($agency_id))."'>Statement PDF</a>";
                $log_params = array(
                    'title' => 45, //statement
                    'details' => $log_details,
                    'display_in_vad' => 1,
        ...
        */
        ...
        foreach($agen_pay_sql->result() as $index => $agen_pay_row){
            ...
            if( $agen_pay_row->remittance == 0 ){
                ...
                $agency_pay_description = "Payment received {$agency_pay_date}. Please email a remittance advice to {make_email('accounts')}";
                    ...
                    <tr>                                                            
                        <td>
                            ...
                            <span class="invoice_num_span mr-1">
                                <a href='<?php echo $this->config->item("crmci_link"); ?>/jobs/details/<?php echo $j_row->jid; ?>'>
        ...
        if( $ageny_pay_j_id > 0 ){
            ...
            if( isset($inv_pay_row) ){
                ...
                if( $transfer_invoice == 1 ){
                    ...
                    $log_title = 43; // Payment    
                    $acc_log = "Transferred <strong>\${$inv_pay_row->amount_paid}</strong> from <a href='{$this->config->item("crmci_link")}/jobs/details/{$job_id}'>{$job_id}</a>, because <strong>{$remove_invoice_reason}</strong>. Ref: <a href='".BASEURL."accounts/agency_payments/?agency_payments_id={$agency_payments_id}&open_edit_lb=1'>{$payment_reference}</a>";                        
                    $log_params = array(
                        'title' => $log_title,
                        'details' => $acc_log,
                        'display_in_accounts' => 1,
                    ...
                    $log_title = 43; // Payment    
                    $acc_log = "Transferred <strong>\${$inv_pay_row->amount_paid}</strong> to <a href='{$this->config->item("crmci_link")}/jobs/details/{$transfer_job_id}'>{$transfer_job_id}</a>, because <strong>{$remove_invoice_reason}</strong>. Ref: <a href='".BASEURL."accounts/agency_payments/?agency_payments_id={$agency_payments_id}&open_edit_lb=1'>{$payment_reference}</a>";                        
                    $log_params = array(
                        'title' => $log_title,
                        'details' => $acc_log,
                        'display_in_accounts' => 1,
