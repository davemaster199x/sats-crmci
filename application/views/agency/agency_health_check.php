                ...
                <div class="row" style="margin-right: 0px;">
                    ...
                    <div class="table_top_head text-left text-center">Snapshot</div>
                        <table class="table table-hover main-table kpi_snapshot_table">
                            ...
                            <tr>
                                <td><strong>Joined <?=$this->config->item('company_name_short')?></strong></td>
                        ...
                        if( $list->num_rows() > 0 ){
                            ...
                            foreach ($list->result_array() as $cr) {     
                                ...
                                <tr>                                
                                    ...
                                    <td>                         
                                        <a href="/properties/details/?id=<?php echo $cr['property_id']; ?>">
                                            <?php echo $full_prop_address; ?>
                                        </a> 
                                    </td>
                                    <td>
...
<div id="about_page_fb" class="fancybox" style="display:none;" >
	...
	<p>This page show all data for agency.</p>

    <h3>Total Property Query</h3>
    <p><?= $about_query['total_property'] ?></p>

    <h3>Joined <?= $this->config->item('company_name_short') ?> Query</h3>
    <p><?= $about_query['joined_sats'] ?></p>

    <h3>API's Query</h3>
    <p><?= $about_query['api'] ?></p>

    <h3>New Properties (Last 30 days) Query</h3>
    <p><?= $about_query['new_properties'] ?></p>

    <h3>Deactivated Properties (Last 30 days) Query</h3>
    <p><?= $about_query['deactivate_properties'] ?></p>

    <h3>Jobs Send Letters Query</h3>
    <p><?= $about_query['jobs_send_letters'] ?></p>

    <h3>Jobs Booked Query</h3>
    <p><?= $about_query['jobs_booked'] ?></p>
    
    <h3>Jobs OnHold Query</h3>
    <p><?= $about_query['jobs_on_hold'] ?></p>

    <h3>Jobs Escalate Query</h3>
    <p><?= $about_query['jobs_escalate'] ?></p>

    <h3>Jobs Allocate Query</h3>
    <p><?= $about_query['jobs_allocate'] ?></p>

    <h3>Jobs TBB Query</h3>
    <p><?= $about_query['jobs_to_be_booked'] ?></p>

    <h3>Jobs Merged Query</h3>
    <p><?= $about_query['jobs_merged'] ?></p>

    <h3>Completed (Last 30 days) Query</h3>
    <p><?= $about_query['jobs_completed'] ?></p>

    <h3>Missed Jobs (Last 30 days) Query</h3>
    <p><?= $about_query['jobs_missed'] ?></p>

    <h3>QLD - Failed Jobs (Last 30 days) Query</h3>
    <p><?= $about_query['jobs_failed_qld'] ?></p>

    <div><h3>Days to Complete</h3><span>(change BETWEEN number for eg.BETWEEN 31 AND 60)</span></div>
    <p><pre>
