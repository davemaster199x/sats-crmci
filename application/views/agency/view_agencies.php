<style>
    .col-mdd-3{max-width:15.5%;}
    .mailer_dl_div{margin-right:10px;}
    code{line-height:}
</style>
...
?>
	...
	<header class="box-typical-header">
        ...
        echo form_open('/agency/view_agencies',$form_attr);
        ?>
                ...
                <div class="col-md-9 columns">
                    <div class="row">
                        ...
                        <div class="col-mdd-3">
                            ...
                            <select id="agency_ht_filter" name="agency_ht_filter" class="form-control field_g2">
                                <option value="" <?php echo ($this->input->post('agency_ht_filter') == "") ? "selected" : ""; ?>>ALL</option>
                                <option value="0" <?php echo ($this->input->post('agency_ht_filter') === "0") ? "selected" : ""; ?>>Regular</option>
                                <option value="1" <?php echo ($this->input->post('agency_ht_filter') === "1") ? "selected" : ""; ?>>HT</option>
                                <option value="2" <?php echo ($this->input->post('agency_ht_filter') === "2") ? "selected" : ""; ?>>VIP</option>
                                <option value="3" <?php echo ($this->input->post('agency_ht_filter') === "3") ? "selected" : ""; ?>>HWC</option>
                            </select>
                            <div class="mini_loader"></div>
                        </div>  
                        <div class="col-mdd-3">
                            ...
                            <select id="sales_rep_filter" name="sales_rep_filter" class="form-control field_g2">
                                ...
                                <?php 
                                    foreach($salesrep->result_array() as $salesrep_row){
                                        $selected = ($this->input->get_post('sales_rep_filter')==$salesrep_row['StaffID'])?'selected':'';
                                ?>
                                    <option <?php echo $selected; ?> value="<?php echo $salesrep_row['StaffID'] ?>"><?php echo "{$salesrep_row['FirstName']} {$salesrep_row['LastName']}" ?></option>
                                <?php
                                    }
                                ?>
                            </select>
                        ...
                        <div class="col-mdd-3">
                            <label for="ht_select">Source of Client</label><span>
                            <select id="source_of_client_filter" name="source_of_client_filter" class="form-control field_g2">
                                <option value="">---</option>
                                <?php
                                foreach( $source_of_client_filter->result() as $source_of_client_row ){ ?>
                                    <option 
                                        value="<?php echo $source_of_client_row->sac_id; ?>" 
                                        <?php echo ( $this->input->get_post('source_of_client_filter') == $source_of_client_row->sac_id )?"selected":null; ?>
                                    >
                                        <?php echo $source_of_client_row->company_name; ?>
                                    </option>
                                <?php
                                }
                                ?>
			...
			<div class="table-responsive">
				<table class="table table-hover main-table">
					<thead>
						<tr>
                            <th>Agency Name</th>
                            <th><?php echo ($this->config->item('country')==1) ? 'ABN' : 'NZBN'; ?> Number</th>
                            <th>Phone</th>
                            <th>Contact</th>
                            <th>Sales Rep</th>
                        ...
                        ?>
                            <tr>
                                <td class="<?php echo ( $row['priority'] > 0 )?'j_bold':null; ?>">
                                    <?php
                                    $agency_name_str = "{$row['a_name']} ".( ( $row['priority'] > 0 )?' ('.$row['abbreviation'].')':null );
                                    echo $this->gherxlib->crmLink('vad',$row['a_id'],$agency_name_str);
                                    ?>
                                </td>
                                ...
                                <td>
                                    <?php
                                       ...
                                       // echo ($this->system_model->isDateNotEmpty($lastContact_query['eventdate']))?$this->system_model->formatDate($lastContact_query['eventdate'],'d/m/Y'):NULL;
                                       echo ($this->system_model->isDateNotEmpty($row['last_contact']) && !empty($row['last_contact']))?$this->system_model->formatDate($row['last_contact'],'d/m/Y'):NULL;
                                  ?>
                                </td>
                                    <td><?php echo $this->system_model->isDateNotEmpty($row['joined_sats']) ? $this->system_model->formatDate($row['joined_sats'],'d/m/y') : NULL;  ?></td>
                                <td align='center'><?php echo $row['active_prop'] ?></td>
                                <td align='center'><?php echo $row['tot_properties']; ?></td>
                            </tr>
                         
                        <?php
                                $active_prop_tot += $row['active_prop'];
                                $prop_tot += intval($row['tot_properties']);                               

                            }
                            }else{
                                echo "<tr><td colspan='9'>There are no active agencies that match your search.</td></tr>";
                            }
                       
                        ?>
                           <tr>
                                <td align='center' colspan="9"><b>Total</b></td>
                                <td align='center'><b><?php echo $active_prop_tot; ?></b></td>
...
<div id="about_page_fb" class="fancybox" style="display:none;" >
	...
	<p>This page displays all active agencies.</p>
    <pre>
