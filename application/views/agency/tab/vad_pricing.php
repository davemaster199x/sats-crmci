                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                ...
                                &nbsp;&nbsp;<?=Alarm_job_type_model::icons($service_row['id']);?>
                            </td>
                            <td>
                                <?php                    
                ...
                <div class="row">
                    ...
                    ?>
                        <section class="card card-blue-fill">
                            <header class="card-header">Alarms</header>
                            <div class="card-block">
                                <?php if(config_item('theme')=='sas'): ?>
                                <div class="text-center form-group">
                                    <button type="button" class="btn btn-primary-outline btn_insert_sa_alarms btn_autofill_alarms" onclick="insert_or_clear_qld_or_sa_alarms(this,this.value, 'edit')" value="2">Insert SA Alarms</button>
                                    <button type="button" class="btn btn-primary-outline btn_insert_qld_alarms btn_autofill_alarms" onclick="insert_or_clear_qld_or_sa_alarms(this,this.value, 'edit')" value="1">Insert QLD Alarms</button>
                                </div>
                                <?php endif; ?>
                                <table class="table table-hover main-table vad_pricing_table text-left table-no-border">
                                    <thead>
                                    ...
                                    <tbody>
                                        <?php 
                                            ...
                                            foreach($alarms as $alarm){ 
                                                ...
                                                $agencySelected = (!empty($alarm['agency_alarm_id']))?true:false;      
                                        ?>
                                            <tr class="alarms_tr tr_main_tt <?php echo ($agencySelected==true)?null:'fadedText'; ?> <?php echo ( $is_alarm_240v_rf == true )?'alarm_240v_rf_row':null; ?> <?php echo ($is_alarm_240v_rf==true || $is_alarm_240vRFCavf==true) ? 'is_alarm_req_for_quotes_row' : NULL ; ?>" value="<?php echo $index2; ?>">
                                                ...
                                                <td>
                                                    <div class="checkbox">
                                                        <input type="checkbox" id="<?php echo $index2.'_check2' ?>" name="agency_alarm_approve[]" class="approve agency_alarm_approve" <?php echo ($agencySelected==true)?'checked="checked"':''; ?> value="<?php echo $index2; ?>" onchange="check_alarm_make('<?=$alarm['alarm_make']?>',<?=$index2?>)" />	
                                                        <label for="<?php echo $index2.'_check2' ?>">&nbsp;</label>
                                                    </div>	
                                                    <input type="hidden" name="alarm_id[]" class="alarm_id hid_alarm_pwr_id" value="<?php echo $alarm['alarm_pwr_id']; ?>" />
                    ...
                    <div class="col-md-4">
                        ...
                        <section class="card card-blue-fill">
                            <header class="card-header">Instructions</header>
                            <div class="card-block">
                                <p>
                                    Alarms must be approved and have a price to be added to the agency.
                                </p>
                            </div>
                            <header class="card-header"><?php echo ($this->config->item('country')==1) ? $row['state'] : 'New Zealand' ?> Alarm Guide</header>
                            <div class="card-block">
                                 <?php 
                ...
                <div class="col">          
                    <table class="table table-hover">
                        ...
                        foreach( $apv_sql->result() as $apv_row ){ ?>
                            <tr 
                                class="
                                    <?php echo (  $this->system_model->isDateNotEmpty($apv_row->expiry) && $apv_row->expiry < date('Y-m-d') )?'apv_expired':null; ?>
                                    <?php echo (  $apv_row->apv_type == 2 )?'apv_type_surcharge':null; ?>
                                "
                            >
                                ...
                                <td>
                                    <?php
                                    // check if variation is being used on property
                                    $today = date('Y-m-d');
                                    ...
                                    AND (
                                        pie.`exclude_until` < '{$today}' OR 
                                        pie.`exclude_until` IS NULL
                                    )

                                    AND apv.`id` = {$apv_row->apv_id} 
                                    AND apv.`active` = 1
                                    AND apv.`scope` = 1  
                                    AND (
                                        apv.`expiry` >= '{$today}' OR
            ...
            <div class="col-md-12">
                <div class="card-block">
                    <table id="prop_variation" class="display table table-striped table-bordered" cellspacing="0" width="100%">
                        <thead>
                            <tr>
                                <th id="prop_var_address_th">Address</th>
                                <th>Type</th>
                                <th>Amount</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            ?>
                            <tr>
                                <td><a href="/properties/details/?id=<?php echo $var_row->property_id ?>"><?=$prop_address; ?></a></td>
                                <td><?php echo ( $var_row->apv_type == 1 )?'Discount':'Surcharge'; ?></td>
                                <td>$<?php echo number_format($var_row->apv_amount,2); ?></td>
                            </tr>
                            <?php } ?>
                        </tbody>
                    </table>
                ...
                <tr>
                    ...
                    <td>
                        <select id="apv_scope" class="form-control" required>
                            ...
                            <?php
                            // get active services by agency
                            $ajt_sql = $this->db->query("
                            SELECT 
                                ajt.`id`,
                                ajt.`type`
                            FROM `agency_services` AS agency_serv
                            LEFT JOIN `alarm_job_type` AS ajt ON agency_serv.`service_id` = ajt.`id`
                            WHERE agency_serv.`agency_id` = {$agency_id}
                            AND ajt.`active` = 1
...
<?php if(config_item('theme') == 'sas'): ?>
    <script type="text/javascript" src="<?= base_url('/inc/js/sas_auto_fill_alarm.js') ?>"></script>
<?php endif; ?>
...
<script type="text/javascript">
    ...
    function clear_add_edit_variation_lightbox(){
        ...
        lightbox.find("#agency_price_variation_amount").prop("disabled",false);
        ...
        $('.btn-update').click(function(){
           ...
           */
            ...
            jQuery(".apv_edit").click(function(){
                ...
                lightbox.find("#agency_price_variation_amount").prop("disabled",true);
    ...
    function check_alarm_make(alarm_make,index2){
        if (alarm_make === "Cavius") {
            var checkbox = document.getElementById(index2+"_check2");
