            ...
            <select name="keys_agency" id="keys_agency" class="form-control">
                ...
                foreach( $agency_keys_sql->result() as $agency_keys_row ){
                    ...
                    if( $agency_keys_row->agency_id > 0 ){ ?>
                        ...
                        AND agen_add.`type` = 2");
                        $key_add_num = 1;
                        $check_address_sql = $this->db->query("SELECT `agency_addresses`.`id`, a.`agency_id`, a.`agency_name`, agency_addresses.`address_1` AS agen_add_street_num, agency_addresses.`address_2` AS agen_add_street_name, agency_addresses.`address_3` AS agen_add_suburb  FROM `agency_addresses` JOIN `property_keys` ON `agency_addresses`.`id`=`property_keys`.`agency_addresses_id` JOIN `agency` AS a ON agency_addresses.`agency_id` = a.`agency_id` WHERE a.`agency_id`={$agency_keys_row->agency_id}");
                        $check_address_row = $check_address_sql->row();
                        if($check_address_row > 0) {
                            ...
                            echo "<option value='$agency_add_row->agency_id' data-agency_addresses_id='$check_address_row->id'>{$check_address_row->agency_name} Key #{$key_add_num} {$agen_add_comb}</option>";
                        } else {
							...
							<?php
								...
								if( $row['row_id_type'] == 'job_id' ){
										...
										if( $showRow ==1 ){
											...
											// job status
											$status_td = "
											<a href='{$this->config->item('crmci_link')}/jobs/details/{$row['jid']}'>{$jstatus_txt}</a>
											";
											...
											// job row view here
                                            $this->load->view('tech_run/tds_admin_map_row_list', $row_data_arr);
...
<?php
/*
echo "key_access_jobs_arr: <br />";
echo "<pre>";
print_r($key_access_jobs_arr);
echo "</pre>";

echo "<hr />";

echo "key_pickup_arr: <br />";
echo "<pre>";
print_r($key_pickup_arr);
echo "</pre>";
*/
...
echo "jobs_no_key_pickup_arr: <br />";
echo "<pre>";
print_r($jobs_no_key_pickup_arr);
echo "</pre>";
*/
