...
?>
    ...
    <div id="map_functions">
        ...
        <span class="float-right">
            ...
            <?php
            if( $tech_run_row['run_complete'] != 1 ){ // not run mapped 

                if( $this->input->get_post('show_booked_only') == 1 ){
                    $show_booked_only_dynamic_btn = 'Show ALL jobs';
                    $show_booked_only_link = "/tech_run/map/?tr_id={$tech_run_row['tech_run_id']}";
                }else{
                    $show_booked_only_dynamic_btn = 'Show Booked Only';
                    $show_booked_only_link = "/tech_run/map/?tr_id={$tech_run_row['tech_run_id']}&show_booked_only=1";
                }                        
            ?>
                <a href="<?php echo $show_booked_only_link; ?>">
                    <button type="button" class="btn btn-warning" id="show_booked_only_btn"><?php echo $show_booked_only_dynamic_btn; ?></button>
                </a>
            <?php
            }
            ?>            
								...
								if( $row['row_id_type'] == 'job_id' ){
                                        ...
                                        */
										...
										if( $showRow ==1 ){
                                            ...
                                            //add start and end date tooltip for status hover
                                            $start_date_tooltip =  ($row2['start_date'] != '') ? $this->system_model->formatDate($row2['start_date'], 'd/m/Y') : 'N/A';
											$end_date_tooltip = ($row2['due_date'] != '') ? $this->system_model->formatDate($row2['due_date'], 'd/m/Y') : 'N/A';
											$status_td = "
											<a data-toggle='tooltip' title='{$start_date_tooltip} - {$end_date_tooltip}' href='{$this->config->item('crmci_link')}/jobs/details/{$row['jid']}'>{$jstatus_txt}</a>
											";
											...
											if( $row2['key_access_required'] == 1 && $row2['j_status']=='Booked' ){
                                                ...
                                                // capture key access jobs
                                                $key_access_jobs_arr[] = $row2;
											...
											$address_td = "
												<a href='/properties/details/?id={$row2['property_id']}'>
													{$paddress}
												</a>
											";
                                            ...
                                            */
                                            ...
                                            // end date is today and any that are due in the next five days
                                            if( $row2['j_status'] == 'To Be Booked' ){

                                                if(                                                  
                                                    $this->system_model->isDateNotEmpty($row2['due_date']) && 
                                                    date('Y-m-d') >= date('Y-m-d',strtotime("{$row2['due_date']} -5 days"))                                                    
                                                ){
                                                    $red_map_pin = 1;
                                                }
    
                                                // Any job 90 days or more needs a red pin
                                                if( $this->system_model->isDateNotEmpty($row2['created']) ){
                                                    
                                                    $date1 = date_create(date('Y-m-d', strtotime($row2['created'])));
                                                ...
                                                // FR - regardless of date
                                                if( $row2['job_type'] == 'Fix or Replace' ){
                                                    $red_map_pin = 1;
                                                }
    
                                                // NSW Retest date MINUS 7 days 
                                                if( 
                                                    $row2['p_state'] == 'NSW' && 
                                                    (
                                                        $this->system_model->isDateNotEmpty($row2['retest_date']) &&
                                                        date('Y-m-d') >= date('Y-m-d',strtotime("{$row2['retest_date']} -7 days"))
                                                    ) 
                                                ){
                                                    $red_map_pin = 1;
                                                }
                                            ...
                                            $prop_address[] = array(
                                                ...
                                                'highlight_color' => $row['hex'],
                                                'lat' => $row2['p_lat'],
                                                'lng' => $row2['p_lng'],
                                                'trr_id' => $row['tech_run_rows_id'],
                                                'red_map_pin' => $red_map_pin
                                            );
...
<?php
$jobs_no_key_pickup_arr = [];
// loop through all booked key access jobs 
foreach( $key_access_jobs_arr as $key_access_jobs ){

    $has_key_pick_up = false;

    // loop through all key pick ups
    foreach( $key_pickup_arr as $key_pickup ){

        if( $key_access_jobs['agency_id'] == $key_pickup['agency_id'] ){

            $has_key_pick_up = true;

        }

    }

    if( $has_key_pick_up == false ){
        $jobs_no_key_pickup_arr[] = $key_access_jobs;

        // check if agency has extra address
        $agency_add_sql = $this->db->query("
        SELECT COUNT(agen_add.`id`) AS 	agen_add_count	
        FROM `agency_addresses` AS agen_add
        LEFT JOIN `agency` AS a ON agen_add.`agency_id` = a.`agency_id`
        WHERE agen_add.`agency_id` = {$key_access_jobs['agency_id']}
        AND agen_add.`type` = 2
        ");        

        if( $agency_add_sql->row()->agen_add_count > 0 ){ // has extra address

            $agency_with_extra_address_arr[] = $key_access_jobs['agency_name'];

        }else{ // no extra address

            $jobs_no_key_pickup_agency_only_arr[] = $key_access_jobs['agency_id']; // just need agency ID here
...
<div id="about_page_fb" class="fancybox" style="display:none;" >
	...
	<table class="table">
        ...
        <tr>
            <td><img src="/images/google_map/key-and-supplier.png"></td>
        ...
        <tr>
            ...
            <td>- Job is expired or nearing its expiry date</td>
        </tr>
        <tr>
            <td><img src="/images/google_map/pin-blue.png"></td>
            <td>- Job is "Booked"</td>
        </tr>
        <tr>
...
<script>
    ...
    function get_marker_icon(image){
        ...
        var icon = {
            url: image,
            labelOrigin: new google.maps.Point(23.5,18.5)
        };
    ...
    */
    ...
    // get marker icon
    function get_marker_icon(marker_icon_obj){

        // custom icon
        var icon = {
            url: marker_icon_obj.image,
            labelOrigin: new google.maps.Point(marker_icon_obj.pin_num_coor.x,marker_icon_obj.pin_num_coor.y)
        };
    ...
    // add markers
    function add_marker(position,popupcontent,icon,trr_id,prop_index,hide_label=false,is_job,font_size){
    ...
    function display_marker(address_lat_lng,address_obj,prop_index){
        ...
        var is_job = false;
        var image = '';
        var pin_num_coor_x = 20;
        var pin_num_coor_y = 16;
        var font_size = '12px';
        ...
        if( parseInt(address_obj['is_accomodation']) == 1 ){ // accomodation
            ...
            image = '<?php echo $this->config->item('crmci_link'); ?>/images/google_map/key-and-supplier.png';

            // re-center map pin number
            pin_num_coor_x = 19.5;
            pin_num_coor_y = 12;

            // resize pin font size
            font_size = '9px';
            ...
            if(address_obj['status'] == 'Booked'){
                image = '<?php echo $this->config->item('crmci_link'); ?>/images/google_map/pin-blue.png';
            }else if( address_obj['red_map_pin'] == 1 || parseInt(address_obj['urgent_job']) == 1 || address_obj['status'] == 'Allocate' ){

                image = '<?php echo $this->config->item('crmci_link'); ?>/images/google_map/pin-red-bigger.png';
                ...
                address_obj['status'] == 'Pre Completion' ||  
                address_obj['status'] == 'Merged Certificates' ||  
                address_obj['status'] == 'Completed' 
            ){
        ...
        var icon = get_marker_icon(marker_icon_obj);

        // add markers
        add_marker(address_lat_lng,address_obj['address'],icon,address_obj['trr_id'],prop_index,hide_marker_label,is_job,font_size);
    ...
    jQuery(document).ready(function(){
        ...
        <?php
        // alert booked key access jobs with no key pick ups
        if( count($jobs_no_key_pickup_arr) > 0 ){ 
            
            $error_msg = "Key Pick up Required for: <br /><br />";
            foreach( $jobs_no_key_pickup_arr as $jobs_no_key ){
                $error_msg .= "{$jobs_no_key['p_address_1']} {$jobs_no_key['p_address_2']}, {$jobs_no_key['p_address_3']}<br />";
            }
            ...
            setTimeout(function(){ 
				...
				function(isConfirm) {
					...
					if (isConfirm) {	                    
                        ...
                        var tr_id = <?php echo $this->input->get_post('tr_id'); ?>;
                        var tech_id = <?php echo $tech_id; ?>;
                        var date = '<?php echo $date; ?>';

                        // convert PHP address array to js array
                        var jobs_no_key_pickup_agency_only_enc_js = <?php echo json_encode(array_unique($jobs_no_key_pickup_agency_only_arr)); ?>;                        

                        <?php                                                
                        if( count($agency_with_extra_address_arr) > 0 ){ 
                            $agency_with_extra_address_imp = implode(", ",array_unique($agency_with_extra_address_arr));
                        ?>

                            if( confirm('<?php echo addslashes($agency_with_extra_address_imp); ?> has multiple key addresses and will be skipped, these needs to be selected manually. Do you still want to proceed the others?') ){

                                // add multiple agency keys
                                jQuery("#load-screen").show();
                                jQuery.ajax({
                                    type: "POST",
                                    url: "/tech_run/ajax_add_agency_keys_multiple",
                                    data: {
                                        tr_id: tr_id,
                                        tech_id: tech_id,
                                        date: date,
                                        keys_agency_arr: jobs_no_key_pickup_agency_only_enc_js
                                    }
                                }).done(function( ret ){
