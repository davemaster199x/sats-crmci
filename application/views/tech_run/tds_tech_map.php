                        ...
                        if(!empty($jr_list2)) {
                            foreach ($jr_list2->result_array() as $row) {
                                ...
                                if ($row['row_id_type'] == 'job_id') {
                                    $jr_sql = $this->tech_model->getJobRowData(
                                        $row['row_id'],
                                    ...
                                    if ($showRow == 1) {
                                        ...
                                        $job_log_time = date(
                                            "Y-m-d H:i",
                                            strtotime("{$chk_log['eventdate']} {$chk_log['eventtime']}:00")
                                        );
                                        ...
                                        if ($this->config->item('country') == 1) { // preferred alarm, AU only
                                            ...
                                            if ($row2['p_state'] == 'QLD') { // QLD only
                                                ...
                                                $use_short_name = true;
                                                $cavi_orca_td = $this->system_model->display_free_emerald_or_paid_brooks(
                                                    $row2['agency_id'],
                                                    $use_short_name
                                                );
                                            }
                                        } else {
                                            if ($this->config->item('country') == 2) { // cavi/orca alarms, NZ only
                                                $cavi_orca_td = $this->system_model->display_orca_or_cavi_alarms(
                                                    $row2['agency_id']
                                                );
                                            }
                                        }
                                    ...
                                    if ($row['row_id_type'] == 'keys_id') {
                                        ...
                                        $nobk = $this->tech_model->getNumberOfBookedKeys(
                                            $tech_id,
                                            $date,
                                            $this->config->item('country'),
                                            $kr['agency_id']
                                        ...
                                        if ($nobk > 0 || in_array($kr['agency_id'], $fn_agency_sub) || in_array(
                                                $kr['agency_id'],
                                                $vision_agency_sub
                                            )) { // only show agency keys, that has remaining booked keys
                                            ...
                                            // agency
                                            $agent_td = $agent_td = "
                                        <a href='javascript:void(0);' class='agency_name_link'>" . str_replace(
                                                    '*do not use*',
                                                    '',
                                                    $kr['agency_name']
                                                ) . "</a>
                                        ...
                                        if ($row['row_id_type'] == 'supplier_id') {
                                            // supplier
