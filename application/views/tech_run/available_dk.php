...
<div class="box-typical box-typical-padding">
	...
	$bc_items = array(
		...
		array(
			'title' => "{$title}",
			'status' => 'active',
			'link' => "/tech_run/available_dk/?tr_id={$this->input->get_post('tr_id')}"
		)
	);
				...
				*/
				...
				if( $showRow ==1 ){
					...
					*/
					...
					$address_td = "<a href='http://maps.google.com/?q={$paddress}' target='_blank'>{$paddress}</a>";
					...
					// job row view here
					$this->load->view('tech_run/available_dk_row_list', $row_data_arr);
...
jQuery(document).ready(function(){
	...
	jQuery("#complete_btn").click(function(){
		...
		var old_ts_link = parent_div.find("#old_ts_link").val();


		swal({
			title: "Warning!",
			text: "This will book job for door knock. Do you want to continue?",
			type: "warning",
		...
		function(isConfirm) {
			...
			if (isConfirm) {
				...
				jQuery.ajax({
					...
					swal({
						title: "Success!",
						text: "Complete Process Successful",
						type: "success",
						confirmButtonClass: "btn-success",
					...
					setTimeout(function(){
						...
						console.log("old_ts_link: "+old_ts_link);
						window.location=old_ts_link;
