...
<div class="box-typical box-typical-padding">
	...
	<header class="box-typical-header">
		...
		<div class="for-groupss row quickLinksDiv2 mt-4 mb-1">
	...
	<section>
		...
		<h4 class="mt-3">All figures on this page are EX GST</h4>
					...
					<table class="table table-hover main-table">
						<thead>
							<tr>    
								...
								<th class="noBorderTop text-center">Working<br />Day</th>
								<th class="noBorderTop jtextalignleft">Day</th>
								<th class="noBorderTop">Total<br />Revenue</th>
								<th class="noBorderTop">Subs<br />Revenue</th>
								<th class="noBorderTop">Non-Subs<br />Revenue</th>
								<th class="noBorderTop">Techs</th>
								<th style="width:170px;" class="noBorderTop">Jobs</th>		
								<th class="noBorderTop">Avg.<br />Jobs</th>
								<th class="noBorderTop">Avg.<br />$ Jobs</th>
								<th class="noBorderTop">MTD<br />Sales</th>	
							</tr>
						</thead>
						...
						<?php
							...
							while( $curr_date <= $end_date ){
								...
								$non_subs_revenue = ($dfpd['sales']-$dfpd['subs_revenue']);
						...
						?>
							...
							<tr class="<?php echo $bgcolorClass; ?>">
									...
									<td>
										<span class="display_elem"><?php echo ( $dfpd['subs_revenue'] > 0 )?'$'.number_format($dfpd['subs_revenue'], 2):null; ?></span>
										<input type="text" class="subs_revenue update_elem form-control" value="<?php echo ($dfpd['subs_revenue']>0)?$dfpd['subs_revenue']:null; ?>" />
									</td>
									<td>
										<span class="display_elem"><?php echo ( $non_subs_revenue > 0 )?'$'.number_format($non_subs_revenue, 2):null; ?></span>
										<input type="text" class="non_subs_revenue update_elem form-control" value="<?php echo ( $non_subs_revenue > 0 )?$non_subs_revenue:null; ?>" />
									</td>
				...
				<div class="col-md-5 columns">
						...
						<div style="text-align: left;" class="instruct_div">	
							<ol>
								...
								<?php 			
									// MAILTO
									$subject_for = "for " . date('d/m/Y',strtotime($date)) . " " . config_item('company_name_short');
                                    if(strpos(config_item('domain'),'smokealarmsolutions.com.au')){
	                                    $subject_for .=  (config_item('country') == 1 ? ' AU' : ' NZ');
                                    }
                                ?>
								<a href="mailto:<?= make_email('figures') . '?Subject=Daily figures ' . $subject_for; ?>">CLICK HERE</a> to email figures
								</li>
								<li>Export from <a href="/jobs/merged_jobs" target="__blank">Merged Certificates</a></li>
								<li>
								<a href="mailto:<?= make_email('accounts') . '?Subject=MYOB Import ' . $subject_for; ?>">CLICK HERE</a> to email Export to Accounts
							</li>
...
jQuery(document).ready(function(){
	...
	jQuery(".btn_fetch_data").click(function(){
		...
		jQuery.ajax({
			...
			obj.parents("tr:first").find(".subs_revenue").val(ret.subs_revenue);
			obj.parents("tr:first").find(".non_subs_revenue").val(ret.non_subs_revenue);
	...
	jQuery(".btn_update").click(function(){
		...
		var subs_revenue = obj.parents("tr:first").find(".subs_revenue").val();
