...
<div style="width:450px;">
    ...
    <?php
    echo form_open('/calendar/add_calendar_entry_static') ?>

    <div class="form-group">
        <input type="hidden" name="cal_id" value="<?php
        echo $row_cal['calendar_id'] ?>">
        <input type="hidden" name="cal_staff_id" value="<?php
        echo $row_cal['staff_id'] ?>">
        <label>Staff *</label>

        <?php
        $staff_id = $row_leave['emp_staff_id'] ?? $row_cal['staff_id'];
        if ($cal_id != "") { // cal_id not null > edit lightbox ?>
            ...
            <select id="staff_id" name="staff_id" class="form-control">
                ...
                foreach ($staff_list->result_array() as $row) {
                    ?>
                    <option <?php
                    echo ($staff_id == $row['StaffID']) ? 'selected' : '' ?> value="<?php
                    echo $row['StaffID'] ?>"><?php
        ...
        <?php
        } else { // cal_id is empty > add lightbox ?>
    ...
    <div class="form-group">
        ...
        <input name="start_date" data-enable-time="true" data-time_24hr="true"
               class="flatpickr form-control flatpickr-input" data-allow-input="true" type="text" value="<?php
               $start_date = (isset($leave_id)) ? $this->system_model->formatDate($row_leave['lday_of_work'], 'd/m/Y') : $start_date_data;
               echo $start_date; ?>">
    </div>

    <div class="form-group">
        <label>Finish Date</label>
        <input name="finish_date" data-enable-time="true" data-time_24hr="true"
               class="flatpickr form-control flatpickr-input" data-allow-input="true" type="text" value="<?php
               $finish_date = (isset($leave_id)) ? $this->system_model->formatDate($row_leave['fday_back'], 'd/m/Y') : $finish_date_data;
               echo $finish_date ?>">
    </div>
    ...
    if ($add == 1) {
        ...
        if ($class_id == 2) {
            ...
            <div class="form-group" id="div_leave" style="display: none;">
                ...
                <input type="text" class="form-control" name="leave_type" value="<?php
                echo isset($leave_id) ? $type_of_leave[0]->leave_name : $row_cal['region'] ?>">
            </div>
        <?php
        } else { ?>
            <div class="form-group" id="div_leave" style="display: none;">
                ...
                <select name="leave_type" id="leave_type" class="form-control" onchange="other_leave_type(this.value)">
                    <option <?php
                    if ($row_cal['region'] == 'Sick') {
                        echo 'selected';
                    } ?> value="Sick">Sick
                    </option>
                    <option <?php
                    if ($row_cal['region'] == 'Annual Leave') {
                        echo 'selected';
                    } ?> value="Annual Leave">Annual Leave
                    </option>
                    <option <?php
                    if ($row_cal['region'] == 'LWOP') {
                        echo 'selected';
                    } ?> value="LWOP">LWOP
                    </option>
                    <option <?php
                    if ($row_cal['region'] == 'Toil') {
                        echo 'selected';
                    } ?> value="Toil">Toil
                    </option>
                    <option <?php
                    if ($row_cal['region'] == 'Bday Leave') {
                        echo 'selected';
                    } ?> value="Bday Leave">Bday Leave
                    </option>
                    <option <?php
                    if ($row_cal['region'] == 'WorkCover') {
                        echo 'selected';
                    } ?> value="WorkCover">WorkCover
                    </option>
                    <option <?php
                    if ($row_cal['region'] == 'Other') {
                        echo 'selected';
                    } ?> value="Other">Other
                    </option>
                    <option <?php
                    if ($row_cal['region'] != 'Sick' || $row_cal['region'] != 'Annual Leave' || $row_cal['region'] != 'LWOP' || $row_cal['region'] != 'Toil' || $row_cal['region'] != 'Bday Leave' || $row_cal['region'] != 'WorkCover' || $row_cal['region'] != 'Other') {
                        echo 'selected';
                    } ?> value="<?php
                    echo $row_cal['region']; ?>"><?php
                        echo $row_cal['region']; ?></option>
                </select><br>
                <input type="text" style="display: none;" class="form-control" id="leave_type_other"
                       name="leave_type_other" value="">
            </div>
        <?php
        }
    } else {
    ...
    <div class="form-group">
        <label style="display:inline-block;"><input type="checkbox" onchange="check_leave()" name="marked_as_leave" value="1" <?= (isset($leave_id) || $row_cal['marked_as_leave'] == 1) ? 'checked' : '' ?> >&nbsp;Leave</label>
    </div>
    ...
    <div class="form-group">
        ...
        <select name="booking_staff" id="booking_staff" class="form-control">
            ...
            foreach ($staff_list->result_array() as $row) {
                ...
                <option <?php
                echo ($row_cal['booking_staff'] == $row['StaffID']) ? 'selected' : '' ?> value="<?php
                echo $row['StaffID'] ?>"><?php
                    echo "{$row['FirstName']} {$row['LastName']}" ?></option>
                <?php
            }
    ...
    <div class="form-group">
        ...
        <textarea name="details" class="form-control"><?php
            echo isset($leave_id) ? $row_leave['reason_for_leave'] : $row_cal['details'] ?></textarea>
    </div>


    <h5 style="margin:0;">Accommodation</h5>
    <div class="form-group">
        <label><input <?php
            echo ($row_cal['accomodation'] == "" || $row_cal['accomodation'] === null) ? 'checked="checked"' : '' ?>
                    name="accomodation" type="radio" value="">&nbsp;None</label>
        <label><input <?php
            echo ($row_cal['accomodation'] == '0') ? 'checked="checked"' : '' ?> name="accomodation" type="radio"
                                                                                 value="0">&nbsp;Required</label>
        <label><input <?php
            echo ($row_cal['accomodation'] == 2) ? 'checked="checked"' : '' ?> name="accomodation" type="radio"
                                                                               value="2">&nbsp;Pending</label>
        <label><input <?php
            echo ($row_cal['accomodation'] == 1) ? 'checked="checked"' : '' ?> name="accomodation" type="radio"
                                                                               value="1">&nbsp;Booked</label>
    </div>
    ...
    <div class="form-group accomodation_drop" style="display:none;">
        ...
        <select name="accomodation_id" class="form-control">
            ...
            foreach ($accomodation_query->result_array() as $row) {
                ?>
                <option <?php
                echo ($row['accomodation_id'] == $row_cal['accomodation_id']) ? 'selected="true"' : ''; ?> value="<?php
                echo $row['accomodation_id'] ?>"><?php
    ...
    <div class="form-group">
        ...
        if ($cal_id) {
            ?>
            <button id="edit_entry" type="button" name="update_event" class="btn float-right">Update</button>

            <button id="delete_entry" type="button" name="delete_entry" class="btn btn-danger float-left">Delete</button>
            <?php
        } else {
            ?>
            ...
            if (new_finish_date < new_start_date) {
                ...
                jQuery.ajax({
                    ...
                    if (ret.status) {
                        ...
                        setTimeout(function () {
                            if (window.location.href.match(/view_tech_calendar/)) {
                                //$('#load-screen').show();
