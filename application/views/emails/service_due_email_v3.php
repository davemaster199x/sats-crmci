...
<?php if( $agency_api_tokens_q->num_rows() > 0 ): ?> 
    ...
    <?php if ($agency_auto_renew === 1 && $subscription_billing === 1 && $apis->active === 1 || $agency_state === 'NSW' ): ?>
        <p>When you process the properties due for subscription renewal, IF we identify that a property needs to be serviced within the next 60 days (to meet legislative requirements) we will send the job straight to booking (previously this job was ‘on hold’ until the beginning of the following month).</p>
    <?php endif; ?>
...
<?php else: ## Agency is NOT connected to any API ?>
    ...
    <?php if( $agency_state == 'NSW' ): ?>
    <p>When you process the properties due for subscription renewal, IF we identify that a property needs to be serviced within the next 60 days (to meet legislative requirements) we will send the job straight to booking (previously this job was ‘on hold’ until the beginning of the following month).</p>
    <?php endif; ?>

    <?php if( $agency_auto_renew == 1 ):  ?>
    ...
    <?php else: ?>
        <p>You are currently set up to not auto-renew the properties so <?=config_item('company_name_short');?> will not attend the property until instructed to by your agency. <?=config_item('company_name_short');?> will not take any responsibility in fulfilling our obligations to your Landlord because you have asked us to not attend unless instructed.</p>
    <?php endif; ?>
...
<?php if ( ($agency_auto_renew === 0 && $subscription_billing === 1 && $apis->active === 1) || ($agency_auto_renew === 0 && $subscription_billing === 1) ): ?>
<p>
...
<p>Should you have any additional questions, please feel free to contact our freindly Customer Service Team on 1300 41 66 67 or please respond to this email directly.</p>
<?php endif; ?>
