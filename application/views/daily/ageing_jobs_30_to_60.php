<div class="box-typical box-typical-padding">
	...
	$bc_items = array(
		array(
			'title' => $title,
			'status' => 'active',
			'link' => "/jobs/ageing_jobs_30_to_60"
		)
	);
	...
	$export_link_params = '/jobs/ageing_jobs_30_to_60/?export=1&'.http_build_query($export_links_params_arr);
	?>
	...
	<header class="box-typical-header">
		...
		echo form_open('/jobs/ageing_jobs_30_to_60',$form_attr);
		?>
				...
				<div class="col-md-3 columns">
                        <section class="proj-page-section float-right">
                            <div class="proj-page-attach">
                                ...
                                <p class="name">View Jobs 30-60d</p>
                                <p>
				...
				<table class="table table-hover main-table">
					...
					if($lists->num_rows()>0){
						...
						<tr class="<?php echo $row_bg; ?>">
							...
							<td><?=Alarm_job_type_model::icons($list_item['jservice']);?></td>
							<td>
							<?php
...
<div id="about_page_fb" class="fancybox" style="display:none;" >
	...
	<h4>Jobs 30-60 Days</h4>
	<p>
		This page shows all jobs between 30-60 days old
	</p>
	<pre>
...
AND (`j`.`status` = 'To Be Booked' OR `j`.`status` = 'Pre Completion' OR `j`.`status` = 'Booked' OR `j`.`status` = 'Escalate') AND CAST(j.`created` AS DATE) BETWEEN '<?php echo date('Y-m-d', strtotime("-60 days")); ?>' AND '<?php echo date('Y-m-d', strtotime("-30 days")); ?>' AND `p`.`holiday_rental` != 1
			...
			if( error != "" ){
				...
				if( job_id.length > 0 ){
					...
					jQuery.ajax({
						...
						setTimeout(function(){ window.location='/jobs/ageing_jobs_30_to_60'; }, <?php echo $this->config->item('timer') ?>);
