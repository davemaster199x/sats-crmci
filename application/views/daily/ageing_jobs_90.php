<div class="box-typical box-typical-padding">
		...
		<?php 
			...
			$export_link_params = '/jobs/ageing_jobs_90/?export=1&'.http_build_query($export_links_params_arr);
		?>
	...
	<header class="box-typical-header">
		...
		echo form_open('/jobs/ageing_jobs_90',$form_attr);
		?>
				...
				<div class="col-md-9 columns">
					<div class="row">
                            ...
                            <div class="col-mdd-3">
										...
										<select id="agency_filter" name="agency_filter" class="form-control field_g2 select2-photo">
											<option value="">All</option>
										</select>
							</div>
                        ...
                        <?php 
                            ...
                            foreach($lists->result_array() as $row){
                                ...
                                $params = array(
                                    ...
                                    'postcode' => $row['p_postcode'],
                                );
                                $getRegion = $this->system_model->get_postcodes($params)->row();	


                                $row_bg = ($this->system_model->isDateNotEmpty($row['j_date'])?'red_hl':'');
                        ?>
						<tr class="<?php echo $row_bg; ?>">
							<td>
                            <?php 
								echo ($this->system_model->isDateNotEmpty($row['j_date']))?date('d/m/Y', strtotime($row['j_date'])):''; 
							?>
							</td>
                           ...
                           <td>
                            ...
                            <td><?php echo ((int)$list_item['allow_en'] === 1 || (int)$list_item['allow_en'] === 2) ? "YES" : ""; ?></td>
                           ...
                           <td>
                            ...
                            <td>
								<div class="pos-rel">
									<input data-jobid="<?php echo $row['jid'] ?>" class="form-control jcomments" type="text" name="jcomments" value="<?php echo $row['access_notes'] ?>">
							...
							<td>
								<div class="checkbox">
									...
									<input class="chk_job" name="chk_job[]" type="checkbox" id="check-<?php echo $row["jid"] ?>" data-jobid="<?php echo $row["jid"]; ?>" value="<?php echo $row['jid']; ?>">
									<label for="check-<?php echo $row["jid"] ?>">&nbsp;</label>
								</div>
							</td>
...
<div id="about_page_fb" class="fancybox" style="display:none;" >
	...
	<h4>Jobs 90 Days</h4>
	<p>
		This page shows all jobs older than 90 days
	</p>
	<pre>
<!-- <code>SELECT `j`.`id` AS `jid`, `j`.`status` AS `j_status`, `j`.`service` AS `j_service`, `j`.`created` AS `j_created`, `j`.`date` AS `j_date`, `j`.`job_price` AS `j_price`, `j`.`job_type` AS `j_type`, `j`.`preferred_time`, `j`.`out_of_tech_hours`, `j`.`access_notes`, `p`.`property_id`, `p`.`address_1` AS `p_address_1`, `p`.`address_2` AS `p_address_2`, `p`.`address_3` AS `p_address_3`, `p`.`state` AS `p_state`, `p`.`postcode` AS `p_postcode`, `p`.`comments` AS `p_comments`, `p`.`deleted` AS `p_deleted`, `a`.`agency_id` AS `a_id`, `a`.`agency_name` AS `agency_name`, `ajt`.`id` AS `ajt_id`, `ajt`.`type` AS `ajt_type`
FROM `jobs` AS `j`
...
AND `a`.`status` = 'active'
AND `a`.`country_id` = 1
AND (`j`.`status` = 'To Be Booked' OR `j`.`status` = 'Pre Completion' OR `j`.`status` = 'Booked' OR `j`.`status` = 'Escalate') AND CAST(j.`created` AS DATE) < '<?php echo date('Y-m-d', strtotime("-90 days")); ?>' AND `p`.`holiday_rental` != 1
