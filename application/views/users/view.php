...
// if SAS is ticked
if( $user["sas_access"] == 1 ){
	$accessibleCountries[] = 'SAS'; // used the accessibleCountries variable just to add SAS in it
}
...
if(!empty($accessibleCountries) || !empty($accessibleStates)){
	$locationAccess = implode(" | ", array_merge($accessibleCountries, $accessibleStates));
}else{
	$locationAccess = "No Data";
}
...
if (!is_null($user["other_call_centre"]) && $user["other_call_centre"] != "0") {
	...
	$callCentreAgent = "{$callCentre["callCentreFirstName"]} {$callCentre["callCentreLastName"]}";
}
...
<div class="box-typical box-typical-padding">
	...
	<?php
		...
		$bc_items = [
            [
				'title' => $this->config->item('company_name_short')." USERS",
				'status' => 'inactive',
				'link' => "/users/index",
			],
            [
	...
	<div class="tabs-section">
		<div class="tabs-section-nav tabs-section-nav-icons">
			<div class="tbl">
				<ul class="nav" role="tablist">
					...
					<!--
					<li class="nav-item">
						<a class="nav-link <?= $tab == "staff_permissions" ? "active show" : "" ?>" href="/users/view/<?= $user["StaffID"] ?>/staff_permissions">
							<span class="nav-link-in">
								<i class="fa fa-file"></i>
								Permissions
							</span>
		...
		<div class="tab-content">
			...
			elseif (in_array($tab, ["staff_permissions"])):
			...
			<div class="box-typical-body">
					...
					<div class="col-12 text-left">
						<div class="row">
							<div class="col-3">
								...
								<a data-auto-focus="false" data-fancybox data-src="#fancybox-user-class" href="javascript:;">
                                    <?= $user['staff_class']->ClassName; ?>
                                </a>
							</div>
                        ...
                        <section class="widget widget-reports">
                            ...
                            <div class="widget-content">
                                <div class="form-row pt-2">
                                    ...
                                    <div class="col-4 mb-2">
										<label>Receive Sales Report for all staff</label>
                                    </div>
									<div class="col-8 mb-2">
				...
				<div class="row">
					<div class="col">
						<div class="checkbox">
							<input type="checkbox" name="prohibit_web_crm_access" id="prohibit_web_crm_access" <?php echo ( $prohib_using_web_crm == true )?'checked':null; ?> />
							<label for="prohibit_web_crm_access">Prohibit tech from using web CRM</label>
						</div>
					</div>
				</div>
				-->											
				...
				<div class="form-group">
					<div class="form-row align-items-center">
						...
						<div class="col-8">
							...
							<?php endforeach ?>
							<span class="checkbox d-inline-block mb-2 mr-2">								
								<input type="checkbox" name="sas_access" id="sas_access" value="1" <?php echo ( $user["sas_access"] == 1 )?'checked':null; ?> />
								<label for="sas_access">SAS</label>
							</span>
						</div>
					</div>
				</div>
...
<div class="fancybox-form-container" id="fancybox-user-password" style="display:none;">
	<form action="/users/update_password" autocomplete="off" method="post" class="fancybox-form different" data-tables='<?= json_encode(["staff_accounts" => ["_idValue" => $user["StaffID"], "_idField" => "StaffID"]]) ?>'>
		<input type="hidden" name="StaffID" value="<?= $user["StaffID"] ?>">
		<section class="card card-blue-fill">
			<header class="card-header">Change Users' Password</header>
			<div class="card-block">
				<div class="form-group">
                    ...
                    <div class="form-row">
                        <div class="col-12">
                            <div style="font-size: 12px;">Minimum 8 characters</div>
                        </div>
                    </div>
				</div>
...
#fancybox-user-weekly_sales_report{
	width: 42%;
}
.nav .nav-item {
	width: 50%;
...
$('document').ready((event) => {
	...
	$("#fancybox-user-password .fancybox-form").on("submit", (evt) => {
		...
		if (newPassword.length != 0 && newPassword.length < 8) {
			swal({
				title: "Error!",
				text: "New Password field should be at least 8 characters.",
				type: "error",
	...
	$('#form-user').on('submit', function (se) {
		...
		if (newPassword.length != 0 && newPassword.length < 8) {
			alert("New Password field should be at least 8 characters.");
			hasError = true;
		}
...
<?php } ?>
	...
	/*
	// "Prohibit tech from using web CRM" checkbox ajax script
	jQuery("#prohibit_web_crm_access").change(function(){

		var no_web_crm_access_dom = jQuery(this);
		var no_web_crm_access =  ( no_web_crm_access_dom.prop("checked") == true )?1:0;
		var staff_id = <?php echo $user["StaffID"]; ?>;

		$('#load-screen').show();
		jQuery.ajax({
			type: "POST",
			url: "/users/ajax_prohibit_web_crm_access",
			data: { 	
				no_web_crm_access: no_web_crm_access,
				staff_id: staff_id
			}
		}).done(function( ret ){
