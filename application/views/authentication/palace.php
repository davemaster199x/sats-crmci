...
#hide_archived_palace_prop_btn,
#show_archived_palace_prop_btn{
	display:none
}

.palace_archived_row .palaceAdd{
	color: white;
}
.palace_archived_row{
										...
										<div class="btn-group">
											<select id="agency_filter" name="agency_filter" class="form-control">
												...
												foreach ($agenList->result_array() as $row) { ?>
													<option data-no_bulk_match="<?php echo $row['no_bulk_match'] ?>" value="<?=$row['agency_id']?>" <?=isset($selected) ? ($row['agency_id'] == $selected) ? 'selected' : '' : ''?>><?php echo $row['agency_name']." ". ( ( $row['no_bulk_match'] > 0 )?"(Shared) ":null ); ?></option>
												<?php
												}
												?>
											</select>					
				...
				<div class="col-sm-6 pmeClass">							
					...
					<div id="pme_main_div">
						...
						<div class="container-fluid" id="pme_button_div">
							...
							<div class="row">
								<div class="col">
									<button type="button" class="btn btn-primary btn-sm mt-2" id="hide_archived_palace_prop_btn">Hide Deactivated</button> 
									<button type="button" class="btn btn-primary btn-sm mt-2" id="show_archived_palace_prop_btn">Show Deactivated</button> 
									<button type="button" class="btn btn-primary btn-sm mt-2" id="export_palace_prop">Export</button>
...
$(document).ready(function() {
	...
	// export palace properties
	jQuery("#export_palace_prop").click(function(){

		var agency_id = jQuery("#agency_filter").val();

		if( agency_id > 0 ){
			window.location = "/palace/export_properties/?agency_id="+agency_id;
		}		
	...
	var hide_palace_archived_prop = Cookies.get('hide_palace_archived_prop');
	if( hide_palace_archived_prop == 1 ){
		jQuery("#hide_archived_palace_prop_btn").hide();
		jQuery("#show_archived_palace_prop_btn").show();
	}else{
		jQuery("#hide_archived_palace_prop_btn").show();
		jQuery("#show_archived_palace_prop_btn").hide();
	}
	
	// show all hidden api property toggle
	var show_all_hidden_prop = Cookies.get('show_all_hidden_prop_palace');
	...
	jQuery("#pme_table_div").on("click",".pmeAdd",function(){		
		...
		var palace_prop_id = jQuery(this).parents("tr:first").find(".pme_prop_id").val();
						...
						if( ret.length == 0 ){
							...
							if( ret.length > 0 ){
								...
								for( var i=0; i<ret.length; i++ ){
									dup_html += ''+
									'<li>'+
										'<a href="/palace/property/'+ret[i].dup_property_id+'/'+ret[i].dup_agency_id+'" target="_blank">'+
					...
					if( ret[0].dup_prop_deleted == 1 && ret[0].dup_agency_id == agency_id ){
						...
						if( ret.length > 0 ){
							...
							if( ret[0].dup_prop_deleted == 1 ){ //new changes > move nlm property to current if already nlm from other agency
								...
								pme_prop_id_arr2.push(pme_prop_id);	
								...
								swal({
									...
									function(isConfirm) {
										if (isConfirm) {		
											...
											$('#load-screen').show(); 
											$.ajax({
												url: "/palace/ajax_bulk_move_nlm_property",
												type: 'POST',
												data: { 
	...
	jQuery("#agency_filter").change(function(){
		...
		if( agency_id > 0 ){
			...
			jQuery('#load-screen').show(); 
			jQuery.ajax({
				url: "/palace/ajax_bulk_connect_get_crm_list",
				type: 'POST',
				data: { 
					'agency_id': agency_id
				}
				...
				jQuery('#load-screen').show(); 
				jQuery.ajax({
					url: "/palace/ajax_bulk_connect_get_palace_list",
					type: 'POST',
					data: { 
						'agency_id': agency_id,
						'hide_palace_archived_prop': hide_palace_archived_prop,
	...
	$("#btnProbable").click(function() {
		...
		setTimeout(function(){ 
			...
			if( countSelec > 0 ) {
				...
				swal({
					title: "Info!",
					text: "There are no Probable Matches.",
					type: "info",
					confirmButtonClass: "btn-primary"
				});
			}
	...
	$("#btnPossibleMatch").click(function() {
		...
		setTimeout(function(){ 
			...
			crmTable.rows().every(function(index, element) {	
				...
				crm_prop_add_fin = crm_prop_add_fin.replace(/\d+/g, '').trim();
				...
				pmeTable.rows().every(function(index2, element2) {					
					...
					*/
					...
					pme_prop_add_fin = pme_prop_add_fin.replace(/\d+/g, '').trim();
					console.log("pme_prop_add_fin: "+pme_prop_add_fin)
			...
			if( countSelec > 0 ) {
				...
				swal({
					title: "Info!",
					text: "There are no Possible Matches.",
					type: "info",
					confirmButtonClass: "btn-primary"
				});
			}
	...
	$("#bulkCon").click(function() {
		...
		if (
			...
			if (numCon > 0) {
				...
				swal({
					title: "Are you sure?",
					text: "You are about to connect the matched CRM properties and Palace properties below.",
					type: "warning",
					showCancelButton: true,
					confirmButtonClass: "btn-success",
					confirmButtonText: "Yes, Connect it!",
	...
	jQuery("#hide_archived_palace_prop_btn").click(function(){

		Cookies.set('hide_palace_archived_prop', 1);
		location.reload();
	...
	jQuery("#show_archived_palace_prop_btn").click(function(){

		Cookies.set('hide_palace_archived_prop', 0);
		location.reload();
	...
	jQuery("#show_all_hidden_prop_btn").click(function(){
		...
		Cookies.set('show_all_hidden_prop_palace', 1);
		location.reload();
	...
	jQuery("#cancel_show_all_hidden_prop_btn").click(function(){
		...
		Cookies.set('show_all_hidden_prop_palace', 0);
		location.reload();
