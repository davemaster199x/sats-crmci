									...
									<div class="col-sm-8">
										...
										<?php
											...
											if( $this->api_model->if_notes_already_exist_in_pnv($t_params)==true ){
												...
												echo '<button class="btn pnv_verify_button">Property Needs Verification</button>';
											}

										// mark/unmark "Dont Connect API"
										if( $do_not_connect_to_api == true ){ ?>
											<button type="button" id="unmark_dont_connect_api_btn" class="btn btn-success ml-2">Unmark Do Not Connect</button>
											<span><?php echo stripslashes($pccta_row->comment); ?></span>
										<?php
										}else{ ?>
											<button type="button" id="dont_connect_api_btn" class="btn btn-danger ml-2">Don't Connect API</button>
										<?php
										}
										?>
									</div>
								...
								<tbody>
									<?php 
										foreach ($palaceList as $row) { 
			                                ...
			                                if (trim($row->PropertyUnit) != "" && is_string($row->PropertyUnit)) {
			                                    $addUnit = $row->PropertyUnit . "/";
			                                }else {
			                                    $addUnit = "";
                                			}

                                            // if(!is_string($row->PropertyUnit) || !is_string($row->PropertyAddress1) || !is_string($row->PropertyAddress1) || !is_string($row->PropertyAddress2) || !is_string($row->PropertyAddress3) || !is_string($row->PropertyAddress4) || !is_string($row->PropertyPostCode)){
                                            //     log_message('error', 'palace_connect.php | PropertyAddress cannot be converted to a string');
                                            //     $data = [
                                            //         'addUnit' 		   => $row->PropertyUnit,
                                            //         'PropertyAddress1' => $row->PropertyAddress1,
                                            //         'PropertyAddress2' => $row->PropertyAddress2,
                                            //         'PropertyAddress3' => $row->PropertyAddress3,
                                            //         'PropertyAddress4' => $row->PropertyAddress4,
                                            //         'PropertyPostCode' => $row->PropertyPostCode,
                                            //     ];
                                            //     log_message('error', json_encode($data, JSON_PRETTY_PRINT));
                                            // }

											//$fullAdd = $addUnit.$row->PropertyAddress1 . " " . $row->PropertyAddress2 . ", " . $row->PropertyAddress3 . " " . $row->PropertyAddress4 . " " . $row->PropertyPostCode ;
											
											/**
											 * Add validation here > Filter string for address only because for some reason Palace response got weird object value below when address is empty
											 * public 'PropertyAddress1' = object(stdClass)[2206] > this is empty address 1 for example
											 */
											$palaceAddress1 = (is_string($row->PropertyAddress1)) ? $row->PropertyAddress1 : '';
											$palaceAddress2 = (is_string($row->PropertyAddress2)) ? $row->PropertyAddress2 : '';
											$palaceAddress3 = (is_string($row->PropertyAddress3)) ? $row->PropertyAddress3 : '';
											$palaceAddress4 = (is_string($row->PropertyAddress4)) ? $row->PropertyAddress4 : '';
											$palacePostcode = (is_string($row->PropertyPostCode)) ? $row->PropertyPostCode : '';

											$fullAdd = $addUnit.$palaceAddress1 . " " . $palaceAddress2 . ", " . $palaceAddress3 . " " . $palaceAddress4 . " " . $palacePostcode ;
									        ?>
											<tr >
												<td class="api_address_tenant_det_btn"><?=str_replace(',','',$fullAdd)?></td>
												<td style="text-align: center;"><?=$palaceAddress1?></td>
												<td><?=$palaceAddress2?></td>
												<td><?=$palaceAddress3?></td>
												<td style="text-align: center;"><?=$palaceAddress4?></td>
												<td style="text-align: center;"><?=$palacePostcode?></td>
...
jQuery(document).ready(function() {
	...
	// dont connect to API script
	jQuery("#dont_connect_api_btn").click(function(){
		...
		if( property_id > 0 ){
			...
			$swal_html_markup = ""
			+"<div>"
				+"<p>This will mark this property as not allowed to be connected to API? Do you want to continue?</p>"
				+"<input type='text' class='form-control' id='cant_connect_to_api_reason' placeholder='Please enter a reason' />"
			+"</div>";

			swal({
				html:true,
				title: "",
				text: $swal_html_markup,
				type: "warning",						
				showCancelButton: true,
				confirmButtonClass: "btn-success",
			...
			function(isConfirm) {
				...
				if (isConfirm) {							  
					...
					var cant_connect_to_api_reason = jQuery("#cant_connect_to_api_reason").val();

					if( cant_connect_to_api_reason == '' ){
					...
					if( error != '' ){ // error
						...
						jQuery.ajax({
							type: "POST",
							url: "/agency_api/mark_property_cant_connect_to_api",
							data: { 					
								property_id: property_id,
							...
							swal({
								title: "Success!",
								text: "Property marked Successful",
								type: "success",
								confirmButtonClass: "btn-success",
								showConfirmButton: <?php echo $this->config->item('showConfirmButton') ?>,
								timer: <?php echo $this->config->item('timer') ?>
	...
	// dont connect to API script
	jQuery("#unmark_dont_connect_api_btn").click(function(){
		...
		if( property_id > 0 ){
			...
			swal({
				title: "",
				text: 'This will unmark this property of not allowed to be connected to API? Do you want to continue?',
				type: "warning",						
				showCancelButton: true,
				confirmButtonClass: "btn-success",
				confirmButtonText: "Yes, Continue",
			...
			function(isConfirm) {
				...
				if (isConfirm) {							  
					...
					jQuery.ajax({
						type: "POST",
						url: "/agency_api/unmark_property_cant_connect_to_api",
						data: { 					
							property_id: property_id
						}
					}).done(function( ret ){
						...
						swal({
							title: "Success!",
							text: "Property unmarked Successful",
							type: "success",
							confirmButtonClass: "btn-success",
							showConfirmButton: <?php echo $this->config->item('showConfirmButton') ?>,
							timer: <?php echo $this->config->item('timer') ?>
