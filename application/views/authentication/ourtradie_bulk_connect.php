		...
		<div class="body-typical-body">
			<div id="crm_pme_table_div" class="row">
				...
				<div class="col-sm-6 crmClass">				
					...
					<form action="/ourtradie/bulk_connect">
							...
							<div class="row">
								...
								<div class="col-sm-5">
									<fieldset class="form-group">
										...
										<div class="btn-group">
                                        <?php 
                                            //print_r($agencyConnectedList);
                                        ?>
											<select id="agency_filter" name="agency_filter" class="form-control">
												<option value="0">-- Select --</option>
												<?php 
                                                    foreach ($agencyConnectedList as $row) { ?>
                                                        <option data-no_bulk_match="<?php echo $row->no_bulk_match; ?>" value="<?php echo $row->agency_id; ?>" <?php isset($selected) ? ($row->agency_id == $selected) ? 'selected' : '' : ''?>><?php echo ( ( $row->count > 0 )?"({$row->count}) ":null ).$row->agency_name." ". ( ( $row->no_bulk_match > 0 )?"(Shared) ":null );  ?></option>
                                                    <?php
                                                    }
												?>
											</select>					
								...
								<div class="col-sm-5">
									<img src="<?= $_ENV['COMPANY_LOGIN_LOGO'] ?>" class="company_logo sats_logo" />
								</div>
							...
							<div class="row">
								<div class="col-sm-8">
									...
									<div class="btn-group" role="group" aria-label="Basic example">
										<button style="border-radius: 3px !important;" type="button" class="btn btn-match btn-primary btn-sm" id="btnMatch">1. Exact Match</button>
										<button style="border-radius: 3px !important;" type="button" class="btn btn-match btn-primary btn-sm" id="btnProbable">2. Probable Match</button>
										<button style="border-radius: 3px !important;" type="button" class="btn btn-match btn-primary btn-sm" id="btnPossibleMatch" style="z-index: 1; position: relative;">3. Possible Match</button>
								...
								<div class="col-sm-4" style="z-index: 0; position: relative;">
									<img src="/images/ourtradie.png" class="company_logo pme_logo"  style="height: 100%; width: 100%; object-fit: contain" />
								</div>
							</div>
...
<div id="pme_details_fb" class="fancybox" style="display:none;" >
	...
	<h4>Ourtradie Property Details</h4>
...
function save_note(pnv_id,property_source,property_id,property_address,agency_id,note,note_btn_class){
	...
	jQuery.ajax({
        ...
        url: "/ourtradie/bulk_connect_save_note",
		type: 'POST',
		data: { 
			'pnv_id': pnv_id,
...
$(document).ready(function() {
	...
	jQuery("#pme_table_div").on("click",".btn_note",function(){
		...
		// prefill data
		var property_source = 7; // ourtradie

		var property_id = jQuery(this).parents("td:first").find(".ot_prop_id").val();
		var property_address = jQuery(this).parents("td:first").find(".ot_full_address").val();
	...
	jQuery("#pme_table_div").on("click",".view_note_btn",function(){
		...
		jQuery.ajax({
			url: "/ourtradie/ajax_get_pnv_note",
			type: 'POST',
			data: { 
				'pnv_id': pnv_id
			}
	...
	jQuery("#crm_table_div").on("click",".verify_nlm_btn",function(){
        ...
        console.log("note_btn_class: "+property_source);
        console.log("note_btn_class: "+property_id);
        console.log("note_btn_class: "+property_address);
        console.log("note_btn_class: "+agency_id);
        console.log("note_btn_class: "+note);
        */
	...
	// Ourtradie property details - CHOPS
	jQuery("#pme_table_div").on("click",".pmeAdd",function(){	
        
		var ot_addr_address = jQuery(this).parents("tr:first").find(".ot_full_address").val();
		var ot_addr_suburb  = jQuery(this).parents("tr:first").find(".ot_addr_suburb").val();
		var ot_addr_postalcode = jQuery(this).parents("tr:first").find(".ot_addr_postalcode").val();
		var ot_addr_state   = jQuery(this).parents("tr:first").find(".ot_addr_state").val();
        var ot_addr_country = jQuery(this).parents("tr:first").find(".ot_addr_country").val();

		var agency_id = jQuery("#agency_filter").val();
		
		var ot_table_fb   = jQuery("#pme_prop_details_tbl_fb");	
        var ot_tenants_fb = jQuery(".pme_prop_tenants_tbl_fb");
        

		// prefill property details
		ot_table_fb.find(".pme_addr_street_td").html(ot_addr_address);
		ot_table_fb.find(".pme_addr_suburb_td").html(ot_addr_suburb);
		ot_table_fb.find(".pme_addr_postalcode_td").html(ot_addr_postalcode);
		ot_table_fb.find(".pme_addr_state_td").html(ot_addr_state);
        ot_table_fb.find(".pme_addr_country_td").html(ot_addr_country);

        //Tenant
		const tenantStrObj = jQuery(this).parent().find('.ot_tenant_list_hidden_table').val()
		const tenantList = (tenantStrObj !== '') ? JSON.parse(tenantStrObj) : {}
		//console.log('==== tenantList: ', tenantList)

		jQuery('.pme_prop_tenants_tbl_fb').html('')
		tenantList.forEach(function (tenant, ndx) {
			const html = ' <tr> ' +
                '<td class="ot_tenant_fname_td">'+ tenant.FirstName+'</td> ' +
                '<td class="ot_tenant_lname_td">'+ tenant.LastName+'</td> ' +
                '<td class="ot_tenant_email_td">'+ tenant.Email+'</td> ' +
                '<td class="ot_tenant_mobile_td">'+ tenant.Mobile+'</td> ' +
            '</tr>'

			jQuery('.pme_prop_tenants_tbl_fb').append(html)
		})
		// pop-up lightbox
	...
	jQuery("#pme_table_div").on("click","#btn_add_prop",function(){
		...
		// confirm add Ourtradie property on crm
		swal({
			title: "Are you sure?",
			...
			if (isConfirm) { // yes		
				...
				jQuery(".pme_prop_chk:checked").each(function(){
					...
					*/
					...
					var ot_full_address = row.find(".ot_full_address").val();
					var ot_addr_suburb  = row.find(".ot_addr_suburb").val();
					var ot_addr_state   = row.find(".ot_addr_state").val();
					var ot_addr_postalcode = row.find(".ot_addr_postalcode").val();
					var ot_addr_country    = row.find(".ot_addr_country").val();

					var ot_prop_id = row.find(".ot_prop_id").val();
					var key_number = row.find(".key_number").val();

					var tenants_name   = row.find(".ot_tenant_name").val();
					var tenants_email  = row.find(".ot_tenant_email").val();
					var tenants_mobile = row.find(".ot_tenant_mobile").val();
						...
						if( ret.length == 0 ){
							...
							if( ret.length > 0 ){
								...
								for( var i=0; i<ret.length; i++ ){
									dup_html += ''+
									'<li>'+
										'<a href="/ourtradie/property/'+ret[i].dup_property_id+'/'+ret[i].dup_agency_id+'" target="_blank">'+
											ret[i].dup_property_address+										
										'</a>'+
										' On <a href="/view_agency_details.php?id='+ret[i].dup_agency_id+'">'+ret[i].dup_agency_name+'</a>'+
									'</li>';
								}
								dup_html +='</ul></div>';							
	...
	// invididual add property function - CHOPS
	jQuery("#pme_table_div").on("click",".btn_add_prop_indiv",function(){
		...
		var ot_addr_address1 = row.find(".ot_addr_address1").val();
		var ot_addr_address2 = row.find(".ot_addr_address2").val();
		var ot_addr_suburb  = row.find(".ot_addr_suburb").val();
		var ot_addr_state   = row.find(".ot_addr_state").val();
		var ot_addr_postalcode = row.find(".ot_addr_postalcode").val();
		var ot_addr_country    = row.find(".ot_addr_country").val();
		...
		var json_data = { 
			'ot_addr_address1': ot_addr_address1, 
			'ot_addr_address2': ot_addr_address2,
			'ot_addr_suburb': ot_addr_suburb,
			'ot_addr_state': ot_addr_state,
			'ot_addr_postalcode': ot_addr_postalcode,
			'ot_addr_country': ot_addr_country,
		...
		if( ot_prop_arr.length > 0 ){
			...
			jQuery.ajax({
				...
				if( ret.length == 0 ){
					...
					//if( ret[0].dup_prop_deleted == 1 && ret[0].dup_agency_id == agency_id ){ ## disabled > used is_nlm as per new nlm/delete changes
					if( ret[0].dup_prop_deleted == 1 && ret[0].dup_agency_id == agency_id ){
						...
						swal({
							...
							function(isConfirm) {
								if (isConfirm) {		
									...
									ot_prop_id_arr2.push(ot_prop_id);	// pme property				  									
									
									// connect them
									$('#load-screen').show(); 
									$.ajax({
										//url: "/property_me/bulk_connect_all",
                                        url: "/ourtradie/bulk_connect_all",
										type: 'POST',
										data: { 
											'agency_id': <?=isset($selected) ? $selected : "1"?>,
											'crmArr' : crm_prop_id_arr2,
											'pmeArr' : ot_prop_id_arr2
										}
									}).done(function( ret ){
						...
						if( ret.length > 0 ){
							...
							if( ret[0].dup_prop_deleted == 1 ){ //new changes > move nlm property to current if already nlm from other agency
								...
								ourtradie_prop_id_arr2.push(ot_prop_id);
								...
								if( ret[0].dup_agency_id == agency_id ){
									...
									//this_agency = '<a href="<?php //echo $this->config->item('crm_link'); ?>/view_agency_details.php?id='+ret[0].dup_agency_id+'">'+ret[0].dup_agency_name+'</a>';
									this_agency = '<a href="<?php echo base_url(); ?>agency/view_agency_details/'+ret[0].dup_agency_id+'">'+ret[0].dup_agency_name+'</a>';
								}
	...
	jQuery("#agency_filter").change(function(){
		...
		var country_id = "<?php echo $this->config->item('country') ?>";
		var no_bulk_match = $(this).find(':selected').attr('data-no_bulk_match');

		console.log(no_bulk_match);
		// match_counter
		match_counter(1);
		...
		if( agency_id > 0 ){
			...
			jQuery.ajax({
                ...
                url: "/ourtradie/ajax_bulk_connect_get_crm_list",
				type: 'POST',
				data: { 
					'agency_id': agency_id
				}
				...
				// get Ourtradie API properties				
				jQuery('#load-screen').show(); 
				jQuery.ajax({
					//url: "/property_me/ajax_bulk_connect_get_pme_list",
                    url: "/ourtradie/ajax_bulk_connect_get_ourtradie_list",
					type: 'POST',
					data: { 
						'agency_id': agency_id,
	...
	$("#btnPossibleMatch").click(function() {
		...
		setTimeout(function(){ 
			...
			crmTable.rows().every(function(index, element) {	
				...
				var crm_addr_street_name = clearStreetName(crm_node.find(".crm_full_address").val());				
				...
				pmeTable.rows().every(function(index2, element2) {					
					...
					*/
					...
					var pme_addr_street = clearStreetName(pme_node.find(".ot_full_address").val());
	...
	$("#bulkCon").click(function() {
		...
		jQuery("#pmeProp .match_hl_bgcolor").each(function(){
			...
			var pme_prop_id = jQuery(this).find(".ot_prop_id").val();
			pme_prop_id_arr.push(pme_prop_id);
			
			console.log(pme_prop_id_arr);
			//return false;
		...
		if (
			...
			if (numCon > 0) {
				...
				swal({
					title: "Are you sure?",
					text: "You are about to connect the matched CRM properties and OurProperty properties below.",
					type: "warning",
					showCancelButton: true,
					confirmButtonClass: "btn-success",
					confirmButtonText: "Yes, Connect it!",
	...
	jQuery("#pme_table_div").on("click",".btn_hide_api_prop",function(){
		...
		var agency_id = jQuery("#agency_filter").val();
		var api_prop_id = row.find(".ot_api_prop_id").val();

		//console.log(agency_id);
		//console.log(api_prop_id);
		//return false;
