										...
										<div class="btn-group">
											<select id="agency_filter" name="agency_filter" class="form-control">
												...
												foreach ($agenList as $row) {
													...
													$option_color = 'unset';
													$agency_apend_txt = null;

													if (!is_null($row['connection_date']) && !empty($row['connection_date'])) {

														if( is_null($row['pme_supplier_id']) || empty($row['pme_supplier_id']) ){
															$option_color =  'red';
															$agency_apend_txt = ' (Connect me to '.config_item('company_name_short').' supplier)';
														}

														/*
														if( $row['api_billable'] == 0 ){
															$option_color =  '#d4d4d4';
															$agency_apend_txt = ' (Do not connect supplier)';
														}
														*/
														
												?>
													<option value="<?=$row['agency_id']?>" <?=isset($selected) ? ($row['agency_id'] == $selected) ? 'selected' : '' : ''?> style="color:<?=$option_color?>" data-api_billable="<?=$row['api_billable']?>"><?=$row['agency_name']?><?php echo $agency_apend_txt; ?>
														<?=$row['SupplierChartAccountId'] == "00000000-0000-0000-0000-000000000000" ? " (Incomplete ".$this->config->item('company_name_short')." Supplier)" : ""?>
													</option>
												<?php
													}
												}
		...
		jQuery(document).on("click",".con_supp",function(){
			...
			jQuery.ajax({
				...
				if (acc == "00000000-0000-0000-0000-000000000000") {
					...
					var ret = text + " (Incomplete <?=$this->config->item('company_name_short')?> Supplier)";
					...
					var text = $("#agency_filter").find("option:selected").text();
					var ret = text.replace('(Incomplete <?=$this->config->item('company_name_short')?> Supplier)','');
					$("#agency_filter").find("option:selected").text(ret);
				}
