                    ...
                    if( $this->input->get_post('agency_filter') > 0 ){		
						if( count($crm_prop_arr) > 0 ){              
                            foreach( $crm_prop_arr as $index => $row ){ 
								...
								$api_name = null;
								$controller = null;	
								$$deactivated_date = 'N/A';	
											
								switch( $row->api_type ){

									case 1: // Pme
										$api_name = 'PropertyMe';
										$controller = 'property_me';
										$deactivated_date = date('d/m/Y',strtotime($row->api_archived_date));
									break;

									case 3: // PropertyTree
										$api_name = 'Property Tree';
										$controller = 'property_tree';
									break;
									...
									case 6: // OurTradie
										$api_name = 'OurTradie';
										$controller = 'ourtradie';
									break;

								}

								// crm CI link
								if( $row->api_type == 3 ){ // property tree
									$crm_ci_page = "/{$controller}/connection_details/{$row->property_id}";
								}else{
									$crm_ci_page = "/{$controller}/property/{$row->property_id}/{$row->agency_id}";
								}																	
								?>
                                <tr>
                                    <td>
                                        <a href="<?php echo $crm_ci_page; ?>" target="_blank">
                                            <?php echo $p_address; ?>
                                        </a>
									</td>    	
                                    <td><?php echo $api_name; ?></td>								
									<td><?php echo $row->api_prop_address; ?></td>									
									<td><?php echo $deactivated_date; ?></td> 
									<td><center>
										<div class="checkbox">
