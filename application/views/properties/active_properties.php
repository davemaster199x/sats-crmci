					...
					<div class="row">
						...
						<div class="col-mdd-2">
							...
							<select id="state_filter" name="state_filter" class="form-control ">
								...
								<?php
								foreach( $state_filter_sql as  $state_row ){
									if($state_row->state!=""){
								?>
					...
					<thead>
						<tr>
							...
							foreach( $service_types->result() as $ajt): ?>
                                <?php
                                    if($this->config->item('country')==2 && $ajt->id != 2) {
                                        //NZ removed other services
                                        continue;
                                    }
                                ?>
                                <th>
						...
						<tr>
							<td>
								<?php
									...
									//echo $c_url;
									if($c_url == "https://crm.staging.{$this->config->item('base_domain')}/"){
										$redirect = 1;
									}
									if($c_url == "https://crm.{$this->config->item('base_domain')}"){
										$redirect = 2;
									}
								?>
								<a href="/properties/details/?id=<?php echo $row->property_id ?>&r=<?php echo $redirect?>">
									<?php echo  $row->p_address_1." ".$row->p_address_2.", ".$row->p_address_3; ?>
								</a>
							</td>
							<td><?php echo $row->p_state; ?></td>
							<td><?php echo $row->p_postcode; ?></td>
							<td><?php echo $row->region_name; ?></td>
							<td><?php echo $row->subregion_name; ?></td>
							<?php 
							/*				
							// get service type
							foreach( $service_types->result() as $ajt ){ 
