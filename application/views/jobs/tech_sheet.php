...
<div class="box-typical box-typical-padding techsheet_main_div">
    ...
    if( $staff_class == 6 ){ // tech
        ...
        $back_link = "{$this->config->item('crmci_link')}/jobs/details/{$this->input->get_post('job_id')}";
        $bc_items[] = array(
			'title' => 'Job Details',			
			'link' => $back_link
        );
                ...
                <ul id="jmain_tab" class="nav j_remember_tab" role="tablist">
                    ...
                    <li id="main_tab_techsheet" class="nav-item" data-tab_step="3">
                        <a class="nav-link" href="#tab_techsheet" role="tab" data-toggle="tab">
                            <span class="nav-link-in">                                                
                                Tech Sheet
                                <?php //echo ( count($service_types_arr) == 1 )?'<img class="service_type_icons" src="/images/serv_img/'.Alarm_job_type_model::icons($service_types_arr[0]).'" />':null; ?>
                                <?=Alarm_job_type_model::icons($job_row->j_service);?>
                            </span>
                        </a>
                    </li>
    ...
    <input type="hidden" id="staff_class" value="<?php echo $staff_class; ?>" />
    <input type="hidden" id="has_ss_view_only" value="<?php echo $has_ss_view_only; ?>" />
