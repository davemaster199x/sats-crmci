...
<div class="box-typical box-typical-padding">
	...
	<header class="box-typical-header">
		...
		echo form_open(base_url('/jobs/bne-to-call'),$form_attr);
		?>
			<div class="for-groupss row">
			<div class="col-md-10 columns">
					<div class="row bne_search_box">
						<div class="col-mdd-3">
					...
					<tbody>
						...
						<tr>
							...
							<td>
                                ...
                                ?> 
                                        <a href="<?php echo $this->config->item('crm_link') ?>/set_tech_run.php?tr_id=<?php echo $str_row['tech_run_id'] ?>"><?php echo date('d/m',strtotime($str_row['date'])); ?></a><?php	
                                        $fcount++;
							...
							*/
							...
							<td>
								<div class="pos-rel">
									<input style="min-width:150px;" type="text" name="bne_notes[]"  class="form-control bne_notes" data-jobid="<?php echo $list_item['jid'] ?>" value="<?php echo $list_item['bne_to_call_notes']; ?>">
									<i class="fa fa-check-circle text-green bne-check-ok check_ok_ajax"></i>
								</div>
							</td>
						</tr>
						<?php endforeach;
...
<div id="about_page_fb" class="fancybox" style="display:none;" >
	...
	<p>
		This page shows jobs that are NOT to be called by the overseas call center for varying reasons. 
		It may be that the tenant has complained, or the tenant is a property manager etc.
	</p>
	<pre>
...
<script type="text/javascript">
	...
	jQuery(document).ready(function() { // Document ready start
		...
		jQuery(".bne_notes").change(function(){
			...
			jQuery.ajax({
				type: "POST",
				url: "/jobs/ajax_update_job_bne_notes",
				data: { 
					job_id: job_id,
					bne_note: bne_note
				}
				...
				// show tick
				obj.parents("tr:first").find(".bne-check-ok").fadeIn();
				// fade
				setTimeout(function(){ 
					obj.parents("tr:first").find(".bne-check-ok").fadeOut();
				}, 5000);
			});
		});
