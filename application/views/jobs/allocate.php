			...
			?>
				...
				<div class="row">
					...
					<label>Assigned To:</label>
						<select class="select2 form-control" multiple="multiple" id="allocate_jobs_select" name="allocate_jobs_select">
							<?php foreach($stafflist as $row): ?>
								<option value="<?php echo $row['StaffID'] ?>" <?php echo (in_array($row['StaffID'], $explode_global_settings))?'selected="selected"':'' ?>>
				...
				<table class="table table-hover main-table">
						...
						<tr>
							...
							<th>
								...
								<a 
                                    ...
                                    href="<?php echo "{$uri}/?sort_header=1&order_by=alloc_by.FirstName&sort={$toggle_sort}&".http_build_query($header_link_params); ?>"
                                >
							...
							<th>
								...
								<a 
                                    data-toggle="tooltip" 
                                    class="a_link <?php echo $sort ?>" 
                                    href="<?php echo "{$uri}/?sort_header=1&order_by=j.allocate_response&sort={$toggle_sort}&".http_build_query($header_link_params); ?>"
                                >
                                    <em class="fa fa-sort-<?php echo $sort; ?>"></em>
                                </a>
							</th>
					...
					<tbody>
						...
						<tr>
							...
							<td><?php echo ( $row['is_eo'] == 1 )?'Yes':'No'; ?></td>
							<td>
								<?php
								/*
...
<pre><code><?php echo $display_query; ?></code></pre>
...
<script type="text/javascript">
	...
	jQuery(document).ready(function(){ //doc ready start
		...
		$('.response').on('change',function(e){
			...
			var allocate_personnel = "<?php echo $global_settings->allocate_personnel; ?>";
	...
	jQuery('.region_filter_main_div').on('click','.region_filter_state',function(){
        ...
        /**
         * Insert/Update jobs allocate
         */
		$('#allocate_jobs_select').change(function(){
			var staffID = $('#allocate_jobs_select').val();
			...
			jQuery.ajax({
				type: "POST",
                url: '/ajax/jobs_ajax/ajax_update_allocate_personnel',
				data: { 
					staff_id: staffID
				}
			}).done(function( ret ){
				$('#load-screen').hide(); //hide loader
				swal('','Assigned To Updated!','success');
			});
