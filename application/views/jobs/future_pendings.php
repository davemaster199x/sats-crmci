        ...
        <div class="text-center col-md-6 columns">
			...
			*/
			...
			for( $i = 3; $i >= 1; $i-- ){ 
				...
				$from_link = date("Y-m-01",strtotime("{$sql_from} -".($i)." months"));				
				$to_link = date("Y-m-t",strtotime("{$sql_from} -".($i)." months"));
				$quick_links_month = date("F",strtotime("{$sql_from} -".($i-1)." months"));
				?>
				<a href="/jobs/future_pendings?date_from_filter=<?php echo $from_link; ?>&date_to_filter=<?php echo $to_link; ?>&get_sats=1&month=<?php echo $this_month; ?>&<?php echo http_build_query($pagi_links_params_arr); ?>">
					<?php echo $quick_links_month; ?> 
			...
			// current
			$from_link = date("Y-m-01",strtotime($sql_from));				
			$to_link = date("Y-m-t",strtotime($sql_from));
			$quick_links_month = date("F",strtotime("{$sql_from} +1 month"))
			?>
		
			<a href="/jobs/future_pendings?date_from_filter=<?php echo $from_link; ?>&date_to_filter=<?php echo $to_link; ?>&get_sats=1&month=<?php echo $this_month; ?>&<?php echo http_build_query($pagi_links_params_arr); ?>">
				<span class="font-weight-bold"><?php echo strtoupper($quick_links_month); ?></span> 
			...
			for( $i = 1; $i <= 3; $i++ ){ 
				...
				<a href="/jobs/future_pendings?date_from_filter=<?php echo $from_link; ?>&date_to_filter=<?php echo $to_link; ?>&get_sats=1&month=<?php echo $this_month; ?>&<?php echo http_build_query($pagi_links_params_arr); ?>">
					<?php echo "{$quick_links_month} "; ?>  
					<?php echo (( $i < 3 )?'<i class="fa fa-chevron-right mx-2"></i>':null); ?>
				</a>				
			<?php
			}
			?>
					...
					<tbody>
						...
						?>
								...
								<tr>
									...
									<td><?php echo Alarm_job_type_model::icons($row['ajt_id'])." {$row['ajt_type']} "; ?></td>
									<td>
									<?php
									...
									echo '$'.number_format($price_var_arr['dynamic_price_total'],2); 
									?>
									</td>
									<td><?php 
