...
<div class="box-typical box-typical-padding">
	...
	$export_link_params = '/jobs/to_be_invoiced/?export=1&'.http_build_query($export_links_params_arr);
	?>
	...
	<header class="box-typical-header">
		...
		?>
			<div class="for-groupss row">
				<div class="col-lg-8 col-md-9 columns">
					...
					<div class="row">
						...
						<div class="col-mdd-3">
							...
							<input name="date_filter" placeholder="ALL"  class="flatpickr form-control flatpickr-input" data-allow-input="true" id="flatpickr" type="text"  value="<?php if($_GET['date_filter'] == ""){echo $this->input->get_post('date_filter');}else{echo date('d/m/Y',strtotime($_GET['date_filter']));} ?>">
						</div>
		...
		<div class="body-typical-body">
			<div class="table-responsive" style="overflow-y: hidden;">
					...
					<?php
						...
						*/
						...
						if( 
							...
							//( $ddmm_format_job_date != $ddmm_format_prop_sub_date ) //old replaced below
							( date("m-d") < date('m-d', strtotime($prop_sub_row->subscription_date)) )
						){
						...
						*/
						...
						if( 
							( $this->system_model->isDateNotEmpty($prop_sub_row->subscription_date) == true && $this->system_model->isDateNotEmpty($list_item['jdate']) == true ) &&
							( date('m-d',strtotime($prop_sub_row->subscription_date)) != date('m-d',strtotime($list_item['jdate'])) )
						){
							$row_color = "style='background-color:#fac3c373;'";
						}
						...
						<!--<tr class="body_tr jalign_left">-->
							...
							<td>
								<a href="<?php echo $this->config->item("crmci_link"); ?>/jobs/details/<?php echo $list_item["jid"]; ?>" target="_blank">
							...
							<td>
								<?=Alarm_job_type_model::icons($list_item['jservice']);?>
							</td>
							<td>
								<?php 
							...
							<td>
								<?php
									//get open jobs
									$open_job_custom_where = "j.id!={$list_item["jid"]} AND j.status!='Completed' AND j.status!='Cancelled'";
									$open_job_params = array(
										...
										'custom_where' => $open_job_custom_where
									);
									$open_job_q = $this->jobs_model->get_jobs($open_job_params);
									$open_job_arr = [];
									foreach( $open_job_q->result_array() as $open_job_row ){
										$open_job_arr[] = $this->gherxlib->crmLink('vjd',$open_job_row['jid'],$open_job_row['jid'],'_blank');
									}
							...
							<td>
								<?php
								// check if agency preference "Does the client allow annual visits?" is NO, then hide checkbox
								$agency_pref_sel_sql = $this->db->query("
								SELECT COUNT(`id`) AS agen_pref_sel_count
								FROM `agency_preference_selected`
								WHERE `agency_id` = {$list_item['agency_id']}
								AND `agency_pref_id` = 24
								AND `sel_pref_val` = 0
								");

								if( $agency_pref_sel_sql->row()->agen_pref_sel_count == 0 ){ ?>
							...
							<td>
								...
								if( 
									$this->system_model->isDateNotEmpty($prop_sub_row->subscription_date) == true && 
									( $this->system_model->isDateNotEmpty($list_item['jdate']) == true && $list_item['jdate'] <= date('Y-m-d') ) &&
									date('m-d',strtotime($prop_sub_row->subscription_date)) == date('m-d',strtotime($list_item['jdate'])) 
								){ ?>
				...
				<div style="margin-top: 15px; float: right; display:none;" id="invoice_btn_div">
					<button type="button" id="btn_do_invoice" class="btn btn-inline blue-btn submitbtnImg">Move to Merged</button>
				</div>
...
<div id="about_page_fb" class="fancybox" style="display:none;" >
	...
	<ul class="about_page_li">	
		<li><span style="background-color: #fac3c373;">Pink Highlight</span> - Red highlight shows then day and month of subscription doesn't match day and month of the job.</li>																										
	</ul>
