...
<div class="box-typical box-typical-padding">
	...
	<header class="box-typical-header">
		...
		echo form_open('/jobs/dha',$form_attr);
		?>
				...
				<table class="table table-hover main-table">
					...
					<tbody>
						...
						foreach($lists->result_array() as $list_item): 	
							...
							// row highlights							
							$row_color = ($i%2==0)?"style='background-color:#eeeeee;'":""; // grey alternation color
						...
						<td>
							...
							<td>
								<?php
									$params_active = array('property_id'=>$list_item['prop_id'], 'active' => 1, 'limit'=> 1);
									$active_tenants = $this->properties_model->get_new_tenants($params_active)->row();
									echo "{$active_tenants->tenant_firstname} {$active_tenants->tenant_lastname}";
								?>
							</td>
							...
							<td>
							<div class="checkbox">
									<input class="chk_dha" name="chk_dha[]" type="checkbox" id="check-<?php echo $list_item["jid"] ?>" data-jobid="<?php echo $list_item["jid"]; ?>" value="<?php echo $list_item["jid"]; ?>">
...
AND `p`.`deleted` = 0
AND `a`.`status` = 'active'
AND `a`.`country_id` = <?php echo $this->config->item('country') ?> 
AND `j`.`status` = 'DHA'
...
<script type="text/javascript">
	...
	jQuery(document).ready(function(){
		...
		$('#check-all').on('change',function(){
			...
			if(isChecked){
				divbutton.show();
				$('.chk_dha').prop('checked',true);
			}else{
				divbutton.hide();
				$('.chk_dha').prop('checked',false);
			}
		})

		$('.chk_dha').on('change',function(){
			var obj = $(this);
			var isLength = $('.chk_dha:checked').length;
			var divbutton = $('#mbm_box');
			if(isLength>0){
				divbutton.show();
			}else{
		...
		jQuery("#assign_btn").on('click',function(){
			...
			jQuery(".chk_dha:checked").each(function(){
