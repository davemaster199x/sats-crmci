...
<input type="hidden" id="add0" value="<?=$property_id?>">
                ...
                <div class="col-md-12">
                    ...
                    <table class="table table-striped table-bordered " id="myTable">
                        <thead>
                            <tr>
                                <th>Address</th>
								<th class="text-center">Unlink</th>
                            </tr>
                        </thead>
                        <tbody>
                        <tr>
							...
							<td class="text-center">
								...
								<a 
									...
									data-toggle="tooltip" 
									title="Unlink this Console property" 
									data-id="<?=$console_prop_row->cp_id?>"
								>
...
<script type="text/javascript">
	...
	jQuery('.btn_save_crm_tenant').click(function() {
		...
		$(document).on('click', '.btn_unlink', function() {
			...
			if( crmId > 0 ){
				...
				swal({
				title: "Are you sure?",
				text: "You will unlink this Console property.",
				type: "warning",
				showCancelButton: true,
				confirmButtonClass: "btn-danger",
				...
				if (isConfirm) {
					$('#load-screen').show(); 
					$.ajax({
						url: "/console/ajax_function_unlink_property",
						type: 'POST',
						data: { 
							'cp_id': cp_id,
							'crmId': crmId
						...
						if (ret.updateStat === true) {
							...
							setTimeout(function(){ location.href = '/console/to_connect/<?php echo $property_id; ?>'; }, <?php echo $this->config->item('timer') ?>);	
						}else {
							swal({
								title: "Error!",
								text: "Something went wrong, contact dev.",
