										...
										<div class="btn-group">
											<select id="agency_filter" name="agency_filter" class="form-control">
												...
												foreach ($agenList->result_array() as $row) { ?>
													<option data-no_bulk_match="<?php echo $row['no_bulk_match'] ?>" value="<?=$row['agency_id']?>" <?=isset($selected) ? ($row['agency_id'] == $selected) ? 'selected' : '' : ''?>><?php echo ( ( $row['count'] > 0 )?"({$row['count']}) ":null ).$row['agency_name']." ". ( ( $row['no_bulk_match'] > 0 )?"(Shared) ":null );  ?></option>
												<?php
												}
												?>
											</select>					
						...
						<div class="container-fluid" id="pme_button_div">
							...
							<div class="row">
								...
								<div class="col-sm-4" style="z-index: 0; position: relative;">
									<img src="/images/third_party/console_default.png" class="company_logo pme_logo"  style="height: 100%; width: 100%; object-fit: contain" />
								</div>
							</div>
...
$(document).ready(function() {
	...
	jQuery("#crm_table_div").on("click",".verify_nlm_btn",function(){
		...
		var is_nlm = obj.parents("td:first").find(".is_nlm").val();

		if( is_nlm == 1 ){ // NLM

			swal({
				title: "Warning!",
				text: "This property is NLM with this agency in CRM. Add to PNV and confirm with the agency if we should reactivate and service this property.",
				type: "warning",						
	...
	*/
	...
	jQuery("#pme_table_div").on("click",".btn_add_prop_indiv",function(){
		...
		var property_use = row.find(".property_use").val();	
		...
		if( pme_prop_arr.length > 0 ){
			...
			jQuery('#load-screen').show(); 
			jQuery.ajax({
				url: "/console/bulk_connect_add_property",
				type: 'POST',
				data: { 
					'agency_id': agency_id,
					'pme_prop_arr': pme_prop_arr
				...
				if( ret.length == 0 ){
					...
					if( ret[0].dup_prop_deleted == 1 && ret[0].dup_agency_id == agency_id ){
						...
						dup_html += ''+
							'<li>'+
								'<a href="/properties/details/?id='+ret[0].dup_property_id+'" target="_blank">'+
	...
	<?php
	// agency filter from url
	if( $this->input->get_post('agency_id') > 0 ){ ?>

			setTimeout(function(){ 			             
				jQuery("#agency_filter").val(<?php echo $this->input->get_post('agency_id'); ?>).change();                              
			}, 1000); 
	...
	jQuery("#agency_filter").change(function(){
		...
		if( agency_id > 0 ){
			...
			jQuery('#load-screen').show(); 
			jQuery.ajax({
				url: "/console/ajax_bulk_connect_get_crm_list",
				type: 'POST',
				data: { 
					'agency_id': agency_id
				}
				...
				jQuery('#load-screen').show(); 
				jQuery.ajax({
					url: "/console/ajax_bulk_connect_get_console_list",
					type: 'POST',
					data: { 
						'agency_id': agency_id,
					...
					*/
					...
					<?php
					// if address filtered from url
					if( $this->input->get_post('p_address_search') != '' ){ ?>

						pmeTable.search("<?php echo $this->input->get_post('p_address_search'); ?>").draw();
						crmTable.search("<?php echo $this->input->get_post('p_address_search'); ?>").draw();
	...
	$("#bulkCon").click(function() {
		...
		if (
			...
			if (numCon > 0) {
				...
				swal({
					title: "Are you sure?",
					text: "You are about to connect the matched CRM properties and Console properties below.",
					type: "warning",
					showCancelButton: true,
					confirmButtonClass: "btn-success",
					confirmButtonText: "Yes, Connect it!",
