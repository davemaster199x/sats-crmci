...
class Pme_model extends CI_Model
{
    ...
    public function getAccessToken($params){
        ...
        if( $agency_id > 0 ){
            ...
            $ret_json = $this->check_if_access_token_is_expired($pme_row->access_token);

            if( $ret_json->httpcode == 200 ){ // session is still active
    ...
    // check if token expired
    public function check_if_access_token_is_expired($access_token){
        ...
        // API endpoint, use 'get supplier LIMIT 1' for fast API call
        $url = "https://app.propertyme.com/api/v1/contacts/suppliers?Offset=0&Limit=1";
    ...
    public function get_tenancies($params){
    ...
    public function get_jobs_with_pme_connect($params) {
        ...
        $this->db->join('`agency_api_documents` AS aad', 'aad.agency_id = a.agency_id', 'left');
    ...
    public function send_all_certificates_and_invoices( ?bool $is_get_data = false ): array
    {
        ...
        foreach ($listsPme as $val) {
            ...
            if( $is_not_compliant == false ){ // do not upload API, if not compliant
                ...
                if( $val['pme_api'] == $pme_api && $val['pme_prop_id'] != '' ){
                    ...
                    $is_invoice = isset($val['is_invoice']) ? (int)$val['is_invoice'] : null;
                    $is_certificate = isset($val['is_certificate']) ? (int)$val['is_certificate'] : null;
                    ...
                    $isFailUpload = $this->upload_invoice_v2($upload_inv_params);
                }
            }

        }

        return ["err" => $isFailUpload];
    }
    ...
    public function upload_invoice($params){
        ...
        $is_certificate = $params['is_certificate'];
        $country_id = $this->config->item('country');
        $isFailUpload = false;
        ...
        if( $job_details['assigned_tech'] != 2 ){
            ...
            if ($is_certificate == 1 || $is_certificate == null) {
        ...
        if ($is_invoice == 1 || $is_invoice == null) {
    ...
    // wrap up upload API function from merge and vjd to one
    public function upload_invoice_v2(array $params = []): bool
    {
        // variables
        $pmePropId = $params['propertyme_prop_id'];
        $agencyId = $params['a_id'];
        ...
        // API Upload Invoice and Certificate 
        if (($is_invoice  === 1 && $is_certificate === 1) || (is_null($is_invoice)  && is_null($is_certificate))) {
        ...
        /*** Upload API Invoice and Send Email Certificate ***/
        if ($is_invoice  === 1 && $is_certificate === 0) {
        ...
        /***  Invoice and Upload API Certificate ***/
        if ($is_invoice  === 0 && $is_certificate === 1) {
        ...
        /*** SEND EMAIL INVOICE/CERTIFICATE ***/
        if ($is_invoice  === 0 && $is_certificate === 0) {
            $this->email_functions_model->batchSendInvoicesCertificates($job_id);
        }
    ...
    public function upload_pdf_to_property($agencyId, $pmePropId, $file, $fileName, $job_details) {
        ...
        $log_details = "<a href='".$protocol."://{$baseUrl}/pdf/certificates/{$encrypt}'>Certificate</a>, #{$bpay_ref_code} to the linked PMe property";
        $log_params = array(
            'title' => 69,  // PMe API
            'details' => $log_details,
            'display_in_vjd' => 1,
    ...
    public function upload_pdf_to_bill($agencyId, $pmePropId, $jobId, $countryId, $job_details, $property_details, $alarm_details, $num_alarms) 
    {
        ...
        $this->system_model->updateInvoiceDetails($jobId); //update invoice - incase they added adjustment payments before re uploading
        ...
        //$getTotalAmount = $this->system_model->getJobTotalAmount($jobId);
        $getTotalAmount = $job_details['invoice_balance'];
       ...
       /* $price_var_params = array(
        ...
        $result = $this->system_model->get_job_price_breakdown($price_var_params);*/

        //$new_dynamic_ss_price = $result['dynamic_price'];
        $new_dynamic_ss_price = $getTotalAmount;
        ...
        if (!is_null($suppId->pme_supplier_id) && floatval($getTotalAmount) > 0) {
            ...
            if( $bill_error == true || $property_details['send_to_email_not_api'] == 1 ){ // bill creation will fail on PMe, send invoice to email instead
                ...
                $create_bill_params = array(
                    ...
                    'owner_id' => $owner_id,
                    'getTotalAmount' => $new_dynamic_ss_price,
                ...
                if( $responseCode == 200 || $responseCode == 204 ){ // OK, success
                    ...
                    // job log1
                    $encrypt = rawurlencode(HashEncryption::encodeString($jobId));
                    $baseUrl = $_SERVER["SERVER_NAME"];
                    if(isset($_SERVER['HTTPS'])){
                        $protocol = ($_SERVER['HTTPS'] && $_SERVER['HTTPS'] != "off") ? "https" : "http";
                    } else{
                    ...
                    $log_details = "<a href='".$protocol."://{$baseUrl}/pdf/invoices/{$encrypt}'>Invoice</a>, #{$invoice_number} uploaded to PMe Agency as a bill of {$new_dynamic_ss_price}";
                    $log_params = array(
                        'title' => 69,  // PMe API
                        'details' => $log_details,
                        'display_in_vjd' => 1,
    ...
    public function get_pme_prop_pm($params){
        ...
        if( $prop_id != '' && $agency_id > 0 ){
            ...
            echo "Missing Property/Agency ID";
        }    
