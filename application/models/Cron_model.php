...
class Cron_model extends CI_Model
{
    ...
    public function create_renewals_v2($renewal_type){      
            ...
            if( $distinct_agency_row->agency_id > 0 ){
                ...
                $job_sql_str = "
                    ...
                    AND p.`property_id` NOT IN(
                        ...
                        AND DATE(l_inner.`created_date`) = '{$today}'
                ...
                foreach( $job_sql->result() as $job_row ){
                    ...
                    $this->process_renewal($params_obj);
        ...
        // insert to 'renewal' database
        if( $renewal_count_total > 0 ){
    ...
    */
    ...
    public function create_renewals_normal(){  
        ...
        AND a.`status` = 'active'        
        AND a.`country_id` = {$country_id}                                
        AND j.`date` BETWEEN '{$renewal_interval_start}' AND '{$renewal_interval_end}'
        AND aop.`id` IS NULL
        AND p.`property_id` NOT IN(
        ...
        // insert renewals db
        $renewal_count_total = $job_sql->num_rows();   
        
        // insert to 'renewal' database
        if( $renewal_count_total > 0 ){

            $renewal_type = 5; // exclude custom interval
            $renewals_params = array(
                'StaffID' => $staff_id,
                'country_id' => $country_id,
                'date' => date("Y-m-d H:i:s"),
    ...
    public function create_renewals_custom(){      
        ...
        if( $renewal_count_total > 0 ){
            ...
            $renewal_type = 6; // custom interval only
            $renewals_params = array(
                'StaffID' => $staff_id,
                'country_id' => $country_id,
                'date' => date("Y-m-d H:i:s"),
    ...
    public function process_renewal($params_obj){

        $job_row = $params_obj->job_row;
        $job_status_filter = ( $params_obj->job_status_filter != '' )?$params_obj->job_status_filter:'Pending'; // default is 'Pending'
        ...
        if( $country_id == 1 ){ // AU only
            ...
            if( $job_row->agency_id == $hume_house_agency_id ){ // if Hume Agency, renewal interval in 9 months
        ...
        // overrides default if values are set
        $renewal_interval = ( $job_row->renewal_interval > 0 )?$job_row->renewal_interval:$renewal_interval_default; 
        ...
        if( $job_row->p_state == 'QLD' ){ 
            ...
            $renewal_start_offset = ( is_numeric($job_row->renewal_start_offset) )?$job_row->renewal_start_offset:$renewal_start_offset_default;
        }                    
    ...
    */
            ...
            foreach( $job_sql->result() as $job_row ){
                ...
                if( $job_id > 0 && $property_id > 0 ){
                    ...
                    if( count($ten_that_has_mob) > 0 ){
                        ...
                        $log_params = array(
                            'title' => 60, // Tenant Welcome SMS
                            'details' => "Welcome SMS Sent, see job ID <a href=\"{$this->config->item('crmci_link')}/jobs/details/{$job_id}\">$job_id</a>",
                            'display_in_vpd' => 1,
                            'created_by_staff' => $staff_id,
                            'auto_process' => $auto_process,
                            'property_id' => $property_id,
                    ...
                    if( $job_row->new_job_email_to_agent == 1 ){
                        ...
                        $log_params = array(
                            ...
                            'details' => "Notification email sent to agency, \"The Tenants at {$paddress} have now been notified that " . config_item('company_name_short') . " will be contacting them to book an appointment to service their property\"",
                            'display_in_vjd' => 1,
                            'created_by_staff' => $staff_id,
                            'auto_process' => $auto_process,
                            'job_id' => $job_id
            ...
            foreach( $job_sql->result() as $job_row ){
                ...
                if( $job_id > 0 && $property_id > 0 ){
                    ...
                    if( count($ten_that_has_email) > 0 ){
                        ...
                        $tenant_welcome_txt = "Recently your Landlord {$private_agency_name_txt} engaged the services of {$country_row->trading_name} (" . config_item('company_name_short') . ") to service the {$service_type} at the property you occupy.";
                        ...
                        $log_params = array(
                            ...
                            'details' => "Welcome email sent to tenant, \"{$tenant_welcome_txt}\". We have attached our contact details in a <strong>VCARD</strong>. Please open that file and save our contact details to your mobile device or computer.",
                            'display_in_vjd' => 1,
                            'created_by_staff' => $staff_id,
                            'auto_process' => $auto_process,
                            'job_id' => $job_id
                        ...
                        $log_params = array(
                            'title' => 57, // Tenant Welcome Email 
                            'details' => "Welcome Email Sent, see job ID <a href=\"{$this->config->item('crmci_link')}/jobs/details/{$job_id}\">$job_id</a>. We have attached our contact details in a <strong>VCARD</strong>. Please open that file and save our contact details to your mobile device or computer.",
                            'display_in_vpd' => 1,
                            'created_by_staff' => $staff_id,
                            'auto_process' => $auto_process,
                            'property_id' => $property_id,
    ...
    public function update_page_totals(){   
        ...
        $custom_where = "a.`allow_upfront_billing` = 0 AND NOT ( j.`job_type` = 'Yearly Maintenance' AND j.`status` = 'Pending' )";
        ...
        /*
        // PropertyTree connect agency
        $this->load->model('property_tree_model');
        
        $page_url = '/property_tree/connect_agency';
        ...
        if( $json_ret->httpcode == 429 ){ // rate limit exceeded error, get from crm instead
        ...
        */
        ...
        $custom_where = "j.del_job = 0 AND j.status = 'To Be Booked' AND a.`franchise_groups_id` != 14 AND p.`holiday_rental` != 1";
        ...
        // to find any job completed last year and exclude it
        $last_year_only = date('Y',strtotime("-1 year"));
        $custom_where_ym_completed_last_year = "
        p.`property_id` NOT IN(
            SELECT j_inner.`property_id`
            FROM `jobs` AS j_inner
            WHERE j_inner.`status` = 'Completed'
            AND YEAR(j_inner.`date`) = '{$last_year_only}'
        ...
        $params = array(
            ...
            'custom_where' => $custom_where,
            'custom_where_arr' => array(
                $custom_where_overdue_tot, $custom_where_ym_completed_last_year
            ),
            'custom_joins' => $custom_joins,
            'join_table' => array('jobs'),
            'p_deleted' => 0,
        ...
        $total_rows_no_active_prop = empty($ptotal) ? 0 : $ptotal;
        ...
        //**** LAST CONTACT >>>>> START
        $page_url = '/daily/view_last_contact';
        $result = count($this->daily_model->get_job_last_contact('', '', '', '', '', '', ''));
        $total_rows_last_contact = $result;

        // update page total
        $page_tot_params = array(
        'page' => $page_url,
        'total' => $total_rows_last_contact
        );
        $this->system_model->update_page_total($page_tot_params);

        sleep($sleep_interval); // sleep interval
        //****** LAST CONTACT >>>>> END
        ...
        /*
        // Property Tree connect page start
        $page_url_pt_connect_agency = '/property_tree/connect_agency';
        $api_id = 3;

        ##get crm current auth keys
        $auth_keys_sql_str = "
        SELECT apt.`access_token`
        FROM `agency_api_tokens` AS apt
        WHERE apt.`api_id` = {$api_id}
        ...
        $current_auth_keys_arr = [];
        $ptree_company_auth_keys_arr = [];

        foreach( $auth_keys_sql->result() as $auth_keys_row ){
            $current_auth_keys_arr[] = $auth_keys_row->access_token;
        }

        ##get PTree company auth keys
        $json_dec_arr = $this->property_tree_model->get_agencies_activated_with_sats();

        foreach( $json_dec_arr as $comp_auth_obj ){
            $ptree_company_auth_keys_arr[] = $comp_auth_obj->key;
        }

        $removed_exist = array_diff($current_auth_keys_arr,$ptree_company_auth_keys_arr);
        $total_rows_pt_connect_agency = count($removed_exist);

        $page_tot_params_pt_connect_agency = array(
            'page' => $page_url_pt_connect_agency,
            'total' => $total_rows_pt_connect_agency
        );
        $this->system_model->update_page_total($page_tot_params_pt_connect_agency);
        // Property Tree connect page end

        // sleep interval
        sleep($sleep_interval);
        */

        //Update Deactivated Agency Jobs totals
        $deactivated_agencies_with_active_jobs_sel_query = "COUNT(j.`id`) AS jcount";

        $custom_where = "( p.`is_nlm` = 0 OR p.`is_nlm` IS NULL ) AND a.`status` IN('deactivated','target')";
        $deactivated_agencies_with_active_jobs_params = array(
            'sel_query' => $deactivated_agencies_with_active_jobs_sel_query,
            'custom_where' => $custom_where,
            'p_deleted' => 0,            
        ...
        $deactivated_agencies_with_active_jobs = $this->jobs_model->get_jobs($deactivated_agencies_with_active_jobs_params)->row_array();
        $page_tot_params_pt_connect_agency = array(
            'page' => '/jobs/deactivated_agencies_with_active_jobs',
            'total' => $deactivated_agencies_with_active_jobs['jcount']
        );
        $this->system_model->update_page_total($page_tot_params_pt_connect_agency);
        //Update Deactivated Agency Jobs totals end


        //API Property Needs Connecting
        //Insert total count for api_unlinked_properties page
        //This will be use in homepage tile count and menu bubbles when needed
        sleep($sleep_interval);

        $api_property_needs_connectiong_params_custom_where = " AND a.agency_id != 1448"; //exclude Adams Test agency
        $api_property_needs_connectiong_params_total = array(
            'sel_query' =>"COUNT(p.property_id) as count",
            'custom_where' => $api_property_needs_connectiong_params_custom_where
        );
        $api_property_needs_connectiong_total_query_row = $this->properties_model->get_unlinked_api_properties($api_property_needs_connectiong_params_total)->row_array();
        $api_property_needs_connectiong_params = array(
            'page' => '/reports/api_unlinked_properties',
            'total' => $api_property_needs_connectiong_total_query_row['count']
        );
        $this->system_model->update_page_total($api_property_needs_connectiong_params);
        //API Property Needs Connecting end
    ...
    public function updateMainPageTotal(){
       ...
       /* $mr_sql = "
        ...
        $this->system_model->update_main_page_total($page_tot_params_call_over);
        // Last Contact Page >>>> END


        // Console API - Property Tenants Info page
        $page_url = '/console/tenants_info';
        ...
        ");
        $total_rows = $webhooks_data_sql->row()->cwd_count;
        ...
        // Console API - Property Tenants Info page
        $page_url = '/console/unprocessed_webhooks';
    ...
    */
    ...
    public function update_active_job_price_from_property_service_price($params_obj = null){

        $run_in_precomp_prepend_txt = ( $params_obj->run_in_precomp == true )?'On precompletion load ':null;

        $country_id = $this->config->item('country');

        $sql_filter = null;
        if( $params_obj->run_in_precomp ==  true ){
            $sql_filter = "AND j.`status` = 'Pre Completion'";
        }
		...
		foreach( $jobs_sql->result() as $jobs_row ){ 
			...
			if( $jobs_row->jid && ( $update_price_to >= 0 && $jobs_row->job_price != $update_price_to ) ){				
                ...
                if( $jv_sql->row()->jv_count > 0 ){ // job variation exist, skip update
                    ...
                    $email_body .= "
                    <tr>
                        <td><a href='{$this->config->item('crmci_link')}/jobs/details/{$jobs_row->jid}' target='_blank'>{$jobs_row->jid}</a></td>
                    ...
                    //Insert log
				    $log_details = "{$run_in_precomp_prepend_txt}Job price updated from \$".number_format($jobs_row->job_price,2)." to \$".number_format($update_price_to,2)." due to price increase";
        ...
        $this->email->cc(make_email('it'));
        //$this->email->bcc(make_email('cc'));

        $this->email->subject($subject);
        $this->email->message($email_body);

        if( $params_obj->run_in_precomp == false ){
    ...
    /*
    public function create_pt_maintenance_request(){      
        
        $job_sql = $this->property_tree_model->merge_jobs();

        foreach( $job_sql->result() as $job_row ){

            $job_id = $job_row->jid;

            // check if it integrator ID already exist on a job
            $integ_sql = $this->db->query("
            SELECT COUNT(`pt_integ_id`) AS pt_integ_id_count
            FROM `propertytree_integrator_ids`
            WHERE `job_id` = {$job_id}
            AND `active` = 1
            ");

            if( $integ_sql->row()->pt_integ_id_count == 0 ){ // doesnt exist
                ...
                // get property      
                $api_prop_json = $this->property_tree_model->get_property($job_details['property_id']);
                ...
                $curl_ret = $this->property_tree_model->create_maintenance_request($params_obj);
                $json_decoded_response = $curl_ret->json_decoded_response;
                $httpcode = $curl_ret->httpcode;

                if( $httpcode == 202 && $json_decoded_response->data->failure_reason == '' ){

                    // insert
                    $insert_data = array(
                        'integrator_id' => $json_decoded_response->data->integrator_operation_id,
                        'job_id' => $job_id
                    );
                    $this->db->insert('propertytree_integrator_ids', $insert_data);                                                        
    ...
    */
    ...
    /*
    public function pt_upload_invoice_and_certificate(){

        $country_id = $this->config->item('country');       
        $api = 3; // PropertyTree        

        // fetch all Propertytree integrator ID's
        $integ_sql = $this->db->query("
        SELECT *
        FROM `propertytree_integrator_ids` AS pt_integ_ids
        INNER JOIN `jobs` AS j ON pt_integ_ids.`job_id` = j.`id`                    
        WHERE pt_integ_ids.`active` = 1
        ");
        
        foreach( $integ_sql->result() as $integ_row ){

            $job_id = $integ_row->job_id;
            $integrator_id = $integ_row->integrator_id;            
            ...
            $agency_pref = $this->property_tree_model->get_agency_preference($job_details['agency_id']);
            ...
            // upload invoice
            if( $aad_row->is_invoice == 1 || ( $aad_row->is_invoice == '' && !is_numeric($aad_row->is_invoice) ) ){

                // get maintenance request ID 
                $params_obj = (object)[
                    'job_id' => $job_id,
                    'agency_id' => $job_details['agency_id'],
                    'integrator_operation_id' => $integrator_id
                ];
                $curl_ret = $this->property_tree_model->check_if_call_is_completed($params_obj);
                $call_is_completed_response = $curl_ret->json_decoded_response;
                $call_is_completed_httpcode = $curl_ret->httpcode;

                // maintenance request ID
                $maintenance_request_external_id = $call_is_completed_response->data->maintenance_request_external_id;                               
                ...
                $curl_ret = $this->property_tree_model->create_attachment($params_obj);
                $json_decoded_response = $curl_ret->json_decoded_response;
                $httpcode = $curl_ret->httpcode;         
                
                // load attachment URI needed to upload file
                $load_attachment_uri = $json_decoded_response->data;

                if( $httpcode == 200 && $load_attachment_uri != '' ){ // load attachment                                         

                    // file name details
                    $file_name = "invoice_{$bpay_ref_code}";
                    $file_type = ".pdf";
                    ...
                    $params_obj = (object)[
                        ...
                        'load_attachment_uri' => $load_attachment_uri
                    ];
                    $curl_ret = $this->property_tree_model->load_attachment($params_obj);
                    $json_decoded_response = $curl_ret->json_decoded_response;
                    $httpcode = $curl_ret->httpcode;
                    
                    if( $httpcode == 201 && $maintenance_request_external_id != '' ){
                                                
                        // create invoice with attachment
                        $params_obj = (object)[
                            'job_id' => $job_id,
                            'job_details' => $job_details,
                            'maintenance_request_external_id' => $maintenance_request_external_id,
                            'load_attachment_uri' => $load_attachment_uri,
                            'file_name' => $file_name,
                            'file_type' => $file_type
                        ];
                        $curl_ret = $this->property_tree_model->create_invoice_with_attachment($params_obj);
                        $json_decoded_response = $curl_ret->json_decoded_response;
                        $httpcode = $curl_ret->httpcode;

                        if( $httpcode == 202 && $json_decoded_response->data->integrator_operation_id != '' && $json_decoded_response->data->failure_reason == '' ){

                            // check if API call is completed
                            $params_obj = (object)[
                                'job_id' => $job_id,
                                'agency_id' => $job_details['agency_id'],
                                'integrator_operation_id' => $json_decoded_response->data->integrator_operation_id
                            ];
                            $curl_ret = $this->property_tree_model->check_if_call_is_completed($params_obj);
                            $call_is_completed_response = $curl_ret->json_decoded_response;
                            $call_is_completed_httpcode = $curl_ret->httpcode;

                            if( $call_is_completed_httpcode == 200 && $call_is_completed_response->data->failure_reason == '' ){
                                ...
                                // SOFT delete integrator ID on db
                                $update_data = array(
                                    'active' => 0
                                );                                
                                $this->db->where('job_id', $job_id);
                                $this->db->update('propertytree_integrator_ids', $update_data);
            ...
            if ( ( $aad_row->is_certificate == 1 || ( $aad_row->is_certificate == '' && !is_numeric($aad_row->is_certificate) ) ) && $job_details['assigned_tech'] != 2 ) {

                // get compliance
                $compliance_api_ret = $this->property_tree_model->get_all_compliance($job_details['agency_id']);
        
                foreach($compliance_api_ret->json_decoded_response as $compliance_obj){
        
                    if( 
                        $compliance_obj->management_id == $prop_management_id &&
                        $compliance_obj->category_id == $agency_pref->prop_comp_cat && 
                        $compliance_obj->status == 'Open' &&    
                        $compliance_obj->deleted == false &&                    
                        $compliance_obj->serviced_by_creditor_id == $agency_pref->creditor
                    ){
                        ...
                        $params_obj = (object)[
                            'job_id' => $job_id,
                            'agency_id' => $job_details['agency_id'],
                            'compliance_id' => $compliance_obj->compliance_id
                        ];
                        $this->property_tree_model->complete_compliance($params_obj);
                ...
                $create_sa_comp_ret = $this->property_tree_model->create_smoke_alarm_compliance($params_obj);
                $create_sa_comp_decoded_response = $create_sa_comp_ret->json_decoded_response;
                $httpcode = $create_sa_comp_ret->httpcode;
                
                if( ( $httpcode == 200 || $httpcode == 201 || $httpcode == 202 ) && $create_sa_comp_decoded_response->property_compliance_id != '' ){
                    
                    // attach document to compliance
                    $params_obj = (object)[
                        'job_id' => $job_id,
                        'agency_id' => $job_details['agency_id'],
                        'property_compliance_id' => $create_sa_comp_decoded_response->property_compliance_id,
                        'certificate_pdf' => $certificate_pdf,
                        'bpay_ref_code' => $bpay_ref_code
                    ];
                    $att_doc_to_comp_ret = $this->property_tree_model->attach_document_to_compliance($params_obj);
                    $att_doc_to_comp_dec_resp = $att_doc_to_comp_ret->json_decoded_response;
                    $httpcode = $att_doc_to_comp_ret->httpcode;

                    if( $httpcode == 201 && $att_doc_to_comp_dec_resp->document_id != '' ){
                        ...
                        $params_obj = (object)[
                            'job_id' => $job_id,
                            'agency_id' => $job_details['agency_id'],
                            'compliance_id' => $create_sa_comp_decoded_response->property_compliance_id
                        ];
                        $this->property_tree_model->complete_compliance($params_obj);
                        ...
                        $this->property_tree_model->create_smoke_alarm_compliance($params_obj);
