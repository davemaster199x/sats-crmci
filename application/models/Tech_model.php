...
class Tech_model extends CI_Model
{
    ...
    public function getTechRunRows($tech_run_id, $country_id, $params = "")
    {
        ...
        $show_booked_only = $params['show_booked_only'];
        ...
        $agency_pref_filter = null;
        if ($params['dk_query_listing'] == 1) {

            $dk_query_listing = null;
            $agency_pref_filter = " AND a.`allow_dk` != 0 ";
        ...
        if (!empty($tr['tr_tech'])) {
            ...
            if (($tr['run_complete'] == 1 && $params['dk_query_listing'] != 1) ||  $params['display_only_booked'] == 1) {
                ...
                if( $show_booked_only == 1 ){ // 'booked' status only
    ...
    function getJobRowDataWithJobIds($jobIds, $countryId)
    {
        ...
        $result = $this->db->query("
            SELECT
                ...
                p.`comments` AS p_comments,
                p.`no_en`,
                p.`qld_new_leg_alarm_num` AS alarm_count,
                ...
                ajt.`bundle_ids` AS alarm_job_type_ids
    ...
    function getAgencyIDsDataWithJobIds($agencyIds)
    {
        if( empty($agencyIds) ){
            return [];
        }
        ...
        $result = $this->db->query("
            SELECT
                `id` AS key_address,
                `address_1` AS aad_address_1,
                `address_2` AS aad_address_2,
                `address_3` AS aad_address_3,
                `state` AS aad_state,
                `postcode` AS aad_postcode
            FROM agency_addresses
            WHERE `agency_id` IN ({$agencyIdsString})
        ");
        if ($result->num_rows() > 0) {
            return $result->result_array();
        }
    ...
    public function getTechRunKeysByIds($techRunKeysIds)
    {
        ...
        $sel_query = "thk.`tech_run_keys_id`, thk.`agency_addresses_id`, thk.`action`, thk.`date` as kdate, thk.`number_of_keys`, thk.`agency_staff`, thk.`completed`, thk.`completed_date`, thk.`sort_order`, thk.`signature_svg`,
    ...
    {
		// Troubleshooting empty $tech_run_id queries
	    $data = [
		    $tech_run_id,
		    $tech_id,
		    $date,
    ...
    public function getSTRnewlyAddedListing($tech_run_id, $tech_id, $date, $sub_regions, $country_id = '', $isAssigned = "", $display_only_booked = "", $agency_id = 0)
    {
    ...
    public function getTechID($jobID)
    {
        // check tech run
        $this->db->select("assigned_tech");
        $this->db->from('jobs');
