...
class Crmtasks_model extends CI_Model {
    ...
    function getButtonCrmTasks($params) {
        ...
        $this->db->join('`crm_task_sub_category` AS ctht', 'ct.`sub_category` = ctht.`id`', 'LEFT');
        $this->db->join('`crm_task_details_devs` AS ctdd', 'ct.`crm_task_id` = ctdd.ticket_id', 'LEFT');
        $this->db->join('`crm_task_category` AS ctc', 'ct.`category` = ctc.id', 'LEFT');
        ...
        if ( $params['help_topic'] > 0 ) {
            $this->db->where("ct.`sub_category` = {$params['help_topic']}");
        }
