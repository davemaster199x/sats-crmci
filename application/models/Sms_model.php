...
class Sms_model extends CI_Model {
    ...
    function parseTags($params) {
        ...
        if( $params['job_id'] > 0 ){
            ...
            if (is_numeric(strpos($params['unparsed_template'], '{en_link}'))) {
                ...
                $encrypt = rawurlencode(HashEncryption::encodeString($row->jid));
        ...
        $sats_domain = "https://www.{$this->config->item('base_domain')}";
        ...
        return Transliterator::create('Any-Latin; Latin-ASCII')->transliterate(str_replace($find, $replace, $params['unparsed_template']));
    }
    ...
    public function get_sms_missing_job($params){
        ...
        $this->db->select('*, snr.`name` AS snr_name');
        $this->db->from('`sms_api_replies` AS sar');
        $this->db->join("`sms_number_reference` AS snr", "sar.`sms_ref_id` = snr.`sms_ref_id`", "LEFT");

        if( !$params['show_all'] AND $params['show_all']!=1 ){
            $this->db->where('sar.`unread`',1);
        }

        if( $params['sms_ref_id'] > 0 ){
            $this->db->where('sar.`sms_ref_id`',$params['sms_ref_id']);
        }

        $this->db->where('sar.`active`',1);
        $this->db->where('sar.`message_id`',NULL);

        if ($params['phrase'] != "") {
            $this->db->group_start();
            $this->db->where("sar.`response` LIKE '%{$params['phrase']}%'");
            $this->db->or_where("sar.`mobile` LIKE '%{$params['phrase']}%'");
            $this->db->group_end();
        }

        $this->db->order_by('sar.`created_date`','DESC');
        if ($params['paginate'] != "") {
            if (is_numeric($params['paginate']['offset']) && is_numeric($params['paginate']['limit'])) {
                $this->db->limit($params['paginate']['limit'], $params['paginate']['offset']);
            }
	...
	/**
	 * This method is called on the tenants ajax template via VPD
	 * @param $job_id
	 * @return bool|mixed|string
	 */
	public function checkSmsforToday($job_id = 0){
        $sql = "SELECT * 
				 FROM  `job_log` 
     ...
     * @param mixed $to=array(name,mobile) > if empty default tenants data (name/mobile)
     * 
     * @return [bool]
     */
    public function vjd_send_sms($job_id,$prop_id,$sms_type,$to = NULL){
        ...
        $sql = $this->getSmsTemplates($params);
        $row = $sql->row();
        ...
        $parsed_template_body = $this->parseTags($sms_params);
        // get template content end


        //set mobile to
        if(empty($to)){ //empty to params > default to = tenants mobile

            //$tenant_mobile_arr = [];
            //$tenant_names_arr = [];
            $tenant_email_arr = [];
            $send_to_mobile_arr = [];
            $send_to_mobile_namarr = [];
            ...
            foreach ($pt_sql->result() as $pt_row) {
                ...
                if ($pt_row->tenant_firstname != '') {
                    ...
                    $send_to_mobile_namarr[] = $pt_row->tenant_firstname;
                ...
                if ($pt_row->tenant_mobile != '') {
                    ...
                    $send_to_mobile_arr[] = $this->formatToInternationNumber($pt_row->tenant_mobile); // format number  
        ...
        }else{ //custom to mobile

            foreach($to as $to_row){
                $send_to_mobile_arr[] = $this->formatToInternationNumber($to_row['mobile']); //format international number
                $send_to_mobile_namarr[] = $to_row['name'];
            }
        ...
        //send SMS
        if(!empty($send_to_mobile_arr)){ //mobile not empty - proceed to send SMS

            foreach (array_unique($send_to_mobile_arr) as $mobile_row) {
                // send SMS
                $sms_params = array(
                    'sms_msg'   => $parsed_template_body,
                    'mobile'    => $mobile_row
                );
                $sms_json = $this->sendSMS($sms_params);
    
                // save SMS data on database sms_api_sent or outgoing sms
                $sms_params = array(
                    'sms_json'  => $sms_json,
                    'job_id'    => $job_id,
                ...
                $this->captureSmsData($sms_params);
            }

            //update job sms_sent field
            $update_data = array(
            ...
            if($this->db->affected_rows()>0){
                ...
                ##insert log
                $send_to_mobile_name = implode(', ', $send_to_mobile_namarr);
                $indiv_logs_str = "Reminder SMS to {$send_to_mobile_name} <strong>{$parsed_template_body}</strong>";
                $log_params = array(
                    'title' => 63, // Job Update
