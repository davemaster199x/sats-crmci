...
class Figure_model extends CI_Model {
    ...
    public function jGetSales($params = []) {
        ...
        $ym_only = $params['ym_only'];
        $allow_ub = $params['allow_ub'];        
        ...
        // job type = YM
        $job_type_sql_filter = null;
        if( $ym_only == true ){
            $job_type_sql_filter = "AND j.`job_type` = 'Yearly Maintenance'";
        }

        // upfront billing/Subscription Billing
        $upfront_billing_sql_filter = null;
        if( $allow_ub == true ){
            $upfront_billing_sql_filter = "AND a.`allow_upfront_billing` = 1";
        }
