...
class Job_functions_model extends CI_Model
{
    ...
    /**
     * This function gets called from a lot of places with encrypted or decrypted job_ids
     * I have set it up to handle them all
     * @param $job_id
     * @param $query_only
    ...
    {
        // check that the job id is set, if not check the url
        $job_id = $job_id ?? $this->input->get('job_id');

        // if its not set at all, throw an error
        if(empty($job_id)){
            log_message('error', 'getJobDetails2 - job_id is empty, details: job_id:' . $job_id . ', ' . print_r($_SESSION, true));
            return [];
        }

        // check that the job_id is an integer, even if its a number within a string
        if(filter_var($job_id,FILTER_VALIDATE_INT ) === false){
            // For logging purposes
            $job_id_encrypted = $job_id;

            // If nots not a number, then it needs to be decrypted
            $job_id = HashEncryption::decodeString($job_id);
            if(filter_var($job_id,FILTER_VALIDATE_INT ) === false){
                // Still failed to get a job id, so lets log this
                log_message('error', 'getJobDetails2 - job_id could not be decrypted to an integer, details: job_id encrypted:' . $job_id_encrypted . ', ' . ' job_id:' . $job_id . ', ' . print_r($_SESSION, true));
                return [];
            }
        }
		...
		$query = $this->db->query(
			"SELECT j.id, DATE_FORMAT(j.date,'%d/%m/%Y') as date,
                        j.id AS jid,
						j.date as date_no_format,
						j.status,
						j.comments,
						j.retest_interval,
						j.auto_renew,
						...
						DATE_FORMAT(j.entry_notice_emailed,
						'%d/%m/%Y @ %r') AS EntryNoticeLastSent,
						sa.ContactNumber, 
