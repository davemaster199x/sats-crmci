...
class Palace_model extends CI_Model
{
        ...
        foreach ($listsPme as $val) {
            ...
            if( $is_not_compliant == false ){ // do not upload API, if not compliant
                ...
                if( $val['palace_api'] == $palace_api && $val['palace_prop_id'] != '' ){
                    ...
                    $this->upload_invoice_v2($upload_inv_params);
    ...
    public function upload_invoice($params){
        ...
        $this->add_upload_palace_documents($job_id);
        ...
        if ($res['response']) { // Api response no error
            ...
            if ( $suppRes['response'] && !empty($suppRes) ) {
                ...
                $this->add_create_palace_bills($job_id);
            }
    ...
    public function upload_invoice_v2($params)
    {
        $palace_prop_id = $params['palace_prop_id'];
        $agencyId = $params['a_id'];
        $job_id = $params['jid'];
...
//            $this->add_upload_palace_documents($job_id);
    ...
    public function upload_pdf_to_property($params) {
        ...
        // lod details
        $log_details = "<a href='".$protocol."://{$baseUrl}/pdf/certificates/{$encrypt}'>Certificate</a>, #{$bpay_ref_code} to the linked Palace property";
        $log_params = array(
            'title' => 70,  // Palace API
            'details' => $log_details,
            'display_in_vjd' => 1,
    ...
    public function upload_pdf_to_property_payload_only($params) {
        ...
        // insert job log
        $encrypt = rawurlencode($this->encryption->encrypt($job_id));
        $baseUrl = $_SERVER["SERVER_NAME"];
        if(isset($_SERVER['HTTPS'])){
            $protocol = ($_SERVER['HTTPS'] && $_SERVER['HTTPS'] != "off") ? "https" : "http";
        } else{
    ...
    public function add_upload_palace_documents($jobId) {
        $ret = $this->db->query("UPDATE jobs SET is_palace_invoice_upload = 1 WHERE `id` = {$jobId}");
        return $ret;
    }    

    public function add_create_palace_bills($jobId) {
    ...
    public function add_billing_on_upload_invoice($agencyId, $pmePropId, $jobId, $countryId, $job_details, $property_details, $alarm_details, $num_alarms) {
        ...
        if( $property_details['send_to_email_not_api'] == 1 ){ // send invoce to email instead of API
            ...
            if (!is_null($suppId) && !is_null($agentId) && floatval($getTotalAmount) > 0) {
                ...
                $invoice_pdf = $this->pdf_template->pdf_combined_template_v2($jobId, $job_details, $property_details, $alarm_details, $num_alarms, $countryId);
                ...
                $log_details = "<a href='".$protocol."://{$baseUrl}/pdf/invoices/{$encrypt}'>Invoice</a>, #{$invoice_number} uploaded to Palace Agency as a bill of {$getTotalAmount}";
                $log_params = array(
                    'title' => 70,  // Palace API
                    'details' => $log_details,
                    'display_in_vjd' => 1,
