...
class Daily_model extends CI_Model
{
    ...
    public function getOtherMultipleJobs($property_id,$jid,$service_type=null){

        $service_type_filter = null;
        if( $service_type > 0 ){
            $service_type_filter = "AND j.`service` = {$service_type}";
        }
        ...
        $sql = "
            ...
            AND a.`country_id` ={$this->config->item('country')}
            AND NOT (
                j.`job_type` = 'Yearly Maintenance' AND 
                j.`status` = 'Pending'
            )            
            {$service_type_filter}
        ";
    ...
    public function get_unserviced_by_markers( $start = null, $limit = null, $agency_filter = null, $job_type_filter = null, $is_show = null ){
        ...
        $filt_agency = '';
        if ($agency_filter != '') {
            $filt_agency = "AND `a`.`agency_id` = {$agency_filter} ";
        }

        $filt_job_type = '';
        if ($job_type_filter != '') {
            $filt_job_type = "AND `j`.`job_type` = '$job_type_filter'";
        }
        ...
        if ($is_show == 1) {
            ...
            $is_show_str = "AND ( hp.`hidden` = 0 OR hp.`hidden` IS  NULL ) AND (hp.hidden_from_pages = 1 OR hp.hidden_from_pages IS NULL)"; // 1 means unserviced page
        }
        ...
        return $this->db->query("
            ...
            LEFT JOIN `agency` AS a ON p.`agency_id` = a.`agency_id`
            LEFT JOIN `property_services` AS ps ON j.`property_id` = ps.`property_id`
            LEFT JOIN `hidden_properties` AS hp ON hp.`property_id` = p.`property_id` 
    ...
    public function get_recent_non_completed_job_per_property($property_id_arr_unique_imp) 
    {
        ...
        $recent_job_sql_str  = "
         ...
         INNER JOIN (
             ...
             AND ( j_inner.`status` != 'Completed' AND j_inner.`status` = 'To Be Booked' )
         ...
         AND a.`status` = 'active'
         AND a.`deleted` = 0    
         AND ( j.status != 'Completed' AND j.status = 'To Be Booked' )   
    ...
    public function hide_properties($params)
    {   
        if ($params['property_id'] == null) {
            return false;
        }
        ...
        $timestamp = date("Y-m-d H:i:s");
        $rresults = $this->check_if_hide_properties_id_exists($params['property_id']);

        if ($rresults > 0) {
            $data = array(
                'hidden'              => $params['is_acknowledge'],
                'hidden_from_pages'   => $params['hidden_from_pages'],
                'added_by'            => $params['staff_id'],
                'date_modified'       => $timestamp
            );

            $this->db->where('property_id', $params['property_id'] );
            $this->db->update('hidden_properties', $data);
        } else {
            $data = array(
                'property_id'         => $params['property_id'],
                'hidden'              => $params['is_acknowledge'],
            ...
            $this->db->where('property_id', $params['property_id'] );
            $this->db->insert('hidden_properties', $data);
        }
    ...
    public function check_if_hide_properties_id_exists($id)
    {
        $this->db->select("*")
    ...
    {
        $date_delay = date('Y-m-d', strtotime('-7 days'));
    ...
    public function get_dynamic_agency_name_on_multiple_jobs()
    {
        $query = $this->db->query("
            SELECT * FROM (
                SELECT COUNT( j.`id` ) AS jcount, `a`.`agency_name` , `a`.`agency_id`
                FROM `jobs` AS `j`
                LEFT JOIN `alarm_job_type` AS `ajt` ON j.`service` = ajt.`id`
                LEFT JOIN `property` AS `p` ON `p`.`property_id` = `j`.`property_id`
                LEFT JOIN `agency` AS `a` ON `a`.`agency_id` = `p`.`agency_id`
                ...
                GROUP BY `j`.`property_id` , `agency_id`
                HAVING `jcount` >=2
     ...
     */
    public function getJobType(string $args = "*")
    {
        return $this->db->select($args)->from("job_type")->get()->result();
    }
