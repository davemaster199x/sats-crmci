...
class Reports_model extends CI_Model
{
     ...
     * @return CI_DB_result
     */
    public function get_dirty_address(array $params = [])
    {
        if($params['sel_query'] && !empty($params['sel_query'])){
            $sel_query = $params['sel_query'];
        }else{
        ...
        /**
         * Filter Dirty Address with spaces
         */
        $like = ("(
                    LOWER(p.address_1) LIKE '% / %' OR
                    LOWER(p.address_1) LIKE '%/ %' OR
                    LOWER(p.address_1) LIKE '% /%' OR
                    LOWER(p.address_1) LIKE '%- %' OR
                    LOWER(p.address_1) LIKE '% -%' OR
                    LOWER(p.address_1) LIKE '% - %' OR
                    UPPER(p.address_1) LIKE '% / %' OR
                    UPPER(p.address_1) LIKE '%/ %' OR
                    UPPER(p.address_1) LIKE '% /%' OR
                    UPPER(p.address_1) LIKE '%- %' OR
                    UPPER(p.address_1) LIKE '% -%' OR
                    UPPER(p.address_1) LIKE '% - %'
                 )");
        $like2 = ( " (LOWER(p.address_2) LIKE '% - %' OR LOWER(p.address_2) LIKE '%- %' OR LOWER(p.address_2) LIKE '% -%') OR (UPPER(p.address_2) LIKE '% - %' OR UPPER(p.address_2) LIKE '%- %' OR UPPER(p.address_2) LIKE '% -%') " ); // address2 hypen
        $like3 = ( " (LOWER(p.address_2) LIKE '%,%' OR LOWER(p.address_3) LIKE '%,%' ) OR (UPPER(p.address_2) LIKE '%,%' OR UPPER(p.address_3) LIKE '%,%' )" ); // comma
        $like4 = ( " (LOWER(p.address_2) LIKE '%(%' OR LOWER(p.address_2) LIKE '%)%' OR LOWER(p.address_3) LIKE '%(%' OR LOWER(p.address_3) LIKE '%)%' ) 
                        OR (UPPER(p.address_2) LIKE '%(%' OR UPPER(p.address_2) LIKE '%)%' OR UPPER(p.address_3) LIKE '%(%' OR UPPER(p.address_3) LIKE '%)%' )" ); // ()
       ...
       //$escape_Hawkes_bay2 = $this->db->escape("Hawkeâ€™s Bay");
        ...
        /**
         * @Note Prefer NOT IN instead of p.state != "Auckland" etc.
         */
        $nz_staten_not_equal = "p.state NOT IN (
            'Auckland', 'Bay of Plenty', 'Canterbury', 'Northland', 'Otago',
            'Waikato', 'Whangarei', 'Manawatu-Wanganui', 'Southland', 'Wellington',
            'Gisborne', 'Taranaki', {$escape_Hawkes_bay}, 'Manawatu', 'Tasman',
            'Nelson', 'Marlborough'
        )";

        $has_space_at_start_and_end_of_string = "p.address_2 LIKE '% '";
    ...
    public function get_sats_serviced_property_service_price_sum_via_postcode_v2($params){
        ...
        if( count($postcode_arr) > 0 ){
            ...
            foreach( $sql->result() as $row ){
                ...
                if( $price_var_arr['price'] > 0 ){
                    $ps_price_tot += $price_var_arr['price'];
                }                
    ...
    public function getComplianceReportData($params)
    {
        ...
        if (!empty($startDate)) {
            $query .= "AND j.`date` >= '$startDate' ";
        }
        if (!empty($endDate)) {
            $query .= "AND j.`date` <= '$endDate' ";
        }
        if (!empty($agency)) {
