...
class Kms_model extends CI_Model
{
    ...
    /**
     * PULL KMS DATA BY VEHICLE ID
     * @param $params | ARRAY | array of data to use for select
     * @return $query | RESULT | result from the database
     * */
    public function getKmsByVehicleId($vehicle_id, $upload, $param = []) {
        $this->db->select('kms_id, vehicles_id, kms, kms_updated, image_url');
        $this->db->from('kms');
        $this->db->where('vehicles_id', $vehicle_id);

        if(isset($param["kms"]) && !empty($param["kms"])) {
            $this->db->where('kms', $param["kms"]);
        }

        if(isset($param["date"]) && !empty($param["date"])) {
            $this->db->where("DATE_FORMAT(kms_updated, '%Y-%m-%d') = ", $param["date"]);
            $this->db->order_by("kms_updated", "DESC");
        } else {
            $this->db->order_by("kms_id", "DESC");
        }
