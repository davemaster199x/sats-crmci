...
class System_model extends CI_Model {
    ...
    public function __construct() {
        ...
        // ACCESS CHECKING - IF PROD AND NOT AJAX
        if(
    ...
    function getSMStemplate($params) {
        ...
        switch ($params['sms_type']) {
            ...
            case 10: // Entry Notice, SMS EN
                $sms_temp = config_item('company_full_name') . '(' . config_item('company_name_short') . ") have issued you an Entry Notice to test the {$params['serv_name']} at {$params['paddress']} on {$params['jdatetemp']} and will collect the keys from your Real Estate. Click here to view <link>";
                break;
            case 9: // Entry Notice, SMS EN
                $sms_temp = config_item('company_full_name') . '(' . config_item('company_name_short') . ") {$params['serv_name']} at {$params['paddress']} on {$params['jdatetemp']} and will collect the keys from your agency. Email may appear in Spam/Junk folders. View this Entry Notice by clicking this link <link>";
                break;
        }
    ...
    public function insert_job_markers($job_id,$new_job_type){
        ...
        if( $job_id > 0 ){
            ...
            log_message('error', 'system_model.insert_job_markers: empty/invalid job id');
        }             
    ...
    function getJobsData($params) {
        ...
        // copied style from above
        // extra job notes table
        $extra_field = '';
        if ($params['extra_job_notes_join'] == 1) {
            $extra_field = '
    ...
    public function isVIP($staff_id) {
        ...
        $vip = array(2025, 11, 2155); //add/edit VIP value
        ...
        if( $this->config->item('country') == 1 ){ // AU
            ...
            if( ENVIRONMENT == 'production' ){ // live
                ...
                */
                ...
                return array(2070, 2025, 11, 2287, 2175, 2495);
                ...
                2222 - Peter Chodyra
                */
                return array(2070, 2025, 11, 2287, 2175, 2222);
            ...
            */
            ...
            return array(2070, 2025, 11, 2231, 2193, 2325);
    ...
    public function getServiceCount($agency_id, $ajt) {
    ...
    public function access_check() {
        ...
        if (
            $this->router->directory == "api/" || 
            $this->router->directory == "api/v2/"
        ) {
            return;
        }
        ...
        $pages = [
			...
			'custom/duplicate_properties',
            'reports/view_crm_tasks',
            'reports/ticket_details',
            'properties/details',
            'agency/view_agency_details',
            'jobs/details',
            'logviewer/index',
            'sms/wholesalesms_delivery',
            'sms/wholesalesms_reply',
		];
            ...
            if (isset($this->session->staff_id) && $this->session->staff_id > 0){
                ...
                if ( !in_array($this->session->staff_id, $allowed_direct_access) || $url == 'admin/passwords' ){ 
                    ...
                    if (!isset($_SERVER['HTTP_REFERER'])) {
                        log_message('error', 'Staff ID #' . $this->session->staff_id . ' tried to direct access: ' . $url);
                        header('location: /home/index?no-direct-access');
                    }
                }

            } else {
                // if session expired kick out
                $this->session->sess_destroy();
                redirect('/');
                exit();
            }
    ...
    public function display_job_icons($params){
		$this->load->model('jobs_model');
        $base_url = '';
		if($params['display_in_email']){
			$base_url = rtrim('/', config_item('base_url'));
		}

		// Combined v2 into this method - if either of these values are blank then job id should be specified
		if($params['job_id'] > 0 ){
			$params = $this->jobs_model->fields('job_type,service as service_type,is_eo')->as_array()->get($params['job_id']);
		}

	    if(!empty($params['service_type'])) {
		    $icons_str = Alarm_job_type_model::icons($params['service_type']);
	    }

	    if(!empty($params['job_type'])){
			switch($params['job_type']){
				case 'IC Upgrade':
					$icons_str .= '<img data-toggle="tooltip" title="'.$params['job_type'].'" src="'.$base_url.'/images/icons-jobs/upgrade_colored.png" class="j_icons" />';
					break;
				case '240v Rebook':
					$icons_str .= '<img data-toggle="tooltip" title="'.$params['job_type'].'" src="'.$base_url.'/images/icons-jobs/240v_colored.png" class="j_icons" />';
					break;
				case 'Fix or Replace':
					$icons_str .= '<img data-toggle="tooltip" title="'.$params['job_type'].'" src="'.$base_url.'/images/icons-jobs/fr_colored.png" class="j_icons" />';
					break;
			}
        }

	    if( !empty($params['is_eo'])){
		    $icons_str .= '<img data-toggle="tooltip" title="Electrician Only" src="'.$base_url.'/images/icons-jobs/EO_colored.png" class="j_icons" />';
	    }
    ...
    /**
     * This function tells techs to use a Cavius alarm, unless they have free orca set
     * @param $agencyIds
     * @return array
     */
...
//            ];
            ...
            // Create an array that indicates whether or not there is a free orca set, or if there is a cavius set at all for the agency
            foreach( $result->result() as $row ){
                if( $row->alarm_make == 'Orca' && $row->price == 0 ){
                    $alarms[$row->agency_id]['free'] = true;
                }

                if( $row->alarm_make == 'Cavius' ){
                    $alarms[$row->agency_id]['cavi'] = true;
                }
            }

            // By default they will use Cavius, unless they dont have any cavius and the orca is $0
    ...
    public function get_property_price_variation($params){
        ...
        }else if( $prop_row->holiday_rental == 1 && isset($stsp_sql) && $stsp_sql->num_rows() > 0 ){ // short term service price
        ...
        $dynamic_price_text = '$'.number_format($dynamic_price,2);
        $price_text = '$'.number_format($dynamic_price_total,2);
        $price = number_format($dynamic_price_total,2);
        $price_breakdown_text = '$'.number_format($dynamic_price,2).$price_variation_total_str.$final_total_str;
    ...
    public function get_subscription_valid_date_range($property_id){
        ...
        if( $property_id > 0 ){
            ...
            if( $prop_subs_sql->num_rows() > 0 ){
                ...
                $sub_valid_from = date('Y-m-d', strtotime($row->jdate));
                $sub_valid_to = date('Y-m-d', strtotime("{$row->jdate} +364 day"));
    ...
    public function get_job_variations_v2($params){
        ...
        $dynamic_price = 0;
        $dynamic_invoice_amount = 0;
        $price_variation_total = 0;
        $price_variation_total_rev = 0;
        $price_variation_total_display_on = 0;
        $computed_to_price = 0;
        $ret_arr = [];
        $display_var_arr = [];
        ...
        $job_row = $job_sql->row();
        $dynamic_price = $job_row->job_price;
        $dynamic_invoice_amount = $job_row->invoice_amount;
            ...
            foreach( $jv_sql->result_array() as $jv_row ){
                ...
                if( $jv_row['type'] == 1 ){  ## Discount
                    ...
                    if( $dynamic_price>0 ){ //apply discount if price !=0 to avoid negative total
                        ...
                        $price_variation_total-=$jv_row['amount'];
                        $price_variation_total_rev+=$jv_row['amount'];
            ...
            foreach( $pv_sql->result() as $pv_row ){  
                ...
                if( $pv_row->apv_type == 1 ){ // discount
                    ...
                    if( $dynamic_price>0 ){ //apply discount if price !=0 to avoid negative total
                        ...
                        $price_variation_total-=$pv_row->amount;
                        $price_variation_total_rev+=$pv_row->amount;
            ...
            foreach( $apv_sql->result() as $apv_row ){  
                ...
                if( $apv_row->apv_type == 1 ){ // discount
                    ...
                    if( $dynamic_price>0 ){ //apply discount if price !=0 to avoid negative total
                        ...
                        $price_variation_total-=$apv_row->amount;
                        $price_variation_total_rev+=$apv_row->amount;
        ...
        //job variation for tweak for VJD
        $dynamic_invoice_amount_total = $dynamic_invoice_amount+$price_variation_total_rev;
        $price_breakdown_text = '$'.number_format($dynamic_invoice_amount_total,2).$price_variation_total_str.' = $'.number_format($dynamic_invoice_amount,2);
        //job variation for tweak for VJD end
        ...
        return $ret_arr = array(
            ...
            'display_var_arr' => $display_var_arr, ##array of variations
            'dynamic_price_included_not_show_var' => $dynamic_price_included_not_show_var,
            'price_breakdown_text' => $price_breakdown_text, ## for VJD price
            'dynamic_price_total_2' => $dynamic_invoice_amount_total, ##for VJD price var popup
            'price_variation_total' => $price_variation_total, ##reserve for testing > variation total regardless if display on/off
            'dynamic_price_total' => $dynamic_price_total ##reserve for testing
    ...
    public function parseDynamicLink($params) {
        ...
        if( strpos($log_row->details, $tag) !== false ) { 
            ...
            if( $log_row->property_id > 0 ){
                ...
                // property link
                $vpd_link = "<a href='/properties/details/?id={$log_row->property_id}'>{$prop_row->address_1} {$prop_row->address_2} {$prop_row->address_3}</a>";
    ...
    public function get_job_price_breakdown($params)
    {
        $job_id = $params['job_id'];
        $service_type = $params['service_type'];
        $property_id = $params['property_id'];
        ...
        if( $jv_sql->num_rows() > 0 ){
            ...
            foreach( $jv_sql->result() as $jv_row ){
                if( $jv_row->type == 1 ){ // discount
        ...
        $pv_sql = $this->db->query("
            ...
            if( $pv_sql->num_rows() > 0 ){
                ...
                foreach($pv_sql->result() as $apv_row) {
                    $service_type_str = ( $pv_row->scope >= 2 ) ? "{$pv_row->short_name} Service " : null;
                     // calculation in reverse
                     if( $pv_row->apv_type == 1 ){ // discount
        ...
        $dynamic_price_text = '$'.number_format($dynamic_price,2);
        $price_text = '$'.number_format($dynamic_price_total,2);
        $price_normal = number_format($dynamic_price_total,2);
    ...
    public function getInvoicePaymentsData($params) {
        ...
        if ($params['join_table'] > 0) {
            foreach ($params['join_table'] as $join_table) {
                ...
                if ($join_table == 'jobs') {
                    $this->db->join('`jobs` AS j', 'ip.`job_id` = j.`id`', 'left');
                }
                if ($join_table == 'created_by_who') {
                    $this->db->join('`staff_accounts` AS sa', 'ip.`created_by` = sa.`StaffID`', 'left');
                }
        ...
        $this->db->where('ip.invoice_payment_id>',0);

        if ($params['ip_id'] != "") {
            $this->db->where('ip.invoice_payment_id',$params['ip_id']);
        }

        if ($params['active'] != "") {
            $this->db->where('ip.active',$params['active']);
        }

        if ($params['job_id'] != "") {
            $this->db->where('ip.job_id',$params['job_id']);
        }

        if ($params['payment_date'] != "") {
            $this->db->where('ip.payment_date',$params['payment_date']);
        }

        if ($params['type_of_payment'] != "") {
            $this->db->where('ip.type_of_payment',$params['type_of_payment']);
        }

        if ($params['created_by'] != "") {
            $this->db->where('ip.created_by',$params['created_by']);
        }
    ...
    public function can_edit_account($permission){

        $this->db->select("COUNT('id') AS sp_count");
        $this->db->from('staff_permissions');
        $this->db->where('staff_id', $this->session->staff_id);
        $this->db->where('has_permission_on',$permission);
        $q = $this->db->get();
        $row = $q->row();
    ...
    public function can_add_account($class_id){
    ...
    public function getInvoiceRefundsData($params){
        ...
        if ($params['join_table'] > 0) {
            foreach ($params['join_table'] as $join_table) {
                if ($join_table == 'jobs') {
                    $this->db->join('`jobs` AS j', 'ir.`job_id` = j.`id`', 'left');
                }
                if ($join_table == 'created_by_who') {
                    $this->db->join('`staff_accounts` AS sa', 'ir.`created_by` = sa.`StaffID`', 'left');
                }
                if ($join_table == 'payment_types') {
        ...
        $this->db->where('ir.invoice_refund_id>',0);

        if ($params['ip_id'] != "") {
            $this->db->where('ir.invoice_refund_id',$params['ip_id']);
        }

        if ($params['active'] != "") {
            $this->db->where('ir.active',$params['active']);
        }

        if ($params['job_id'] != "") {
            $this->db->where('ir.job_id',$params['job_id']);
        }

        if ($params['payment_date'] != "") {
            $this->db->where('ir.payment_date',$params['payment_date']);
        }

        if ($params['type_of_payment'] != "") {
            $this->db->where('ir.type_of_payment',$params['type_of_payment']);
        }

        if ($params['created_by'] != "") {
            $this->db->where('ir.created_by',$params['created_by']);
        }
    ...
    public function getInvoiceCreditsData($params){
        ...
        if ($params['join_table'] > 0) {
            foreach ($params['join_table'] as $join_table) {
                if ($join_table == 'jobs') {
                    $this->db->join('`jobs` AS j', 'ic.`job_id` = j.`id`', 'left');
                }
                if ($join_table == 'created_by_who') {
                    $this->db->join('`staff_accounts` AS sa_who', 'ic.`created_by` = sa_who.`StaffID`', 'left');
                }
                if ($join_table == 'approved_by') {
                    $this->db->join('`staff_accounts` AS sa_ab', 'ic.`approved_by` = sa_ab.`StaffID`', 'left');
                }
        ...
        $this->db->where('ic.invoice_credit_id>',0);

        if ($params['ip_id'] != "") {
            $this->db->where('ic.invoice_credit_id',$params['ip_id']);
        }

        if ($params['active'] != "") {
            $this->db->where('ic.active',$params['active']);
        }

        if ($params['job_id'] != "") {
            $this->db->where('ic.job_id',$params['job_id']);
        }

        if ($params['credit_date'] != "") {
            $this->db->where('ic.credit_date',$params['credit_date']);
        }

        if ($params['approved_by'] != "") {
            $this->db->where('ic.approved_by',$params['approved_by']);
        }


        if ($params['created_by'] != "") {
            $this->db->where('ic.created_by',$params['created_by']);
        }
    ...
    public function getPaymentTypes($params) {
    ...
    public function getCreditReason_v2($credit_reason_id=null) {
    ...
    public function get_leave_types( $param_obj = null ){

        $filter_sql_arr = []; 
        $filter_sql_imp = null;
        $custom_where = null;

        if( $param_obj->leave_type_id > 0 ){
            $filter_sql_arr[] = "AND `leave_type_id` = {$param_obj->leave_type_id}";
        }

        if( $param_obj->ex_not_show == true ){
            $filter_sql_arr[] = "AND `hidden` = 0";
        }

        if( count($filter_sql_arr) > 0 ){
            $filter_sql_imp = implode(" ",$filter_sql_arr);
        }

        if( $param_obj->custom_where != '' ){
            $custom_where = $param_obj->custom_where;
        }
