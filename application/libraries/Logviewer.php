...
class Logviewer {
    ...
    private $LOG_VIEW_FILE_NAME = "template.php";
    private $LOG_VIEW_FILE_PATH = "";

    //this is the name of the view file passed to CI load->view()
    const CI_LOG_VIEW_FILE_PATH = "_blade/logviewer/template";
    ...
    private function processLogs($logs) {
        ...
        foreach ($logs as $log) {
            ...
            if(!empty($logLineStart)) {
				...
				// Custom - cut the log msg off at the first EOL
				$extra = strpos($logMessage, PHP_EOL);
				if($extra){
					$data['content'] = $extra . substr($logMessage, 0, $extra);
					$data["extra"] = substr($logMessage, $extra+1);
				} else if(strlen($logMessage) > self::MAX_STRING_LENGTH) {
                    $data['content'] = $extra . substr($logMessage, 0, self::MAX_STRING_LENGTH);
