...
class JcClass {
	...
	*/
	...
	public function parseDynamicLink2($logObj){
		...
		if( strpos($log_details,$tag) !== false ){
			...
			if( $logObj->p_address_1 ){
				$vpd_link = "<a href='/properties/details/?id={$logObj->property_id}'>{$logObj->p_address_1} {$logObj->p_address_2} {$logObj->p_address_3}</a>";
	...
	public function getQldUpgradeQuoteAmount($params){
		...
		$quote_price = ( $price_240vrf > 0 ) ? $price_240vrf : $this->CI->config->item('fallback_price_for_ic_alarms_without_a_price_set');;
