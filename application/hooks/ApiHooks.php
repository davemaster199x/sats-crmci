			...
			if ($inApi) {
				...
				if (!in_array($action, $allowedActions) && !in_array('*', $allowedActions)) {
					...
					if ($authorization) {
						...
						// check if JWT token created todday, if its not current date then ask user to login again
						if (
							empty($validatedToken->createdAt) ||
							date("Y-m-d") != date("Y-m-d", $validatedToken->createdAt)
						) {
							// if its not current date then throw invalid token error
